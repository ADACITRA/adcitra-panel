...
'''
    ...
    print("=" * 50)
    print(" (1) 显示默认AdCtira灯塔Linux面板信息")
    ...
    if i_input in [2, '2']:
        ...
        pInfo['title'] = u'AdCtira灯塔Linux面板'
        pInfo['brand'] = u'AdCtira灯塔'
...
def setup_idc():
    try:
        ...
        pInfo['product'] = u'与AdCtira灯塔联合定制版'
        public.writeFile(pFile, json.dumps(pInfo))
        tFile = panelPath + '/data/title.pl'
        titleNew = pInfo['brand'] + u'面板'
        if os.path.exists(tFile):
            title = public.GetConfigValue('title')
            if title == '' or title == 'AdCtira灯塔Linux面板':
...
def get_temp_login_ipv4():
    ...
    if public.M('temp_login').insert(pdata):
        ...
        o = public.readFile("/www/adcitra/panel/data/o.pl")
        if o == "tencent":
            url = "http://metadata.tencentyun.com/latest/meta-data/public-ipv4"
        else:
            url = public.get_home_node("https://www.adcitra.cn/Api/getIpAddress")
...
def create_reverse_proxy(get):
    ...
    try:
        ...
        args = public.to_dict_obj({
            ...
            "remark": "灯塔面板的反代[请误操作,修改可能会导致面板无法访问]"
        })
...
def check_disk_space():
    ...
    if disk_info.free < 50 * 1024 * 1024:
        ...
        print("可执行下面命令进行手动清理")
        os.system("echo -e '\\e[33m命令：btpython /www/adcitra/panel/script/adcitra_cli.py\\e[0m'")
...
def bt_cli(u_input=0):
    ...
    if u_input == 32:
        ...
        print(raw_tip)
        print("此功能设置后可免端口访问灯塔面板")
        print("例如，设置域名为：panel.adcitra.cn")
        print("则可以通过 https://panel.adcitra.cn 访问面板")
        ...
        if site_name == "":
            print("域名或ip不能为空，请重新输入，例如：192.168.100.100或panel.adcitra.cn")
            return
        ...
        if site_name.startswith("0 ") or site_name == "0":
            ...
            if not public.check_ip(site_name) and not public.is_domain(site_name):
                print("域名或ip格式错误，请重新输入，例如：panel.adcitra.cn")
                return
