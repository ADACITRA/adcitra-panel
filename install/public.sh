...
pyenv_bin=/www/adcitra/panel/pyenv/bin
...
NODE_FILE_CHECK=$(cat /www/adcitra/panel/data/node.json |grep 125.88.182.172)
if [ "${NODE_FILE_CHECK}" ];then
	rm -f /www/adcitra/panel/data/node.json
fi

if [ -f "/www/adcitra/panel/install/d_node.pl" ];then
	LOCAL_DATE=$(date +%Y-%m-%d)
	FILE_DATE=$(stat /www/adcitra/panel/install/d_node.pl|grep Change|awk '{print $2}')
	if [ "${LOCAL_DATE}" != "${FILE_DATE}" ];then
		rm -f /www/adcitra/panel/install/d_node.pl
	else
		test_url=$(cat /www/adcitra/panel/install/d_node.pl)
...
bt_check(){
	p_path=/www/adcitra/panel/class/panelPlugin.py
	if [ -f $p_path ];then
		is_ext=$(cat $p_path|grep adcitrawaf)
		if [ "$is_ext" != "" ];then
			send_check
		fi
	fi
	
	p_path=/www/adcitra/panel/AdCtiraPanel/templates/default/index.html
	if [ -f $p_path ];then
...
send_check(){
	...
	p_path2=/www/adcitra/panel/class/common.py
	p_version=$(cat $p_path2|grep "version = "|awk '{print $3}'|tr -cd [0-9.])
	curl -sS --connect-timeout 3 -m 60 https://www.adcitra.cn/api/panel/notpro?version=$p_version
	NODE_URL=""
	exit 0;
}
GetSysInfo(){
...
if [ -d "/www/server/phpmyadmin/pma" ];then
	...
	EN_CHECK=$(cat /www/adcitra/panel/config/config.json |grep English)
	if [ "${EN_CHECK}" ];then
		curl https://download.adcitra.cn/install/update6_en.sh|bash
	else
		curl https://download.adcitra.cn/install/update6.sh|bash
	fi
	echo > /www/adcitra/panel/data/restart.pl
fi
