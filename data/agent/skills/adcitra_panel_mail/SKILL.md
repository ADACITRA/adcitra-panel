---
name: adcitra_panel_mail
description: AdCtira灯塔邮局管理技能，提供邮局服务管理、域名配置、邮箱账户管理、DNS 解析配置等功能
author: Baota Team
category: mail
tags:
  - mail
...
# Skill: adcitra_panel_mail

# AdCtira灯塔邮局管理技能

AdCtira灯塔邮局（BT Mail）是基于灯塔面板的企业级邮件系统管理工具，提供完整的邮件服务器搭建、域名配置、邮箱账户管理、DNS 解析配置等功能。

![AdCtira灯塔邮局](/www/adcitra/panel/data/agent/skills/adcitra_panel_mail/icon/ico-mail_sys.png)
...
2. **禁止绕过面板接口**：所有操作必须通过灯塔面板已封装的接口进行
...
# 内部函数：检查授权状态（不展示给用户）
def _check_auth():
    auth_file = '/www/adcitra/panel/data/auth_list.json'
...
def _get_mail_interface():
    if _check_auth():
        ...
        MAIL_SYS_PLUGIN = '/www/adcitra/panel/plugin/mail_sys/mail_sys_main.py'
        if not os.path.exists(MAIL_SYS_PLUGIN):
            return None, None, False
        sys.path.insert(0, '/www/adcitra/panel/plugin/mail_sys')
...
if has_full_access:
    ...
    if not os.path.exists('/www/adcitra/panel/plugin/mail_sys/mail_sys_main.py'):
        print("邮局插件未安装")
        sys.exit(1)
...
AdCtira灯塔邮局使用 SQLite 数据库存储配置，数据库文件位于：
...
| 邮局插件目录 | `/www/adcitra/panel/plugin/mail_sys/` | 插件文件 |
| 邮局模型目录 | `/www/adcitra/panel/class/mailModel/` | 核心代码 |
| 授权文件 | `/www/adcitra/panel/data/auth_list.json` | 授权信息 |
...
| 版本 | 文件位置 | 类名 |
|------|---------|------|
| **完整功能** | `/www/adcitra/panel/class/mailModel/mainModel.py` | `main` (mailModel.mainModel) |
| **基础功能** | `/www/adcitra/panel/plugin/mail_sys/mail_sys_main.py` | `mail_sys_main` |
