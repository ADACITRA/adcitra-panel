...
本文档记录 `adcitra_panel_runtime` 技能安装运行环境时必须遵守的源码入口、参数和完成判断规则。
...
- 不绕过项目模型或插件方法。
- 脚本必须创建 Flask request context，并使用 `from AdCtiraPanel import app, session`。
...
```text
/www/adcitra/panel/plugin/nodejs
```
...
```text
/www/adcitra/panel/plugin/dotnet
```
...
不得直接在无上下文环境中调用依赖 `session` 的面板方法；不得使用 `from flask import session` 替代 `AdCtiraPanel.session`。
