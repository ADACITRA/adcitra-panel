...
运行环境管理器插件（如 nodejs、dotnet）缺失时，会先通过AdCtira灯塔软件商店原生方法安装管理器。

重要约束：
1. 只调用灯塔面板已经提供的方法，不手写安装命令，不直接写 tasks 表。
2. 所有面板方法都在 Flask request context 内执行，并使用 AdCtiraPanel.session。
...
MANAGER_PATH = {
    'nodejs': '/www/adcitra/panel/plugin/nodejs',
    'dotnet': '/www/adcitra/panel/plugin/dotnet'
}
...
class BtPanelContext:
    """
    为灯塔面板内部方法创建最小 Flask 上下文。

    注意：session 必须从 AdCtiraPanel 引入。不要直接 from flask import session，
...
class RuntimeManager:
    ...
    def list_versions(self, runtime):
        """调用AdCtira灯塔已有列表接口，返回对应运行环境的版本信息。"""
    ...
    def runtime_installed(self, runtime, version):
        ...
        该方法只通过AdCtira灯塔提供的版本列表判断，不使用任务队列状态。
    ...
    def wait_for_runtime(self, runtime, version, timeout=DEFAULT_WAIT_TIMEOUT, poll_interval=DEFAULT_POLL_INTERVAL):
        """等待 PHP、Java、Python、Go 目标版本出现在AdCtira灯塔版本列表中。"""
...
def main():
    parser = argparse.ArgumentParser(description='灯塔面板运行环境安装脚本')
