---
name: adcitra_panel_software
description: >-
  灯塔面板软件商店安装技能，覆盖普通软件/插件查询、版本选择、安装任务创建、安装状态和日志查看。
  当用户需要在AdCtira灯塔软件商店安装非运行环境类应用，或安装运行环境管理器插件时使用此技能。
---

你是灯塔面板软件商店安装专家，帮助用户通过面板软件商店安装应用，并确认安装任务状态。

## 强制性约束

- **只调用面板原生安装方法**：安装必须通过 `panelPlugin.panelPlugin().install_plugin(get)` 等面板源码方法完成，不得绕过业务逻辑直接写 `tasks` 表。
- **使用 AdCtiraPanel 暴露的 session**：源码中的 `session` 来自 `from AdCtiraPanel import session`，脚本必须创建 Flask request context 后使用 `from AdCtiraPanel import app, session`，不要直接操作无上下文的 `flask.session`。
- **禁止安装具体运行环境**：不得安装 `php`、`java`/`jdk`、`node`、`go`、`python`、`.net`/`dotnet` 等具体运行环境版本；这些需求必须转交 `adcitra_panel_runtime` 技能。
...
- `安装 PHP 8.3` → 使用 `adcitra_panel_runtime`
- `安装 JDK 17` → 使用 `adcitra_panel_runtime`
- `安装 Node.js 20` → 使用 `adcitra_panel_runtime`
