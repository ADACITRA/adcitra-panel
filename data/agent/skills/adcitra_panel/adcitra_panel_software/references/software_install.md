...
灯塔面板软件商店查询、安装、状态和日志查看指南。本技能只安装普通软件/插件和运行环境管理器插件，不安装具体运行环境版本。
...
| 禁止示例 | 正确技能 |
|----------|----------|
| 安装 PHP 8.3 | `adcitra_panel_runtime` |
| 安装 JDK 17 | `adcitra_panel_runtime` |
| 安装 Node.js 20 | `adcitra_panel_runtime` |
| 安装 Go 1.22 | `adcitra_panel_runtime` |
| 安装 Python 3.12 | `adcitra_panel_runtime` |
| 安装 .NET 8 | `adcitra_panel_runtime` |
...
如果用户要安装具体版本，应切换到 `adcitra_panel_runtime`，由运行环境管理技能先确保管理器存在，再调用面板项目模型或插件方法安装具体版本。
...
脚本调用面板方法前必须创建 Flask request context，并使用 `AdCtiraPanel.session`：

```python
from AdCtiraPanel import app, session
...
1. 初始化 `/www/adcitra/panel` 运行环境和 `AdCtiraPanel.session`
...
```json
{
  ...
  "install_result": {
    ...
    "file_name": "/www/adcitra/panel/logs/installed/mysql-5.7_xxx.log",
    "task_id": 82
  }
}
```
