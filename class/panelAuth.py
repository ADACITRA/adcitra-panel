...
# AdCtira灯塔Linux面板
# -------------------------------------------------------------------
# Copyright (c) 2015-2019 AdCtira灯塔软件(http:#adcitra.cn) All rights reserved.
# -------------------------------------------------------------------
...
class panelAuth:
    ...
    def create_plugin_other_order(self, get):
        ...
        try:
            ...
            if pdata['uid'] == -1:
                return public.returnMsg(False, '请先绑定AdCtira灯塔官网帐号!')
    ...
    def get_plugin_price(self,get):
        ...
        try:
            userPath = 'data/userInfo.json'
            if not os.path.exists(userPath):
                return public.returnMsg(False,'请先登陆AdCtira灯塔官网帐号!')
    ...
    def send_cloud_v2(self, module, params):
        ...
        cloudURL = 'https://www.adcitra.cn/api/'
        userInfo = self.create_serverid(None);
        if 'status' in userInfo:
            params['uid'] = 0
            params['serverid'] = ''
    ...
    def receive_products(self, get):
        ...
        try:
            ...
            if not isinstance(u, dict):
                return public.returnMsg(False, '请先绑定AdCtira灯塔账号')
            ...
            url = 'https://www.adcitra.cn/newapi/activity/panelapi/receive_products'
    ...
    def get_free_activity_info(self, get):
        ...
        try:
            ...
            url = 'https://www.adcitra.cn/newapi/activity/panelapi/get_free_activity_info'
            res = public.httpPost(url, payload)
            if not res:
                return public.returnMsg(False, '连接服务器失败')
            try:
