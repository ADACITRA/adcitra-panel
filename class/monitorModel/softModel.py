...
class main(monitorBase):
    ...
    def __is_installation(self, name):
        map = {
            ...
            "php": "/www/server/php/",
            "tamper_proof": "/www/adcitra/panel/plugin/tamper_proof",
            "bt_security": "/www/adcitra/panel/plugin/bt_security",
            "syssafe": "/www/adcitra/panel/plugin/syssafe",
            "tamper_proof_refactored": "/www/adcitra/panel/plugin/tamper_proof_refactored",
        }
        if name in map:
            if name == 'php':
    ...
    def __get_tamper_proof_log_path(self) -> str:
        try:
            log_path = '/www/adcitra/panel/plugin/tamper_proof/service.log'
            if os.path.exists(log_path):
                return log_path
            else:
                return ''
    ...
    def __new_get_tamper_proof_log_path(self) -> str:
        try:
            log_path = '/www/adcitra/panel/plugin/tamper_proof_refactored/service.log'
            if os.path.exists(log_path):
                return log_path
            else:
                return ''
    ...
    def soft_log_list(self, get):
        try:
            ...
            name_map = {
                ...
                'nginx': 'Nginx', 'php': 'Php', 'dockerd': 'Docker', 'tamper_proof': '网站防篡改程序-旧版', 'bt_security': '堡塔防入侵', 'syssafe': 'AdCtira灯塔系统加固',
