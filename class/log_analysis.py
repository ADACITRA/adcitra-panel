...
def save_analysis(func):
    def wrapper(*args, **kwargs):
        ...
        if result['data']['start_time'] != "2022/2/22 22:22:22":
            if not os.path.exists('/www/adcitra/panel/data/analysis.json'):
                data = {}
            else:
                data = json.loads(public.ReadFile('/www/adcitra/panel/data/analysis.json'))
            if path not in data.keys():
                data[path] = []
            if result['data']:
                if result['data'] not in data[path]:
                    data[path].append(result['data'])
            public.WriteFile('/www/adcitra/panel/data/analysis.json', json.dumps(data))
            data[path].reverse()
...
class log_analysis:
    path = '/www/adcitra/panel/script'
    log_analysis_path = '/www/adcitra/panel/script/log_analysis.sh'
    logs_path = '/www/adcitra/panel/logs/analysis/'
...
'''
    ...
    def set_cron_task(self, get):  # 已不再使用，告警在告警模块中添加
        ...
        try:
            cron_task_path = '/www/adcitra/panel/data/cron_task_analysis.json'
    ...
    def add_crontab(self):
        ...
        if not public.M('crontab').where('name=?', (cron_name,)).count():
            cmd = 'btpython /www/adcitra/panel/script/cron_log_analysis.py'
            args = {
                "name": cron_name,
                "type": 'day',
                "where1": '',
    ...
    def remove_analysis(self, get):
        ...
        filedata = '/www/adcitra/panel/data/analysis.json'
        try:
