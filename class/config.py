...
try:
    from AdCtiraPanel import session, admin_path_checks, g, request, cache
    import send_mail
    import requests
except:
    pass
...
class config:
    ...
    def user_mail_send(self, get):
        ...
        # 测试发送
        if self.mail.qq_smtp_send(get.email.strip(), 'AdCtira灯塔告警测试邮件', 'AdCtira灯塔告警测试邮件'):
    ...
    def setPanel(self, get):
        ...
        if "webname" in get and get.webname != session['title']:
            if get.webname.strip() == '': get.webname = 'AdCtira灯塔Linux面板'
    ...
    def SetControl(self, get):
        try:
            ...
            pass
        # 开启监控时开启面板日报统计
        # if not os.path.exists('/www/adcitra/panel/data/start_daily.pl') and get.type == '1':
        #     date = time.time()
        #     # 格式化日期为字符串
        #     yesterday_date_str = public.format_date("%Y%m%d", date - 86400)
        #     public.writeFile('/www/adcitra/panel/data/start_daily.pl', yesterday_date_str)
        # elif os.path.exists('/www/adcitra/panel/data/start_daily.pl') and get.type == '0':
        #     os.remove('/www/adcitra/panel/data/start_daily.pl')
        ...
        if get.type == '1':
            ...
            public.ExecShell("/www/adcitra/panel/AdCtira-Task")
            public.ExecShell("touch /www/adcitra/panel/data/system.db")
            public.ExecShell("chmod 777 /www/adcitra/panel/data/system.db")
            public.ExecShell("chmod 777 /www/adcitra/panel/data/system.sql")
    ...
    def check_tables(self):
        import db
        db_obj = db.Sql()
        db_obj.dbfile('/www/adcitra/panel/data/system.db')
...
"""
    ...
    def SetPanelSSL(self, get):
        if hasattr(get, "email"):
            ...
            sslConf = '/www/adcitra/panel/data/ssl.pl'
            if os.path.exists('/www/adcitra/panel/ssl/baota_root.crt'):
                public.ExecShell('rm -f /www/adcitra/panel/ssl/baota_root.crt')
            if os.path.exists(sslConf) and not 'cert_type' in get:
                if os.path.exists('/www/adcitra/panel/data/ssl_verify_data.pl'):
    ...
    def CreateSSL(self):
        ...
        if userInfo:
            ...
            cert_api = 'https://api.adcitra.cn/bt_cert'
  ...
  `addtime` INTEGER
    ...
    def set_two_step_auth(self, get):
        ...
        if get.act == "1":
            ...
            if os.path.exists("/www/adcitra/panel/data/dont_vcode_ip.txt"):
                os.remove("/www/adcitra/panel/data/dont_vcode_ip.txt")
    ...
    def get_menu_list_old(self, get):
        ...
        '''
        try:
            menu_file = '/www/adcitra/panel/config/menu.json'
            menu_data = json.loads(public.ReadFile(menu_file))["menu"]
            hide_menu_file = '/www/adcitra/panel/config/hide_menu.json'
            show_menu_file = '/www/adcitra/panel/config/show_menu.json'
            default_data = ["memuA", "memuAsite", "memuAdatabase", 'memuDocker', "memuAcontrol", "memuAfirewall", "memuAfiles", "memuAlogs", "memuAxterm", "memuAcrontab", "memuAsoft", "memuAconfig", "dologin", "memu_adcitrawaf"]
            ...
            # 新增菜单保存到文件
            path = '/www/adcitra/panel/config/{}.pl'
            menu_list = ["memuAvhost"]
            for menu in menu_list:
                if not os.path.exists(path.format(menu)):
                    # if menu == "memuAmail" and not os.path.exists('/www/adcitra/panel/plugin/mail_sys'):
                    #     continue
            ...
            if os.path.exists('/www/adcitra/panel/data/site_menu.json'):
                site_menu_data = public.readFile('/www/adcitra/panel/data/site_menu.json')
                if site_menu_data:
            ...
            if uid != 1 and uid:
                if public.M('users').where('id=?', (uid,)).select():
                    plugin_path = '/www/adcitra/panel/plugin/users'
    ...
    def get_menu_list(self, get):
        default_path = '/www/adcitra/panel/config/menu.json'
        local_menu_path = '/www/adcitra/panel/config/local_menu.json'

        if not os.path.exists(local_menu_path):
            if not os.path.exists("/www/adcitra/panel/config/show_menu.json"):
                menu_data = {}
            else:
    ...
    def set_hide_menu_list(self, get):
        ...
        public.writeFile('/www/adcitra/panel/config/local_menu.json', json.dumps(menu_data))
    ...
    def set_site_hide_menu(self, get):
        ...
        if os.path.exists("/www/adcitra/panel/data/site_menu.json"):
            data = public.ReadFile("/www/adcitra/panel/data/site_menu.json")
            if data:
                try:
        ...
        public.WriteFile("/www/adcitra/panel/data/site_menu.json", json.dumps(site_hide_menu))
        try:
    ...
    def get_login_send(self, get):
        send_type = ""
        login_send_type_conf = "/www/adcitra/panel/data/panel_login_send.pl"
        if os.path.exists(login_send_type_conf):
            send_type = public.ReadFile(login_send_type_conf).strip()
        else:

            if os.path.exists("/www/adcitra/panel/data/login_send_type.pl"):
                send_type = public.readFile("/www/adcitra/panel/data/login_send_type.pl")
            else:
                if os.path.exists('/www/adcitra/panel/data/login_send_mail.pl'):
                    send_type = "mail"
                if os.path.exists('/www/adcitra/panel/data/login_send_dingding.pl'):
                    send_type = "dingding"
    ...
    def clear_login_send(self, get):
        ...
        if type == 'mail':
            if os.path.exists("/www/adcitra/panel/data/login_send_mail.pl"):
                os.remove("/www/adcitra/panel/data/login_send_mail.pl")
        elif type == 'dingding':
            if os.path.exists("/www/adcitra/panel/data/login_send_dingding.pl"):
                os.remove("/www/adcitra/panel/data/login_send_dingding.pl")

        login_send_type_conf = "/www/adcitra/panel/data/login_send_type.pl"
        if os.path.exists(login_send_type_conf):
    ...
    def set_login_send(self, get):
        ...
        # if type=='mail':
        #     if not os.path.exists("/www/adcitra/panel/data/login_send_mail.pl"):
        #         os.mknod("/www/adcitra/panel/data/login_send_mail.pl")
        #     if os.path.exists("/www/adcitra/panel/data/login_send_dingding.pl"):
        #         os.remove("/www/adcitra/panel/data/login_send_dingding.pl")
        #     return public.returnMsg(True, '设置成功')
        # elif type=='dingding':
        #     if not os.path.exists("/www/adcitra/panel/data/login_send_dingding.pl"):
        #         os.mknod("/www/adcitra/panel/data/login_send_dingding.pl")
        #     if os.path.exists("/www/adcitra/panel/data/login_send_mail.pl"):
        #         os.remove("/www/adcitra/panel/data/login_send_mail.pl")
        #     return public.returnMsg(True, '设置成功')
        # else:
    ...
    def get_login_ipwhite(self, get):
        try:
            path = '/www/adcitra/panel/data/send_login_white.json'
            ip_white = json.loads(public.ReadFile('/www/adcitra/panel/data/send_login_white.json'))
    ...
    def set_ssl_verify(self, get):
        ...
        ca_path = "/www/adcitra/panel/plugin/ssl_verify/client/ca.pem"
        if not os.path.exists(ca_path):
            return public.returnMsg(False, 'ca文件不存在,可通过企业版插件[堡塔限制访问型证书->双向认证->服务器证书]获取')
        ca_content = public.readFile(ca_path)

        crl_path = "/www/adcitra/panel/plugin/ssl_verify/client/crl.pem"
    ...
    def test_node(self, node_ip):
        '''
            ...
            @return int 节点延迟
        '''
        if not node_ip: node_ip = "api.adcitra.cn"
        s_time = time.time()
        import requests
        headers = {"Host": "api.adcitra.cn", "User-Agent": "AdCtira-Panel"}
        try:
    ...
    def sync_node_config(self):
        ...
        public.ExecShell("sed -i '/api.adcitra.cn/d' /etc/hosts")
        public.ExecShell("sed -i '/www.adcitra.cn/d' /etc/hosts")
        if not node_info['node_ip']: return
        public.ExecShell("echo '{} api.adcitra.cn' >> /etc/hosts".format(node_info['node_ip']))
        public.ExecShell("echo '{} www.adcitra.cn' >> /etc/hosts".format(node_info['node_ip']))
    ...
    def get_msg_configs(self, get):
        ...
        default = {
            ...
            "version": "1.0",
            "date": "2023-10-30",
            "help": "https://www.adcitra.cn/bbs/thread-121791-1-1.html",
            "ps": "AdCtira灯塔自定义API信息通道，用于接收面板消息推送",
            "setup": True,
    ...
    def set_default_channel(self, get):
        ...
        设置默认消息通道
        """
        default_channel_pl = "/www/adcitra/panel/data/default_msg_channel.pl"
    ...
    def update_nps(self, get):
        url = "https://nps.adcitra.cn/api/index.php"
        data = {
            'action': "list",
        ...
        if res.status_code == 200:
            data1 = res.json()['data']
            public.writeFile('/www/adcitra/panel/data/nps_{}.json'.format(get.get('product_type', 0)), json.dumps(data1))
            return data1
        return False
    ...
    def get_nps_new(self, get):
        ...
        try:
            product_type = get.get('product_type', 0)
            nps_path = '/www/adcitra/panel/data/nps_{}.json'.format(product_type)
    ...
    def get_nps_info(self, get):
        try:
            api_url = 'https://nps.adcitra.cn/api/index.php'
            data = {
                'action': 'tags',
    ...
    def write_nps(self, get):
        ...
        api_url = 'https://www.adcitra.cn/api/v2/contact/nps/submit'
        user_info = json.loads(public.ReadFile("{}/data/userInfo.json".format(public.get_panel_path())))
        if 'rate' not in get:
            return public.returnMsg(False, "参数错误")
    ...
    def _get_panel_nps_data():
        panel_path = public.get_panel_path()
        try:
            nps_file = "{}/data/adcitra_panel_nps_data".format(panel_path)
    ...
    def set_recommend_show(self, get):
        ...
        show = get.show
        if int(show):
            if os.path.exists('/www/adcitra/panel/data/recommend_show.pl'):
                os.remove('/www/adcitra/panel/data/recommend_show.pl')
        else:
            public.writeFile('/www/adcitra/panel/data/recommend_show.pl')
    ...
    def get_memo_body(self, get):
        memo_path = "/www/adcitra/panel/data/memo.txt"
    ...
    def set_ua(self, get):
        try:
            ua_file = '/www/adcitra/panel/class/limitua.json'
            ...
            public.writeFile('/www/adcitra/panel/class/limitua.json', json.dumps(data))  # 将数据保存为JSON文件
            return public.returnMsg(True, "设置成功！")
        except Exception as e:
    ...
    def balance_pay_cert(self,get):
        try:
            user_info = json.loads(public.ReadFile("{}/data/userInfo.json".format(public.get_panel_path())))
            url = "https://nps.adcitra.cn/api/v2/order/product/create_cert_with_credit"
            data = {
    ...
    def err_collection(self, get):
        ...
        error_infos = {
            ...
            "PACK_TIME": public.readFile("/www/adcitra/panel/config/update_time.pl") if os.path.exists("/www/adcitra/panel/config/update_time.pl") else public.getDate(),  # 打包时间
            "TYPE": 1,
        ...
        if not public.cache_get(pkey):
            try:
                public.run_thread(public.httpPost, ("https://api.adcitra.cn/bt_error/index.php", error_infos))
    ...
    def set_wechat_login_status(self, get):
        wechat_login_file = '/www/adcitra/panel/data/wechat_login.pl'
        if not 'status' in get: return public.returnMsg(False, '必需要有status参数!')
        status = 1 if int(get.status) else 0
        if status:
            if not os.path.exists("/www/adcitra/panel/data/userInfo.json"):
                return public.returnMsg(False, '开启微信登录前请先给面板绑定AdCtira灯塔账号!')
            ...
            if not account_wechat_bind_status['status'] or not account_wechat_bind_status['msg'].get('id', False):
                return public.returnMsg(False, '开启微信登录前请先给AdCtira灯塔账号绑定微信!')
            t_str = '开启'
    ...
    def account_wechat_bind_status(self, get):
        result = public.httpPost("https://www.adcitra.cn/api/v2/wx_auth/user_bind_status",public.get_user_info())
