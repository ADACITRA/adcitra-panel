...
class database(datatool.datatools):
    ...
    def __init__(self):
        ...
        self.panel_path = '/www/adcitra/panel'
...
'''
    ...
    def SetupPassword(self, get):
        ...
        try:
            ...
            if isError != None and not sid:
                ...
                isError = self.IsSqlError(result)
                if isError != None:
                    public.ExecShell("cd /www/adcitra/panel && " + public.get_python_bin() + " tools.py root \"" + password + "\"")
                    is_modify = False
    ...
    def SetDataDir(self, get):
        if os.path.exists("/www/adcitra/panel/config/db_dir_cp_status.pl"):
            public.ExecShell("rm -f /www/adcitra/panel/config/db_dir_cp_status.pl")
        if os.path.exists("/www/adcitra/panel/config/db_dir_cp_info.json"):
            public.ExecShell("rm -f /www/adcitra/panel/config/db_dir_cp_info.json")
        ...
        DB_DIR_CP_INFO={}
        public.ExecShell("echo True > /www/adcitra/panel/config/db_dir_cp_info.json")
        # time.sleep(10)
        ...
        public.WriteFile("/www/adcitra/panel/config/db_dir_cp_info.json",json.dumps(DB_DIR_CP_INFO))
        ...
        if len(result[0]) > 10:
            ...
            public.writeFile('/www/adcitra/panel/config/db_dir_cp_status.pl',"true")
    ...
    def GetmvDataDirSpeed(self,get):
        if not os.path.exists("/www/adcitra/panel/config/db_dir_cp_info.json"):
            return public.returnMsg(False,'获取迁移信息失败！')

        try:
            DB_DIR_CP_INFO=json.loads(public.ReadFile("/www/adcitra/panel/config/db_dir_cp_info.json"))
        except:
    ...
    def BinLog(self, get):
        ...
        if bin_log_status is True:  # 关闭 binlog 日志
            master_slave_conf_1 = "/www/adcitra/panel/plugin/masterslave/data.json"
            master_slave_conf_2 = "/www/adcitra/panel/plugin/mysql_replicate/config.json"
...
USE \`{db_name}\`;
        ...
        try:
            ...
            if public.M('crontab').where('name=?', (task_name,)).count() == 0:
                task = {
                    ...
                    "backupTo": "",
                    "save": "",
                    "sBody": "btpython /www/adcitra/panel/script/restart_database.py ",
                    "urladdress": "",
                    "status": "0",
                    "notice":0,
                }
    ...
    def SetAuditLogConfig(self,get=None):
        ...
        if not os.path.exists('/www/server/mysql/lib/plugin/audit_log.so'):
            public.ExecShell("wget -O /www/server/mysql/lib/plugin/audit_log.so http://download.adcitra.cn/src/audit_log.so")
            if not os.path.exists('/www/server/mysql/lib/plugin/audit_log.so'):
