...
run_time_file = '/www/adcitra/panel/jobs_run_time.pl'
...
def control_init():
    ...
    if not os.path.exists('/www/adcitra/panel/data/check_ssl_cron.pl'): check_ssl_cron()
    ...
    #clean_session()
    #set_crond()
    clean_max_log("/www/adcitra/panel/logs/ipfilter.log",1024*1024*10)
    clean_max_log('/www/adcitra/panel/plugin/rsync/lsyncd.log')
    ...
    clean_max_log("/www/adcitra/panel/plugin/webhook/script",1024*1024*1)
    # #check_firewall()
    check_dnsapi()
    clean_php_log()
    files_set_mode()
...
def check_ssl_cron():
    ...
    public.ExecShell("sed -i 's#/usr/bin/python#/www/adcitra/panel/pyenv/bin/python3#g' {}".format(crontab_path))
    conf = public.readFile(crontab_path)
    if '/www/adcitra/panel/pyenv/bin/python3' in conf:
        cmd = public.M('crontab').where('name=?',("续签Let's Encrypt证书",)).getField('sBody')
        cmd = cmd.replace('/usr/bin/python','/www/adcitra/panel/pyenv/bin/python3')
        public.M('crontab').where('name=?',("续签Let's Encrypt证书",)).setField('sBody',cmd)
        public.ExecShell("systemctl restart crond")
        public.writeFile('/www/adcitra/panel/data/check_ssl_cron.pl','True')
...
def check_cryptography_pyopenssl_version():
    out1, error1 = public.ExecShell('/www/adcitra/panel/pyenv/bin/python -c "import cryptography; print(cryptography.__version__.split(\'.\')[0])"') # type: str, str
    out2, error2 = public.ExecShell('/www/adcitra/panel/pyenv/bin/python -c "import OpenSSL.crypto, OpenSSL; print(OpenSSL.__version__.split(\'.\')[0])"')
    ...
    if need_reinstall:
        ...
        try:
            for i in chain(
                glob("/www/adcitra/panel/pyenv/lib/python3.*/site-packages/pyopenssl*"),
                glob("/www/adcitra/panel/pyenv/lib/python3.*/site-packages/OpenSSL"),
            ):
                if os.path.exists(i):
                    shutil.rmtree(i)
        ...
        try:
            for i in chain(
                glob("/www/adcitra/panel/pyenv/lib/python3.*/site-packages/cryptography"),
                glob("/www/adcitra/panel/pyenv/lib/python3.*/site-packages/cryptography*"),
            ):
                if os.path.exists(i):
                    shutil.rmtree(i)
...
def check_brotli():
    ...
    '''
    # 未开启面板SSL则不安装
    if not os.path.exists('/www/adcitra/panel/data/ssl.pl'): return
    try:
        import brotli
    except ImportError:
...
def rep_pyenv_link():
    '''
        ...
        @return void
    '''
    pyenv_bin = '/www/adcitra/panel/pyenv/bin/python3'
    btpython_bin = '/usr/bin/btpython'
    pip_bin = '/www/adcitra/panel/pyenv/bin/pip3'
...
def clear_other_conf():
    path = '/www/adcitra/panel/vhost/nginx'
...
def clear_other_files():
    ...
    dirPath = '/www/adcitra/panel/adminer'
    if os.path.exists(dirPath):
        public.ExecShell("rm -rf {}".format(dirPath))
    ...
    try:
        ...
        src_file = '/www/adcitra/panel/init.sh'
    ...
    # public.ExecShell("chmod -R  600 /www/adcitra/panel/data;chmod -R  600 /www/adcitra/panel/config;chmod -R  700 /www/server/cron;chmod -R  600 /www/server/cron/*.log;chown -R root:root /www/adcitra/panel/data;chown -R root:root /www/adcitra/panel/config;chown -R root:root /www/server/phpmyadmin;chmod -R 755 /www/server/phpmyadmin")
    if os.path.exists("/www/server/mysql"):
        public.ExecShell("chown mysql:mysql /etc/my.cnf;chmod 600 /etc/my.cnf")
    public.ExecShell("rm -rf /www/adcitra/panel/temp/*")
    ...
    if os.path.exists("/www/adcitra/panel/adminer"):
        public.ExecShell("rm -rf /www/adcitra/panel/adminer")
    ...
    node_service_src = '/www/adcitra/panel/script/nodejs-service.py'
...
def null_html():
    null_files = ['/www/server/nginx/html/index.html','/www/server/apache/htdocs/index.html','/www/adcitra/panel/data/404.html']
    null_new_body='''<html>
...
def write_run_script_log(_log,rn='\n'):
    _log_file = '/www/adcitra/panel/logs/run_script.log'
...
def run_script():
    try:
        ...
        run_config ='/www/adcitra/panel/data/run_config'
        script_logs = '/www/adcitra/panel/logs/script_logs'
        ...
        for sname in os.listdir(run_config):
            ...
            if not os.path.exists(script_info['script_file']) \
                or script_info['script_file'].find('/www/adcitra/panel/plugin/') != 0 \
...
def files_set_mode():
    ...
    m_paths = [
        ...
        ["/www/server/adcitrawaf","/*.lua","root",755,False],
        ...
        ["/www/adcitra/panel/class","","root",600,True],
        ["/www/adcitra/panel/data","","root",600,True],
        ["/www/adcitra/panel/plugin","","root",600,False],
        ["/www/adcitra/panel/AdCtiraPanel","","root",755,True],
        ["/www/adcitra/panel/vhost","","root",600,True],
        ["/www/adcitra/panel/rewrite","","root",600,True],
        ["/www/adcitra/panel/config","","root",600,True],
        ["/www/adcitra/panel/backup","","root",600,True],
        ["/www/adcitra/panel/package","","root",600,True],
        ["/www/adcitra/panel/script","","root",700,True],
        ["/www/adcitra/panel/temp","","root",600,True],
        ["/www/adcitra/panel/tmp","","root",600,True],
        ["/www/adcitra/panel/ssl","","root",600,True],
        ["/www/adcitra/panel/install","","root",600,True],
        ["/www/adcitra/panel/logs","","root",600,True],
        ["/www/adcitra/panel/AdCtira-Panel","","root",700,False],
        ["/www/adcitra/panel/AdCtira-Task","","root",700,False],
        ["/www/adcitra/panel","/*.py","root",600,False],
        ["/www/adcitra/panel","","root",755,False],
        ["/www/adcitra/panel/AdCtiraPanel/__init__.py","","root",600,False],
        ["/www/adcitra/panel/AdCtiraPanel/templates","","root",600,True],
        ...
        ["/www/adcitra/panel/init.sh","","root",600,False],
        ["/www/adcitra/panel/license.txt","","root",600,False],
        ["/www/adcitra/panel/requirements.txt","","root",600,False],
        ["/www/adcitra/panel/update.sh","","root",600,False],
        ["/www/adcitra/panel/default.pl","","root",600,False],
        ["/www/adcitra/panel/hooks","","root",600,True],
        ["/www/adcitra/panel/cache","","root",600,True],
        ["/root","","root",550,False],
    ...
    public.ExecShell("nohup chmod -R go-w /www/adcitra/panel &")
    # 标记文件设置时间
...
#尝试升级到独立环境
def update_py37():
    pyenv='/www/adcitra/panel/pyenv/bin/python3'
    pyenv_exists='/www/adcitra/panel/data/pyenv_exists.pl'
...
def test_ping():
    _f = '/www/adcitra/panel/data/ping_token.pl'
...
def run_new():
    try:
        new_file = '/www/adcitra/panel/data/new.pl'
        port_file = '/www/adcitra/panel/data/port.pl'
        ...
        import psutil
        p_in = any("AdCtira-Panel" in p.name() for p in psutil.process_iter(attrs=['name']))
        ...
        if cmd_line.find('AdCtira-Panel') != -1: return False
        public.writeFile('/www/adcitra/panel/data/restart.pl','True')
...
#清理webhook日志
def clean_hook_log():
    path = '/www/adcitra/panel/plugin/webhook/script'
...
def disable_putenv(fun_name):
    try:
        is_set_disable = '/www/adcitra/panel/data/disable_%s' % fun_name
...
def set_crond():
    try:
        ...
        if not cron_id:
            ...
            shell = public.get_python_bin() + ' /www/adcitra/panel/class/panelLets.py renew_lets_ssl'
    ...
    for tmp in temp.split('\n'):
        ...
        try:
            ...
            arr = {
                ...
                'type': disk[1].strip(),
                'path': disk[6].replace('/usr/local/lighthouse/softwares/adcitra_panel', '/www')
            }
...
def ping_test():
    if os.system("ping -c 1 -w 1 100.100.100.200") == 0:
        ...
        server_store = public.readFile("/www/adcitra/panel/data/cloudthirdlogin.pl")
        if not server_store:
            server_store = "others"
    public.writeFile("/www/adcitra/panel/data/server_store.pl", server_store)


def sync_default_msg_channel():
    if "/www/adcitra/panel" not in sys.path:
        sys.path.append("/www/adcitra/panel")

    try:
        from mod.base.msg import update_mod_push_msg
        os.remove("/www/adcitra/panel/data/mod_push_data/update_sender.pl")
        update_mod_push_msg()
...
def check_docker_version():
    ver, error = public.ExecShell('/www/adcitra/panel/pyenv/bin/python -c "import docker; print(docker.__version__.split(\'.\')[0])"')
    try:
