...
# +-------------------------------------------------------------------
# | Author: 沐落 <cjx@adcitra.cn>
# +-------------------------------------------------------------------
import os,sys,json,time,re
setup_path = '/www/adcitra/panel'
os.chdir(setup_path)
...
class panelLets:
    ...
    setupPath = None #安装路径
    server_type = None
    log_file = '/www/adcitra/panel/logs/letsencrypt.log'
    ...
    def crate_let_by_oper(self,data):
        ...
        try:
            ...
            if not 'renew' in data:
                ...
                AdCtiraPanel.dns_client = sewer.Client(domain_name = data['first_domain'],dns_class = None,account_key = data['account_key'],domain_alt_names = data['domains'],contact_email = str(data['email']) ,ACME_AUTH_STATUS_WAIT_PERIOD = 15,ACME_AUTH_STATUS_MAX_CHECKS = 5,ACME_REQUEST_TIMEOUT = 20,ACME_DIRECTORY_URL = self.let_url)
                domain_dns_value = "placeholder"
                dns_names_to_delete = []
                self.write_log("|-正在注册帐户...")
                AdCtiraPanel.dns_client.acme_register()
                authorizations, finalize_url = AdCtiraPanel.dns_client.apply_for_cert_issuance()
                responders = []
                self.write_log("|-正在获取验证信息...")
                for url in authorizations:
                    identifier_auth = AdCtiraPanel.dns_client.get_identifier_authorization(url)
                    ...
                    acme_keyauthorization, domain_dns_value = AdCtiraPanel.dns_client.get_keyauthorization(dns_token)
                ...
                for i in responders:
                    ...
                    auth_status_response = AdCtiraPanel.dns_client.check_authorization_status(i["authorization_url"])
                    if auth_status_response.json()["status"] == "pending":
                        AdCtiraPanel.dns_client.respond_to_challenge(i["acme_keyauthorization"], i["dns_challenge_url"])

                for i in responders:
                    self.write_log("|-获取CA验证结果[{}]...".format(i['dns_name']))
                    AdCtiraPanel.dns_client.check_authorization_status(i["authorization_url"], ["valid","invalid"])
                self.write_log("|-所有域名验证通过，正在发送CSR...")
                certificate_url = AdCtiraPanel.dns_client.send_csr(finalize_url)
                self.write_log("|-正在获取证书内容...")
                certificate = AdCtiraPanel.dns_client.download_certificate(certificate_url)

                if certificate:
                    certificate = self.split_ca_data(certificate)
                    result['cert'] = certificate['cert']
                    result['ca_data'] = certificate['ca_data']
                    result['key'] = AdCtiraPanel.dns_client.certificate_key
                    result['account_key'] = AdCtiraPanel.dns_client.account_key
                    result['status'] = True
                    AdCtiraPanel.dns_client = None
                else:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if type == 'renew_lets_ssl':
            ...
            os.system(public.get_python_bin() + " /www/adcitra/panel/class/acme_v2.py --renew=1")
