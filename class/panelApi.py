...
class panelApi:
    ...
    def login_for_app(self,get):
        ...
        try:
            if not os.path.exists('/www/adcitra/panel/data/app_login_check.pl'):return public.returnMsg(False,'无效的登录密钥3')
            key, init_time, tid2, status = public.readFile('/www/adcitra/panel/data/app_login_check.pl').split(':')
            ...
            public.writeFile("/www/adcitra/panel/data/app_login_check.pl", data)
            return public.returnMsg(True,'扫码成功,正在登录!')
        except:
            os.remove("/www/adcitra/panel/data/app_login_check.pl")
