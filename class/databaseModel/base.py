...
class databaseBase:
    ...
    def IsSqlError(self, mysqlMsg):
        if mysqlMsg:
            ...
            if "DB-Lib error message 20018" in mysqlMsg: return public.returnMsg(False, '创建失败，SQL Server需要GUI支持，请通过远程桌面连接连接上SQL Server管理工具，依次找到安全性 -> 登录名 -> NT AUTHORITY\SYSTEM -> 属性 -> 服务器角色 -> 勾选列表中的sysadmin后重启SQL Server服务，重新操作添加数据库。<a style="color:red" href="https://www.adcitra.cn/bbs/thread-50555-1-1.html">帮助</a>')
