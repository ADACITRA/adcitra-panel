...
# | AdCtira
# +-------------------------------------------------------------------
# | Copyright (c) 2015-2016 AdCtira(www.adcitra.cn) All rights reserved.
# +-------------------------------------------------------------------
# | Author: zhwen <zhwen@adcitra.com>
# +-------------------------------------------------------------------
import os
import time
import json
...
class one_key_wp(wpbase):
    ...
    # wp_package_url = 'http://download.adcitra.cn/install/package/wordpress-5.9.1.zip'
    base_path = "/www/adcitra/panel/"
    ...
    panel_db = "/www/adcitra/panel/data/default.db"
    timeout_count = 0
    old_time = 0
    __wp_session = None
    __session_resp = None
    ...
    def set_urlrewrite(self, site_name, site_path):
        ...
        swfile = '/www/adcitra/panel/rewrite/{}/wordpress.conf'.format(webserver)
    ...
    def check_param(self, args):
        ...
        if hasattr(args, 'domain'):
            if re.search(rep_domain, args.domain):
                ...
                return public.return_message(-1, 0, public.lang("Please check if the [{}] format is correct For example: {}", "domain", "adcitra.com"))
        ...
        if hasattr(args, 'admin_email'):
            if re.search(rep_email, args.admin_email):
                ...
                return public.return_message(-1, 0, public.lang("Please check if the [{}] format is correct For example: {}", "admin_email", "adimn@adcitra.com"))
...
""" % version
    ...
    def get_fast_cgi_status(self, sitename, php_v):
        if public.get_webserver() != "nginx":
            return False
        conf_path = "/www/adcitra/panel/vhost/nginx/{}.conf".format(sitename)
