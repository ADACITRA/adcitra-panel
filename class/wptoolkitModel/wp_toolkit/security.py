...
class wp_security:
    ...
    def get_security_info(self, get):
        ...
        #取防火墙配置
        result= public.run_plugin('adcitrawaf', 'get_site_config_byname', public.to_dict_obj({'siteName':get.site_name}))
        try:
            if result['open']:
                return_result['firewall_status']=1
        except:
            pass
        result= public.run_plugin('adcitrawaf', 'get_site_config3', public.to_dict_obj({'siteName':get.site_name}))
        try:
   ...
   #关闭文件保护
    ...
    def open_firewall_protection(self, get):
        ...
        result= public.run_plugin('adcitrawaf', 'get_total_all', public.to_dict_obj({
        }))
        if not result['open']:
            result= public.run_plugin('adcitrawaf', 'set_open', public.to_dict_obj({
            }))
            if not result['status']:
                return public.return_message(-1, 0, public.lang("开启失败"))
        
        result= public.run_plugin('adcitrawaf', 'get_site_config_byname', public.to_dict_obj({
            'siteName': get.site_name,
        }))
        if not result['open']:
            result= public.run_plugin('adcitrawaf', 'set_site_obj_open', public.to_dict_obj({
                    'siteName': get.site_name,
                    'obj': get.obj,
                }))
            if not result['status']:
