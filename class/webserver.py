...
class webserver:
    ...
    def __init__(self):
        ...
        if not hasattr(public,'get_panel_path'):
            self.__panel_path = '/www/adcitra/panel'
        else:
...
'''
    ...
    def chmod_static_files(self):
        ...
        '''
        # 设置静态目录链权限
        res = self.exec_shell('chmod 755 /www /www/server /www/adcitra/panel /www/adcitra/panel/AdCtiraPanel')
        if res[1] != '':
            self.print_log('\n'.join(res),self.__log_warning)
            return False
        
        # 递归设置静态文件权限
        res = self.exec_shell('chmod  -R 755 /www/adcitra/panel/AdCtiraPanel/static')
        if res[1] != '':
            self.print_log('\n'.join(res),self.__log_warning)
            return False
