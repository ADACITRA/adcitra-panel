...
# | AdCtira灯塔Linux面板
# +-------------------------------------------------------------------
# | Copyright (c) 2015-2016  AdCtira灯塔软件(http://adcitra.cn) All rights reserved.
# +-------------------------------------------------------------------
...
class ajax:
    ...
    def _get_jdk_status(self):
        ...
        skey = 'is_down_jdk'
        tmp_file = '/www/adcitra/panel/data/jdk.json'
...
CREATE TABLE IF NOT EXISTS `network` (
  ...
  `addtime` INTEGER
    ...
    def get_docker_model_info(self):
        ...
        """
        try:
            if not os.path.exists("/www/adcitra/panel/data/docker.db"):
                return ""
            # 获取docker模块创建的容器数量
            dk_c_num = str(
    ...
    def UpdatePanel(self, get):
        try:
            ...
            if (updateInfo['force'] == True or hasattr(get, 'toUpdate') == True
                ...
                if os.path.exists('/www/adcitra/panel/runserver.py'):
                    public.ExecShell('rm -f /www/adcitra/panel/*.pyc')
                if os.path.exists('/www/adcitra/panel/class/common.py'):
                    public.ExecShell('rm -f /www/adcitra/panel/class/*.pyc')
    ...
    def get_phpmyadmin_conf(self):
        if public.get_webserver() == "nginx":
            conf_file = "/www/adcitra/panel/vhost/nginx/phpmyadmin.conf"
            rep = r"listen\s*(\d+)"
        else:
            conf_file = "/www/adcitra/panel/vhost/apache/phpmyadmin.conf"
            rep = r"Listen\s*(\d+)"
    ...
    def get_phpmyadmin_ssl(self, get):
        ...
        if conf:
            port = re.search(rep, conf).group(1)
            path = "/www/adcitra/panel/vhost/{}/phpmyadmin.conf".format(public.get_webserver())
    ...
    def change_phpmyadmin_ssl_port(self, get):
        ...
        for i in ["nginx", "apache"]:
            file = "/www/adcitra/panel/vhost/{}/phpmyadmin.conf".format(i)
            ...
            if i == "nginx":
                if not os.path.exists(
                    "/www/adcitra/panel/vhost/apache/phpmyadmin.conf"):
                    return public.returnMsg(
                        False,
    ...
    def set_phpmyadmin_ssl(self, get):
        if public.get_webserver() == "openlitespeed":
            return public.returnMsg(False, 'OpenLiteSpeed 目前尚不支持该操作')
        if not os.path.exists("/www/adcitra/panel/ssl/certificate.pem"):
...
"""
            ...
            ssl_conf = """server
    {
        ...
        ssl_certificate    /www/adcitra/panel/ssl/certificate.pem;
        ssl_certificate_key    /www/adcitra/panel/ssl/privateKey.pem;
        ...
        access_log  /www/wwwlogs/access.log;
            ...
            public.writeFile("/www/adcitra/panel/vhost/nginx/phpmyadmin.conf",
                             ssl_conf)
            import panelPlugin
...
<VirtualHost *:887>
    ...
    #SSL
    SSLEngine On
    SSLCertificateFile /www/adcitra/panel/ssl/certificate.pem
    SSLCertificateKeyFile /www/adcitra/panel/ssl/privateKey.pem
...
{}
       ...
       DirectoryIndex index.php index.html index.htm default.php default.html default.htm
            ...
            public.writeFile("/www/adcitra/panel/vhost/apache/phpmyadmin.conf",
                             ssl_conf)
        else:
            if os.path.exists("/www/adcitra/panel/vhost/nginx/phpmyadmin.conf"):
                os.remove("/www/adcitra/panel/vhost/nginx/phpmyadmin.conf")
            if os.path.exists(
                "/www/adcitra/panel/vhost/apache/phpmyadmin.conf"):
                os.remove("/www/adcitra/panel/vhost/apache/phpmyadmin.conf")
            public.serviceReload()
    ...
    def check_user_auth(self, get):
        ...
        try:
            ...
            return public.returnMsg(False, 'AdCtira灯塔帐户绑定已失效，请在[设置]页面重新绑定!')
        pdata = {
    ...
    def php_info(self, args):
        ...
        tmp = public.ExecShell(
            php_bin +
            ' -c {} /www/adcitra/panel/class/php_info.php'.format(php_ini))[0]
    ...
    def get_panel_error_info(self, get):
        ...
        return public.returnMsg(
            False,
            '未识别的错误信息，请前往<a href="https://www.adcitra.cn/bbs" class="btlink">AdCtira灯塔论坛</a> 发帖提问.'
        )
    
    def Clean_bt_host(self, get):
        '''
        清理本机adcitra.cn的hosts
        ...
        default = [
    {
        ...
        "name": "ltd",
        "price": "999.99",
        "preview": "https://www.adcitra.cn/new/product/linux_ltd.html",
        "ps": [
			"5分钟极速响应",
			"15天无理由退款",
            "30+款付费插件",
        ...
        "list": [
            {
                ...
                "dependent": "nginx",
                "pluginType": "pro",
                "preview": "https://www.adcitra.cn/new/product_nginx_firewall.html",
                "ps": "web防火墙，有效抵御CC攻击、SQL注入、XSS跨站攻击、建站程序漏洞、一句话木马等常见渗透攻击"
            },
            {
                "pay": "40",
                "title": "网站防火墙",
                "name": "adcitrawaf_httpd",
                "install": False,
                "isBuy": False,
                "pid": 100000012,
                "pluginType": "pro",
            ...
            },
            {
                ...
                "pluginType": "pro",
                "pid": 100000014,
                "preview": "https://www.adcitra.cn/new/product_website_total.html",
        ...
        "list": [
            {
                ...
                "previewImg": "https://www.adcitra.cn/Public/new/plugin/introduce/site/tamper_proof_preview.png",
                "menu_id": 15,
				"menu_name":"防篡改",
                "isBuy": False,
                "pid": 100000015,
            ...
            },
            {
                ...
                "previewImg": "https://www.adcitra.cn/Public/new/plugin/introduce/site/ssl_verify_preview.png",
                "menu_id": 3,
				"menu_name":"访问限制",
                "isBuy": False,
                "pid": 100000062,
        ...
        "list": [
            {
                ...
                "previewImg": "https://www.adcitra.cn/Public/new/plugin/rsync/1.png",
                "menu_id": 15,
                "isBuy": False,
                "pid": 100000005,
                "preview": ""
