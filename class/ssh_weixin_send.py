...
def get_ip():
    if os.path.exists('/www/adcitra/panel/data/iplist.txt'):
        data=ReadFile('/www/adcitra/panel/data/iplist.txt')
        if data:
            return data.strip()
        else:
            return 'None'
...
#检查配置文件是否存在
def is_config():
    if not os.path.exists('/www/adcitra/panel/data/dingding.json'):
        return False
    else:return True

def pam_sm_authenticate(pamh, flags, argv):
    if not os.path.exists('/www/adcitra/panel/data/dingding.json'):return pamh.PAM_SUCCESS
    try:
        config=ReadFile('/www/adcitra/panel/data/dingding.json')
