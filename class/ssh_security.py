...
class ssh_security:
    ...
    __ip_data = None
    __ClIENT_IP = '/www/adcitra/panel/data/host_login_ip.json'
    ...
    def return_python(self):
        if os.path.exists('/www/adcitra/panel/pyenv/bin/python'): return '/www/adcitra/panel/pyenv/bin/python'
    ...
    def send_mail_data(self, title, body, login_ip, type=None):
        ...
        try:
            login_send_type_conf = "/www/adcitra/panel/data/ssh_send_type.pl"
    ...
    def get_server_ip(self):
        if os.path.exists('/www/adcitra/panel/data/iplist.txt'):
            data = public.ReadFile('/www/adcitra/panel/data/iplist.txt')
            return data.strip()
        else:
            return '127.0.0.1'
    ...
    # 登陆的情况下
    def login(self):
        path_time = "/www/adcitra/panel/data/ssh_login_times.pl"
        import time
    ...
    def repair_bashrc(self):
        ...
        if re.search(self.return_python() + ' /www/adcitra/panel/class/ssh_security.py', data):
            public.WriteFile(self.return_bashrc(), data.replace(self.return_python() + ' /www/adcitra/panel/class/ssh_security.py login', ''))
            # 遗留的错误信息
    ...
    def start_jian(self, get):
        ...
        if not re.search(self.return_python() + ' /www/adcitra/panel/class/ssh_security.py', data):
            cmd = '''shell="%s /www/adcitra/panel/class/ssh_security.py login"
...
disown $!''' % (self.return_python())
    ...
    def stop_jian(self, get):
        data = public.ReadFile(self.return_profile())
        if re.search(self.return_python() + ' /www/adcitra/panel/class/ssh_security.py', data):
            cmd = '''shell="%s /www/adcitra/panel/class/ssh_security.py login"''' % (self.return_python())
            ...
            # 检查是否还存在遗留
            if re.search(self.return_python() + ' /www/adcitra/panel/class/ssh_security.py', data):
                public.WriteFile(self.return_profile(), data.replace(self.return_python() + ' /www/adcitra/panel/class/ssh_security.py login', ''))
            # 遗留的错误信息
    ...
    def get_jian(self, get):
        data = public.ReadFile(self.return_profile())
        if re.search('/www/adcitra/panel/class/ssh_security.py login', data):
    ...
    def set_anti_conf(self, get):
        ...
        _set_up_path = "/www/adcitra/panel/plugin/fail2ban"
        ...
        config = [
            {
                ...
                "version": "1.0",
                "date": "2022-08-19",
                "help": "https://www.adcitra.cn",
                "ps": "AdCtira灯塔微信公众号通知，用于接收面板消息推送"
            },
            {
                "name": "mail",
                "title": "邮箱",
                "version": "1.1",
                "date": "2022-08-10",
                "help": "https://www.adcitra.cn/bbs/thread-66183-1-1.html",
                "ps": "AdCtira灯塔邮箱消息通道，用于接收面板消息推送"
            },
            {
                "name": "dingding",
                "title": "钉钉",
                "version": "1.2",
                "date": "2022-08-10",
                "help": "https://www.adcitra.cn/bbs/thread-44497-1-1.html",
                "ps": "AdCtira灯塔钉钉消息通道，用于接收面板消息推送"
            },

            {
                ...
                "version": "1.2",
                "date": "2022-08-10",
                "help": "https://www.adcitra.cn/bbs/thread-52540-1-1.html",
                "ps": "AdCtira灯塔企业微信消息通道，用于接收面板消息推送"
            },
            {
                "name": "feishu",
                "title": "飞书",
                "version": "1.2",
                "date": "2022-08-10",
                "help": "https://www.adcitra.cn/bbs/",
                "ps": "AdCtira灯塔飞书消息通道，用于接收面板消息推送"
            },
            {
                "name": "sms",
                "title": "短信通知",
    ...
    def check_dingding(self, get):
        ...
        '''
        # 检查文件是否存在
        if not os.path.exists('/www/adcitra/panel/data/dingding.json'): return False
        dingding_config = public.ReadFile('/www/adcitra/panel/data/dingding.json')
    ...
    def get_record_video(self, get):
        old_path = "/www/adcitra/panel/plugin/jumpserver/static/video/"
        new_path = "/www/adcitra/panel/data/jumpserver_video/"
