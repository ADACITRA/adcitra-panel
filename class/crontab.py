...
class crontab:
    ...
    def set_task_top(self, get=None):
        ...
        :return:
        """
        cron_task_top_path = '/www/adcitra/panel/data/cron_task_top'
    ...
    def check_crontab_service(self):
        ...
        try:
            ...
            public.ExecShell('nohup btpython  /www/adcitra/panel/script/crontab_check.py > /dev/null 2>&1 &')
    ...
    def repair_crontab_service(self, get):
        ...
        exec_result = public.ExecShell("nohup stdbuf -oL btpython /www/adcitra/panel/script/crontab_repair.py > /tmp/repair_crontab.txt 2>&1 &")
    ...
    def modify_crond(self, get):
        try:
            ...
            if "拔测" in get['name'] and "/www/adcitra/panel/class/monitorModel/boceModel.py" in get['sBody']:
    ...
    [Service]
    Type=simple
    ExecStart=btpython /www/adcitra/panel/script/execute_commands.py
    Restart=on-failure
    ...
    case "$1" in
        start)
            ...
            nohup btpython /www/adcitra/panel/script/execute_commands.py > /dev/null 2>&1 &
            pid=$!
    ...
    def GetDataList(self, get):
        ...
        for lib in libs:
            ...
            if not os.path.exists(filename):
                ...
                plugin_path = '/www/adcitra/panel/plugin/{}/aes_status'.format(lib['opt'])
    ...
    def GetShell(self, param):
        ...
        if type == 'toFile':
            ...
            if second:
                ...
                head += ' btpython /www/adcitra/panel/script/second_task.py {} {} \n'.format(second,cronName)
                head += ' exit 0\n'
                head += 'fi\n'
            public.ExecShell("chmod +x /www/adcitra/panel/script/modify_second_cron.sh")
            public.ExecShell("nohup /www/adcitra/panel/script/modify_second_cron.sh {} {} {} &".format(time_type,second,cronName) )
            ...
            if time_type:
                ...
                # 调用 Python 脚本进行时间检查
                head += 'if [[ $1 != "start" ]]; then\n'
                head += ' if ! btpython /www/adcitra/panel/script/time_check.py time_type={} special_time={} time_list={}; then\n'.format(time_type, ",".join(special_time.split(",")), ",".join(time_list.split(",")))
                head += '   exit 1\n'
                head += ' fi\n'
                head += 'fi\n'
            if param['sType']=="site_restart":
                ...
                head +='fi\n'
                head += 'if [[ $1 != "start" ]]; then\n'
                head += ' if ! btpython /www/adcitra/panel/script/special_time.py special_time={} ; then\n'.format(",".join(special_time.split(",")))
                head += '   exit 1\n'
                head += ' fi\n'
                head += 'fi\n'
            log = '-access_log'
...
echo "----------------------------------------------------------------------------"
if [[ "$1" != "start" ]]; then
    btpython /www/adcitra/panel/script/log_task_analyzer.py {logname}
fi
rm -f {cronFile}
'''
        if type == 'toShell' and param.get('notice') and param['notice_channel'] and param['notice_channel'] and len(param.get('keyword', '')):
            shell += "btpython /www/adcitra/panel/script/shell_push.py {} {} {} {} &".format(cronName, param['notice_channel'], param['keyword'], param['name'])
    ...
    def get_task_name_and_body(self,model_name, project_name):
        ...
        sBody = 'btpython /www/adcitra/panel/script/restart_project.py {} {}'.format(model_name, project_name)
    ...
    def modify_values(self, cronName, new_time_type, new_special_time, new_time_list):
        ...
        for i, line in enumerate(lines):
            if "btpython /www/adcitra/panel/script/time_check.py" in line:
                lines[i] = 'if ! btpython /www/adcitra/panel/script/time_check.py time_type={} special_time={} time_list={}; then\n'.format(new_time_type, new_special_time, new_time_list)
    ...
    def set_execute_script(self, get):
        ...
        public.ExecShell("chmod +x /www/adcitra/panel/script/check_crontab.sh")
        # 使用nohup运行脚本，并将输出重定向到/www/test.txt，同时确保命令在后台运行
        exec_result=public.ExecShell("nohup /www/adcitra/panel/script/check_crontab.sh > /tmp/check_crontab.txt 2>&1 &")
    ...
    def change_cronta(self, log_conf):
        ...
        cronInfo['sBody'] = '{pyenv} {script_path}'.format(
            pyenv=python_path,
            script_path="/www/adcitra/panel/script/rotate_log.py",
        )
        ...
        if not cronInfo:
            ...
            if public.M('crontab').where('name=?', (task_name,)).count() == 0:
                task = {
                    ...
                    "backupTo": "",
                    "save": num,
                    "sBody": "btpython /www/adcitra/panel/script/rotate_log.py",
                    "urladdress": "",
                    "status": "1"
                }
