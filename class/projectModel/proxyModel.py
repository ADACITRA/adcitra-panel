...
'''
    ...
    def GetSiteLogs(self, get):
        ...
        re_log_file = None
        if serverType == "nginx":
            config_path = '/www/adcitra/panel/vhost/nginx/{}.conf'.format(get.siteName)
            config = public.readFile(config_path)
            re_log_file = self.nginx_get_log_file(config, is_error_log=False)
        elif serverType == 'apache':
            config_path = '/www/adcitra/panel/vhost/apache/{}.conf'.format(get.siteName)
            config = public.readFile(config_path)
            if not config:
