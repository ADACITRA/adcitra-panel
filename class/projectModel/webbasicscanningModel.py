...
class main:
    __count = 0
    __shell = "/www/adcitra/panel/data/webbasic_shell_check.txt"
    ...
    def WebConfigSecurity(self, webinfo, get):
        ...
        # 防火墙检测
        if not os.path.exists("/www/server/adcitrawaf/"):
            result.append({
    ...
    def WebSSLSecurity(self, webinfo, get, result):
        ...
        '''
        if public.get_webserver() == 'nginx':
            conf_file = '/www/adcitra/panel/vhost/nginx/{}.conf'.format(webinfo['name'])
    ...
    def WebWeakPasswordDetection(self, webinfo, get):
        ...
        # 读取弱口令字典
        weekpassfile = "/www/adcitra/panel/config/weak_pass.txt"
        pass_list = []
    ...
    def SaveScanResult(self, result, scan_type='single'):
        ...
        '''
        try:
            result_dir = '/www/adcitra/panel/data/webbasic_scan_results'
    ...
    def save_statistics_result(self, details=None):
        ...
        try:
            # 准备保存目录
            save_path = '/www/adcitra/panel/data/safeCloud'
    ...
    def get_scan_result(self, get):
        ...
        '''
        try:
            save_path = '/www/adcitra/panel/data/safeCloud/webbasic_scan_result.json'
            if not os.path.exists(save_path):
                return None
    ...
    def GetSiteRunPath(self, siteName, sitePath):
        ...
        path = sitePath
        if public.get_webserver() == 'nginx':
            filename = '/www/adcitra/panel/vhost/nginx/' + siteName + '.conf'
            ...
            filename = '/www/adcitra/panel/vhost/apache/' + siteName + '.conf'
            if os.path.exists(filename):
                conf = public.readFile(filename)
