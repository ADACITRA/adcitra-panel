...
"""
    ...
    def install_tomcat_new(self, get):
        ...
        if download_url is None:
            error = (
                '<br>错误：连接AdCtira灯塔官网异常，请按照以下方法排除问题后重试：<br>解决方法：'
                '<a target="_blank" class="btlink" href="https://www.adcitra.cn/bbs/thread-87257-1-1.html">'
                'https://www.adcitra.cn/bbs/thread-87257-1-1.html</a><br>'
            )
            raise public.PanelError(error)
...
'''.format(jsvc_make_path, jdk_path, tomcat_path + "/bin/jsvc")
    ...
    def _get_jdk_status(self):
        ...
        tmp_pl = '/www/adcitra/panel/data/jdk.json.pl'
    ...
    def install_jdk_new(self, get):
        try:
            ...
            if not os.path.exists('/www/adcitra/panel/install/jdk.sh'):
                public.ExecShell('wget -O /www/adcitra/panel/install/jdk.sh ' + public.get_url() + '/install/0/jdk.sh')
            ...
            res = public.ExecShell('bash /www/adcitra/panel/install/jdk.sh install ' + get['version'])
