...
class projectBase(LimitNet, Redirect):
    ...
    def set_daemon_time(self, get):
        ...
        public.writeFile("/www/adcitra/panel/data/daemon_time.pl", str(daemon_time))
        return public.returnMsg(True, "设置成功")

    def get_daemon_time(self, get):
        """获取守护进程重启检测时间"""
        res = public.readFile("/www/adcitra/panel/data/daemon_time.pl")
        if res is False:
            return {
                "status": True,
                "daemon_time": 120
    ...
    def _nginx_set_domain(project_id, project_name, prefix="") -> bool:
        file = '/www/adcitra/panel/vhost/nginx/{}{}.conf'.format(prefix, project_name)
