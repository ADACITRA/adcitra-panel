...
class main(safeBase):
    ...
    def _monitor_enabled(self):
        """检测是否启用监控报表数据源。存在 /www/adcitra/panel/plugin/monitor 且配置中的 data_save_path 可用时返回 True"""
        try:
            if not os.path.exists('/www/adcitra/panel/plugin/monitor'):
                return False
    ...
    def _get_monitor_db_path(self):
        ...
        try:
            ...
            conf_file = '/www/adcitra/panel/plugin/monitor/monitor/config/config.json'
            conf_data = None
            try:
