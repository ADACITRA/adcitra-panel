...
# AdCtira灯塔Linux面板
# -------------------------------------------------------------------
# Copyright (c) 2014-2099 AdCtira灯塔软件(http://adcitra.cn) All rights reserved.
# -------------------------------------------------------------------
# Author: wpl <wpl@adcitra.cn>
# -------------------------------------------------------------------
...
class YaraDetector(WebshellDetector):
    ...
    def __init__(self):
        ...
        self.base_path = '/www/adcitra/panel/data/safeCloud/rules'
...
class CloudDetector(WebshellDetector):
    ...
    def __init__(self):
        self.cache_file = '/www/adcitra/panel/data/safeCloud/cloud_config.json'
    ...
    def _update_check_url(self) -> bool:
        ...
        try:
            ret = requests.get('http://www.adcitra.cn/checkWebShell.php').json()
...
class SafeCloudModel:
    ...
    def __init__(self):
        self.upload_config = {
            'url': 'http://w-check.adcitra.cn/upload_web.php',
            'max_file_size': 2 * 1024 * 1024,  # 最大文件大小限制(1MB)
            'max_daily_uploads': 50,  # 每日最大上报数量
            'min_upload_interval': 300,  # 最小上报间隔(秒)
            'cache_file': '/www/adcitra/panel/data/safeCloud/upload_stats.json'
        }
...
class main(projectBase):
    def __init__(self):
        self.__path = '/www/adcitra/panel/data/safeCloud'
        # 确保配置目录存在
    ...
    def webshell_detection(self, get: Dict) -> Dict:
        ...
        try:
            ...
            # task任务频率控制：12小时执行一次
            safecloud_dir = '/www/adcitra/panel/data/safeCloud'
    ...
    def start_service(self, get):
        ...
        public.WriteFile('/www/adcitra/panel/class/projectModel/bt_cloud_safe', shell_info)
        time.sleep(0.3)
    ...
    def get_safecloud_list(self, get) -> dict:
        # 获取云安全检测 查杀数量
        # 恶意文件检测、漏洞扫描、首页风险
        # 首页风险：/www/adcitra/panel/data/warning/resultresult.json  读取risk里的status为false的
        # 漏洞扫描：/www/adcitra/panel/data/scanning.json，读取loophole_num
        # 恶意文件检测：/www/adcitra/panel/data/safeCloud/log/detection_all.log 只统计高危
        # 总数  累加
        """获取云安全检测统计数据
        @return: dict {
            'total': 总风险数,
        ...
        try:
            ...
            security_file = '/www/adcitra/panel/data/warning/resultresult.json'
            try:
            ...
            # 2. 统计漏洞扫描数量
            scanning_file = '/www/adcitra/panel/data/scanning.json'
            try:
            ...
            # 3. 统计恶意文件检测数量（高危）
            detection_log = '/www/adcitra/panel/data/safeCloud/log/detection_all.log'
            try:
            ...
            # 4. 统计基线检测数量
            safe_detect_file = '/www/adcitra/panel/data/safe_detect.json'
            try:
                ...
                pass
            # 5. 统计入侵检测数量
            hids_installed = os.path.exists('/www/adcitra/panel/plugin/bt_hids/adcitra_panelhids.sh')
            if hids_installed:
                try:
                    # 统计高危和中危数量
    ...
    def get_security_logs(self, get) -> dict:
        ...
        try:
            ...
            # 1. 读取首页风险
            try:
                risk_file = '/www/adcitra/panel/data/warning/resultresult.json'
            ...
            # 2. 读取漏洞扫描
            try:
                vuln_file = '/www/adcitra/panel/data/scanning.json'
    ...
    def get_virus_db_time(self):
        """获取病毒库更新时间"""
        try:
            rules_dir = '/www/adcitra/panel/data/safeCloud/rules'
    ...
    def get_protect_days(self):
        """获取保护天数"""
        try:
            protect_file = '/www/adcitra/panel/data/safeCloud/install_time.pl'
    ...
    def get_security_status(self) -> dict:
        ...
        try:
            ...
            try:
                # 检查安装状态
                status['tamper_installed'] = os.path.exists('/www/adcitra/panel/plugin/tamper_core/install.sh')
            ...
            # 3. 检查恶意文件检测状态
            try:
                config_file = '/www/adcitra/panel/data/safeCloud/config.json'
            ...
            try:
                # 检查安装状态
                status['hids_installed'] = os.path.exists('/www/adcitra/panel/plugin/bt_hids/adcitra_panelhids.sh')
                ...
                # 检查状态文件
                status_file = '/www/adcitra/panel/data/hids_data/status.pl'
    ...
    def _calculate_time_decay(self, risk_details: dict) -> dict:
        ...
        try:
            ...
            # 2. 检查漏洞扫描时间
            try:
                vul_file = '/www/adcitra/panel/data/scanning.json'
    ...
    def _check_no_new_risks(self, days: int) -> bool:
        ...
        try:
            ...
            # 2. 检查恶意文件
            malware_log = '/www/adcitra/panel/data/safeCloud/log/detection_all.log'
    ...
    def _check_baseline_compliance(self) -> bool:
        ...
        try:
            ...
            # 2. 检查服务器安全检测
            safe_file = '/www/adcitra/panel/data/safe_detect.json'
    ...
    def get_safe_overview(self, get) -> dict:
        ...
        try:
            ...
            # 11. 判断入侵检测和防篡改是否安装
            # 判断路径/www/adcitra/panel/plugin/bt_hids是否存在
            result['hids'] = os.path.exists('/www/adcitra/panel/plugin/bt_hids')
            # 判断路径/www/adcitra/panel/plugin/bt_tamper是否存在
            result['tamper'] = os.path.exists('/www/adcitra/panel/plugin/tamper_core')
            return result
    ...
    def get_pending_alarm_trend(self, get) -> dict:
        """获取待处理告警趋势"""
        try:
            trend_file = '/www/adcitra/panel/data/safeCloud/alarm_trend.json'
    ...
    def get_security_trend(self, get) -> dict:
        ...
        """
        try:
            trend_file = '/www/adcitra/panel/data/safeCloud/security_trend.json'
    ...
    def get_security_dynamic(self, get) -> dict:
        ...
        try:
            ...
            # 1. 获取恶意文件检测事件
            try:
                log_file = '/www/adcitra/panel/data/safeCloud/log/detection_all.log'
            ...
            #  3. 获取网站漏洞扫描事件
            try:
                sf = '/www/adcitra/panel/data/scanning.json'
            ...
            # 4. 首页风险检测
            try:
                rf = '/www/adcitra/panel/data/warning/resultresult.json'
        ...
        try:
            ...
            if hasattr(get, 'filename') and hasattr(get, 'risk'):
                if not current_file_exists_for_upload:
                    ...
                    import requests
                    upload_url = 'https://www.adcitra.cn/api/v2/error/information'
