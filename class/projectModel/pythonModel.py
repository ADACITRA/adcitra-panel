...
try:
    from AdCtiraPanel import cache, python_env_ssh
...
def _init_gvm() -> None:
    ...
    try:
        if not os.path.exists(pyvm_path):
            real_path = '{}/class/projectModel/adcitra_pyvm.py'.format(panel_path)
            os.chmod(real_path, mode=0o755)
            os.symlink(real_path, pyvm_path)

        # if not os.path.exists(bt_py_project_env_path):
        #     real_path = '{}/script/adcitra_pyprojectenv.sh'.format(panel_path)
        #     os.chmod(real_path, mode=0o755)
        #     os.symlink(real_path, bt_py_project_env_path)

        if not os.path.exists(bt_py_project_env_path) or os.path.samefile(bt_py_project_env_path, '{}/script/adcitra_pyprojectenv.sh'.format(panel_path)):
            real_path = '{}/script/adcitra_pyprojectenv.sh'.format(panel_path)
            run_path = "/www/server/python_project/adcitra_pyprojectenv.sh"
...
class main(projectBase):
    ...
    #     if not res:
    #         return {"status": False,
    #                 "msg": "请求不到官方python版本数据，请切换节点试一试.<br>相关教程参考:<br><a>https://www.adcitra.cn/bbs/thread-87257-1-1.html</a>"}
    #     text = res.split('\n')
    ...
    def prep_status(self, project_conf: dict):
        try:
            ...
            if isinstance(pid, str):
                ps = psutil.Process(int(pid))
                if ps.is_running() and os.path.samefile(ps.exe(), "/www/adcitra/panel/pyenv/bin/python3") and \
...
echo $! > {pid_file}'''.format(
    ...
    def get_active_shell(self, p_name, pyenv) -> str:
        ...
        if pyenv.env_type == "conda":
            ...
            return "unset _BT_PROJECT_ENV && source /www/server/python_project/adcitra_pyprojectenv.sh {} {}".format(p_name, evn_sh)
    ...
    def change_cronta(self, name, log_conf):
        ...
        @name 更改站点运行日志切割任务
        """
        python_path = "/www/adcitra/panel/pyenv/bin/python3"
    ...
    def async_install_py_version(self, get: public.dict_obj) -> Dict:
        if not os.path.exists("{}/class/projectModel/adcitra_pyvm.py".format(public.get_panel_path())):
        ...
        if version is None:
            return public.returnMsg(False, "版本参数信息错误")
        sh_str = "{}/pyenv/bin/python3 {}/class/projectModel/adcitra_pyvm.py install {} --extend='{}'".format(
