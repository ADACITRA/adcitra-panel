...
class ssh_terminal:
    ...
    def __init__(self):
        ...
        import db
        sql = db.Sql().dbfile('/www/adcitra/panel/data/db/default.db')
    ...
    def connect(self):
        ...
        try:
            ...
            return returnMsg(False, 'paramiko模块不存在且安装失败，请联系AdCtira灯塔官方人员!')
        ...
        try:
            ...
            if self._old_conf:
                s_file = '/www/adcitra/panel/config/t_info.json'
        ...
        if self._host in ('127.0.0.1', 'localhost'):
            time.sleep(0.1)
            payload_cmd = (
                b"source /www/adcitra/panel/script/safe_adcitra_shell.sh paramiko > /dev/null 2>&1\n"
        ...
        self._video_addr = "/www/adcitra/panel/data/jumpserver_video/%s.json" % str(int(self._connect_time))
        if not os.path.exists("/www/adcitra/panel/data/jumpserver_video/"):
            os.makedirs("/www/adcitra/panel/data/jumpserver_video/")
        # 如果开启了录像功能
...
class local_ssh_terminal(ssh_terminal):
    ...
    class _LocalShell:
        ...
        def do_ssh_login_push():
            try:
                # 防抖 10s
                ssh_login_times_file = "/www/adcitra/panel/data/ssh_login_times.pl"
                last_time = {"time": 0}
                try:
                ...
                client_ip_file = "/www/adcitra/panel/data/host_login_ip.json"
                try:
...
bt() {
  ...
  if [ $# -eq 0 ]; then
    if [[ "$tool_en" == true ]]; then
      echo "" | btpython /www/adcitra/panel/tools_en.py cli; printf '\033[1A\033[2K\033[1A\033[2K'
    ...
    9)
      if [[ "$tool_en" == true ]]; then
        echo "Clearing panel cache cannot be executed in the panel terminal. You can delete [/www/adcitra/panel/data/session] and restart the panel"
      else
        echo "清除面板缓存无法在面板终端执行，您可以删除【/www/adcitra/panel/data/session】并重启面板"
      fi
      ;;
    12)
      if [[ "$tool_en" == true ]]; then
    ...
    * )
      if [[ "$tool_en" == true ]]; then
        btpython /www/adcitra/panel/tools_en.py cli $@
      else
        /etc/init.d/bt $@
      fi
    esac
...
''' % (not_debug_str)
        ...
        def __init__(self):
            ...
            self.first_recv = b"The current terminal create by AdCtira-Panel.\r\n"
    ...
    def connect(self):
        ...
        self._connect_time = time.time()
        self._last_send = []
        from AdCtiraPanel import request, session
        self._client = public.GetClientIp() + ':' + str(public.get_remote_port())
        public.WriteLog(self._log_type, '成功登录到SSH服务器 [{}:{}]'.format(self._host, self._port))
        self.history_send("登录成功\n")
        self.debug('通道已构建')
...
class ssh_host_admin(ssh_terminal):
    ...
    def completion_tool_status(args):
        ...
        zh_json_file = "{}/AdCtiraPanel/static/zh.json.gz".format(public.get_panel_path())
    ...
    def set_completion_tool_status(args):
        ...
        if not os.path.exists(zh_json_file):
            public.downloadFile("https://download.adcitra.cn/src/zh.json.gz", zh_json_file)
        if not os.path.exists(zh_json_file):
