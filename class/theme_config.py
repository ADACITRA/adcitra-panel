...
class ThemeConfigManager:
    ...
    def __init__(self, config_file_path='/www/adcitra/panel/data/panel_asset.json', auto_init=True):
        """初始化主题配置管理器"""
    ...
    def _get_full_path(self, relative_path, base_path=None):
        ...
        if base_path is None:
            # 优先使用生产环境路径
            production_base = '/www/adcitra/panel/AdCtiraPanel'
            # 如果生产环境路径不存在，使用当前项目路径
            if not os.path.exists(production_base):
                current_dir = os.path.dirname(os.path.abspath(__file__))
                project_root = os.path.dirname(current_dir)
                base_path = os.path.join(project_root, 'AdCtiraPanel')
            else:
    ...
    def export_theme_config(self, export_path=None):
        ...
        try:
            ...
            try:
                ...
                # 获取实际使用的基础路径
                actual_base_path = self._get_full_path('static').replace('/static', '') if self._get_full_path('static') else '/www/adcitra/panel/AdCtiraPanel'
    ...
    def import_theme_config(self, import_file_path, backup_existing=True):
        ...
        try:
            ...
            try:
                ...
                failed_files = []
                # 使用动态基础路径检测
                base_path = self._get_full_path('static').replace('/static', '') if self._get_full_path('static') else '/www/adcitra/panel/AdCtiraPanel'
