...
# AdCtira灯塔Linux面板
# -------------------------------------------------------------------
# Copyright (c) 2015-2017 AdCtira灯塔软件(http:#adcitra.cn) All rights reserved.
# -------------------------------------------------------------------
# Author: hwliang <hwl@adcitra.cn>
# -------------------------------------------------------------------
...
try:
    from AdCtiraPanel import session
    from AdCtiraPanel import cache
except:
    pass
        ...
        nginx_default = '''server
{
    ...
    ServerName %s.%s
    ServerAlias %s
    IncludeOptional /www/adcitra/panel/vhost/apache/extension/%s/*.conf
    ...
    <Directory "%s">
        ...
        ext_path = "/www/adcitra/panel/vhost/apache/extension/%s/" % self.siteName
    ...
    def set_404_config(self, get):
        try:
            ...
            # 写回配置文件
            with open('/www/adcitra/panel/class/404_settings.json', 'w') as f:
                json.dump(config, f)
    ...
    def get_404_config(self,get):
        try:
            with open('/www/adcitra/panel/class/404_settings.json', 'r') as f:
    ...
    def nginxAdd(self):
        ...
        if not os.path.exists('/www/adcitra/panel/class/404_settings.json'):
            public.writeFile('/www/adcitra/panel/class/404_settings.json',json.dumps({}))
...
{{
    ...
    include /www/adcitra/panel/vhost/nginx/well-known/{site_name}.conf;
    #CERT-APPLY-CHECK--END
    include /www/adcitra/panel/vhost/nginx/extension/{site_name}/*.conf;
    ...
    error_log  {log_path}/{site_name}.error.log;
        ...
        # 写配置文件
        if not os.path.exists("/www/adcitra/panel/vhost/nginx/well-known"):
            os.makedirs("/www/adcitra/panel/vhost/nginx/well-known", 0o600)
        public.writeFile("/www/adcitra/panel/vhost/nginx/well-known/{}.conf".format(self.siteName), "")
        if not os.path.exists("/www/adcitra/panel/vhost/nginx/extension/{}/".format(self.siteName)):
            os.makedirs("/www/adcitra/panel/vhost/nginx/extension/{}/".format(self.siteName), 0o600)
...
vhRoot BT_RUN_PATH
configFile /www/adcitra/panel/vhost/openlitespeed/detail/BT_SITENAME.conf
allowSymbolLink 1
enableScript 1
restrained 1
setUIDMode 0
...
rewrite  {
  enable                  1
  autoLoadHtaccess        1
  include /www/adcitra/panel/vhost/openlitespeed/proxy/BTSITENAME/urlrewrite/*.conf
  include /www/adcitra/panel/vhost/apache/redirect/BTSITENAME/*.conf
  include /www/adcitra/panel/vhost/openlitespeed/redirect/BTSITENAME/*.conf
}
include /www/adcitra/panel/vhost/openlitespeed/proxy/BTSITENAME/*.conf
'''
    ...
    def add_task(self, get):
        try:
            self.config_path = '/www/adcitra/panel/data/syncsite.json'
...
set $bt_safe_open "{}/:/tmp/";'''.format(self.sitePath)
    ...
    def DeleteSite(self, get, multiple=None):
        ...
        # 删除openlitespeed配置
        vhost_file = "/www/adcitra/panel/vhost/openlitespeed/{}.conf".format(siteName)
        if os.path.exists(vhost_file):
            public.ExecShell('rm -f {}*'.format(vhost_file))
        vhost_detail_file = "/www/adcitra/panel/vhost/openlitespeed/detail/{}.conf".format(siteName)
        if os.path.exists(vhost_detail_file):
            public.ExecShell('rm -f {}*'.format(vhost_detail_file))
        vhost_ssl_file = "/www/adcitra/panel/vhost/openlitespeed/detail/ssl/{}.conf".format(siteName)
        if os.path.exists(vhost_ssl_file):
            public.ExecShell('rm -f {}*'.format(vhost_ssl_file))
        vhost_sub_file = "/www/adcitra/panel/vhost/openlitespeed/detail/{}_sub.conf".format(siteName)
        if os.path.exists(vhost_sub_file):
            public.ExecShell('rm -f {}*'.format(vhost_sub_file))

        vhost_redirect_file = "/www/adcitra/panel/vhost/openlitespeed/redirect/{}".format(siteName)
        if os.path.exists(vhost_redirect_file):
            public.ExecShell('rm -rf {}*'.format(vhost_redirect_file))
        vhost_proxy_file = "/www/adcitra/panel/vhost/openlitespeed/proxy/{}".format(siteName)
        ...
        filename = '/www/adcitra/panel/vhost/rewrite/' + siteName + '.conf'
        if os.path.exists(filename):
            os.remove(filename)
    ...
    def _del_ols_listen_conf(self, sitename):
        conf_dir = '/www/adcitra/panel/vhost/openlitespeed/listen/'
        if not os.path.exists(conf_dir):
            return False
    ...
    def _check_ols_ssl(self, webname):
        conf = public.readFile('/www/adcitra/panel/vhost/openlitespeed/listen/443.conf')
    ...
    def openlitespeed_domain(self, get):
        listen_dir = '/www/adcitra/panel/vhost/openlitespeed/listen/'
...
""" % (get.port, get.port, get.webname, get.domain)
    ...
    def SetSSL(self, get):
        ...
        path = '/www/adcitra/panel/vhost/cert/' + siteName
    ...
    def CreateLet(self, get):
        ...
        if result['status'] and not 'code' in result:
            get.onkey = 1
            path = '/www/adcitra/panel/cert/' + get.siteName
            if os.path.exists(path + '/certOrderId'): os.remove(path + '/certOrderId')
            result = self.SetSSLConf(get)
        return result
    ...
    def set_ols_ssl(self, get, siteName):
        ...
        ssl_conf = """
        vhssl {
          keyFile                 /www/adcitra/panel/vhost/cert/BTDOMAIN/privkey.pem
          certFile                /www/adcitra/panel/vhost/cert/BTDOMAIN/fullchain.pem
          certChain               1
          sslProtocol             24
...
listener SSL443 {
  ...
  address                 *:443
  secure                  1
  keyFile                 /www/adcitra/panel/vhost/cert/BTSITENAME/privkey.pem
  certFile                /www/adcitra/panel/vhost/cert/BTSITENAME/fullchain.pem
  certChain               1
  sslProtocol             24
  ciphers                 EECDH+AESGCM:EDH+AESGCM:AES256+EECDH:AES256+EDH:ECDHE-RSA-AES128-GCM-SHA384:ECDHE-RSA-AES128-GCM-SHA256:ECDHE-RSA-AES128-GCM-SHA128:DHE-RSA-AES128-GCM-SHA384:DHE-RSA-AES128-GCM-SHA256:DHE-RSA-AES128-GCM-SHA128:ECDHE-RSA-AES128-SHA384:ECDHE-RSA-AES128-SHA128:ECDHE-RSA-AES128-SHA:ECDHE-RSA-AES128-SHA:DHE-RSA-AES128-SHA128:DHE-RSA-AES128-SHA128:DHE-RSA-AES128-SHA:DHE-RSA-AES128-SHA:ECDHE-RSA-DES-CBC3-SHA:EDH-RSA-DES-CBC3-SHA:AES128-GCM-SHA384:AES128-GCM-SHA128:AES128-SHA128:AES128-SHA128:AES128-SHA:AES128-SHA:DES-CBC3-SHA:HIGH:!aNULL:!eNULL:!EXPORT:!DES:!MD5:!PSK:!RC4
  enableECDHE             1
...
"""
    ...
    ssl_certificate    /www/adcitra/panel/vhost/cert/%s/fullchain.pem;
    ssl_certificate_key    /www/adcitra/panel/vhost/cert/%s/privkey.pem;
    ssl_protocols %s;
...
""" % (get.first_domain, get.first_domain, self.get_tls_protocol(self.get_tls13(), is_apache=False), http3_header)
    ...
    #SSL
    SSLEngine On
    SSLCertificateFile /www/adcitra/panel/vhost/cert/%s/fullchain.pem
    SSLCertificateKeyFile /www/adcitra/panel/vhost/cert/%s/privkey.pem
    ...
    <Directory "%s">
        ...
        result['csr'] = public.readFile('/www/adcitra/panel/vhost/cert/' + get.siteName + '/fullchain.pem')
        result['key'] = public.readFile('/www/adcitra/panel/vhost/cert/' + get.siteName + '/privkey.pem')
        if 'isBatch' not in get:
            return result
        else:
            return True
    ...
    def save_cert(self, get):
        ...
        get.keyPath = '/www/adcitra/panel/vhost/cert/' + get.siteName + '/privkey.pem'
        get.certPath = '/www/adcitra/panel/vhost/cert/' + get.siteName + '/fullchain.pem'
...
#HTTP_TO_HTTPS_END'''
    ...
    def CloseSSLConf(self, get):
        ...
        partnerOrderId = '/www/adcitra/panel/vhost/cert/' + siteName + '/partnerOrderId'
    ...
    def _del_ols_443_domain(self, sitename):
        file = "/www/adcitra/panel/vhost/openlitespeed/listen/443.conf"
        conf = public.readFile(file)
        if conf:
    ...
    def GetSSL(self, get):
        ...
        if public.get_webserver() == "openlitespeed":
            ...
            if s_tmp and s_tmp_pkey:
                ...
                path = os.path.join('/www/adcitra/panel/vhost/cert/', siteName)
                csrpath = path + "/fullchain.pem"  # 生成证书路径
                keypath = path + "/privkey.pem"  # 密钥文件路径
        ...
        try:
            if not os.path.exists("{}/data/ignore_upgrade_cert.pl".format(public.get_panel_path())) and cert_data:
                if type == 3:
                    ...
                    act_data = public.HttpPost('https://www.adcitra.cn/newapi/activity/panelapi/activity_info', data=user_info,
                                               timeout=5)
    ...
    def create_activity_order(self, get):
        ...
        try:
            ...
            act_data = public.HttpPost('https://www.adcitra.cn/newapi/activity/panelapi/order/create', data=user_info,
                                       timeout=5)
            act_data = json.loads(act_data)
            if user_info["detail_id"] == 37698:
                act_data["data"]["data"] = {
                    "name": "AdCtira灯塔域名型SSL证书(C5) (DV)",
                    "year": 1,
                    "type": "单域名",
                    "notice":[
                        "29.9超低价购买价值128.66元的AdCtira灯塔域名型SSL证书(C5) (DV) 1年",
                        "附赠价值200元的人工部署服务",
    ...
    def activity_payment_status(self, get):
        ...
        try:
            ...
            act_data = public.HttpPost('https://www.adcitra.cn/newapi/activity/panelapi/order/payment_status', data=user_info,
                                       timeout=5)
            act_data = json.loads(act_data)
            return act_data
        except:
...
#BINDING-%s-END''' % (domain, port, webdir, domain, public.GetConfigValue('logs_path') + '/' + siteInfo['name'], public.GetConfigValue('logs_path') + '/' + siteInfo['name'], phpConfig, webdir, apaOpt,
    ...
    def GetSitePHPVersion(self, get):
        try:
            ...
            if data['phpversion'] == 'other':
                other_file = '/www/adcitra/panel/vhost/other_php/{}/enable-php-other.conf'.format(siteName)
    ...
    def SetPHPVersion(self, get, multiple=None):
        ...
        try:
            ...
            conf = public.readFile(file)
            if conf:
                other_path = '/www/adcitra/panel/vhost/other_php/{}'.format(siteName)
...
{{
    ...
    def GetDirUserINI(self, get):
        ...
        data = {}
        if os.path.exists("/www/adcitra/panel/data/syncsite.json"):
            data['sync_git'] = True if get.id in json.loads(public.readFile("/www/adcitra/panel/data/syncsite.json")) else False
        else:
            data['sync_git'] = False
    ...
    def _set_ols_open_basedir(self, get):
        ...
        try:
            ...
            f = "/www/adcitra/panel/vhost/openlitespeed/detail/{}.conf".format(sitename)
            c = public.readFile(f)
...
""" % (get.proxydir, get.proxyname, get.todomain)
    ...
    def CreateProxy(self, get):
        ...
        if not get.proxysite.split('//')[-1]:
            return public.returnMsg(False, '目标URL不能为[http://或https://],请填写完整URL，如：https://www.adcitra.cn')
        proxyUrl = self.__read_config(self.__proxyfile)
        proxyUrl.append({
            "proxyname": get.proxyname,
            "sitename": get.sitename,
...
#PROXY-END%s''' % (get.proxydir, get.proxydir, get.proxysite, get.proxydir,
  ...
  "zblog"
        ...
        if ws == "openlitespeed":
            ws = "apache"
        file_path = os.path.join('/www/adcitra/panel/rewrite/', ws, name + '.conf')
    ...
    def CloseHasPwd(self, get):
        ...
        if get.siteName == 'phpmyadmin':
            get.configFile = self.setupPath + '/nginx/conf/nginx.conf'
            if os.path.exists('/www/adcitra/panel/vhost/nginx/phpmyadmin.conf'):
                conf = public.readFile('/www/adcitra/panel/vhost/nginx/phpmyadmin.conf')
                rep = "\n\s*#AUTH_START(.|\n){1,200}#AUTH_END"
                conf = re.sub(rep, '', conf)
                public.writeFile('/www/adcitra/panel/vhost/nginx/phpmyadmin.conf', conf)
        else:
            get.configFile = self.setupPath + '/panel/vhost/nginx/' + get.siteName + '.conf'
  ...
  RewriteRule (.*) http://%s/$1 [L]
    ...
    def GetSecurity(self, get):
        file = '/www/adcitra/panel/vhost/nginx/' + get.name + '.conf'
        conf = public.readFile(file)
        data = {}
    ...
    include enable-php-''' % (
        ...
        # apache
        file = '/www/adcitra/panel/vhost/apache/' + get.name + '.conf'
        if os.path.exists(file):
            conf = public.readFile(file)
            if conf.find('SECURITY-START') != -1:
                rep = "#SECURITY-START(\n|.){1,500}#SECURITY-END\n"
            ...
            if get.status == "false":
                ...
                public.writeFile(file, conf)
        # OLS
        cond_dir = '/www/adcitra/panel/vhost/openlitespeed/prevent_hotlink/'
...
"""
    ...
    def GetSiteLogs(self, get):
        ...
        re_log_file = None
        if serverType == "nginx":
            config_path = '/www/adcitra/panel/vhost/nginx/{}.conf'.format(res + get.siteName)
            config = public.readFile(config_path)
            re_log_file = self.nginx_get_log_file(config, is_error_log=False)
        elif serverType == 'apache':
            config_path = '/www/adcitra/panel/vhost/apache/{}.conf'.format(res + get.siteName)
            config = public.readFile(config_path)
            if not config:
...
{{
    ...
    # DEFAULT SSL CONFIG
    ssl_certificate    /www/adcitra/panel/vhost/cert/0.default/fullchain.pem;
    ssl_certificate_key    /www/adcitra/panel/vhost/cert/0.default/privkey.pem;
...
<VirtualHost *:443>
    ...
    # DEFAULT SSL CONFIG
    SSLEngine On
    SSLCertificateFile /www/adcitra/panel/vhost/cert/0.default/fullchain.pem
    SSLCertificateKeyFile /www/adcitra/panel/vhost/cert/0.default/privkey.pem
    ...
    def create_default_cert(self):
        '''
            ...
            @return bool
        '''
        cert_pem = '/www/adcitra/panel/vhost/cert/0.default/fullchain.pem'
        cert_key = '/www/adcitra/panel/vhost/cert/0.default/privkey.pem'
    ...
    def check_total_install_info(get):
        ...
        try:
            total_version = json.loads(public.ReadFile('/www/adcitra/panel/plugin/total/info.json'))['versions']
        except:
            total_version = '0.0.0'
...
""".format(real_ip_from, header_cdn, "" if recursive not in ['true', True] else "\nreal_ip_recursive on;"))
    ...
    def startScan(self, get):
        ...
        self.__config_file = '/www/adcitra/panel/config/vulnerability_scanning.json'
        result22 = []
        ...
        try:
            ...
            if public.M('crontab').where('name=?', (task_name,)).count() == 0:
                task = {
                    ...
                    "backupTo": "",
                    "save": "",
                    "sBody": "btpython /www/adcitra/panel/script/restart_apache.py ",
                    "urladdress": "",
                    "status": "0",
                    "notice":0,
                }
    ...
    def set_free_total_status(self, get):
        ...
        if set_type == "site":
            ...
            if res:
                return public.returnMsg(False,
                                        "设置失败，可能无法更新收集服务导致的，请检查网络状态是否可以正常访问到adcitra.cn")
            return public.returnMsg(True, "设置成功")
        else:
            stc = SiteTotalConfig()
