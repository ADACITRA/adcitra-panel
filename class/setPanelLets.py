...
class setPanelLets:
    __vhost_cert_path = "/www/adcitra/panel/vhost/cert/"
    __panel_cert_path = "/www/adcitra/panel/ssl/"
    __tmp_key = ""
    __tmp_cert = ""
    def __init__(self):
        pass
    ...
    def __check_cert_dir(self,get):
        ...
        get.vpath = "/www/adcitra/panel/vhost/ssl_saved/"
    ...
    def __read_site_cert(self,domain_cert):
        try:
            ...
            key_file = "/www/adcitra/panel/{}/privkey.pem".format(domain_cert['save_path'])
            cert_file  = "/www/adcitra/panel/{}/fullchain.pem".format(domain_cert['save_path'])
        if not os.path.exists(key_file):
            key_file = "{path}{domain}/{key}".format(path="/www/adcitra/panel/vhost/ssl/",domain=domain_cert["subject"],key="privkey.pem")
            cert_file = "{path}{domain}/{cert}".format(path="/www/adcitra/panel/vhost/ssl/", domain=domain_cert["subject"],
                                           cert="fullchain.pem")
    ...
    def set_panel_update_ssl_task(self):
        shell = "/www/adcitra/panel/pyenv/bin/python3 -u /www/adcitra/panel/script/panel_ssl_task.py"
