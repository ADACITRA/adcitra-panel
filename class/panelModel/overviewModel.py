...
class main(panelBase):
    ...
    def GetTemplateOverview(self, get):
        ...
        try:
            ...
            if ad_config.get('homeOverview'):
                hide_modules = ['safety_risk', 'monitor', 'adcitrawaf', 'tamper_core', 'ssh_log']
        ...
        for template in overview_template:
            ...
            for i in range(len(template_option)-1, -1, -1):
                ...
                if option["name"] == "total":
                    if not os.path.exists("/www/adcitra/panel/plugin/total"):
    ...
    def GetOverview(self, get):
        ...
        for overview in overview_setting:
            ...
            if overview["name"] == "total":
                ...
                if not os.path.exists("/www/adcitra/panel/plugin/total"): continue
            nlist.append(overview)
    ...
    # waf
    def _adcitrawaf(self, name: str, params_list: list) -> list:
        value_list = [0, 0]

        waf_db_path = "/www/server/adcitrawaf/totla_db/totla_db.db"
    ...
    def _ssh_log(self, name: str, params_list: list) -> list:
        ...
        #
        # # 获取数据
        # filepath = "/www/adcitra/panel/config/ssh_intrusion.json"
        # is_today = False
