...
class main(panelBase):
    ...
    def get_update_logs(self,get):
        try:
            ...
            res = public.httpPost('https://www.adcitra.cn/Api/getUpdateLogs?type=Linux',{})
    ...
    def check_exp_ltd(self):
        ...
        sUrl = 'https://api.adcitra.cn/auth/GetUserGiveAway'
        import panelSSL
    ...
    def get_public_config(self, args):
        ...
        if os.path.exists('/www/adcitra/panel/config/api.json'):
            try:
                res = json.loads(public.readFile('/www/adcitra/panel/config/api.json'))
                data['api'] = 'checked' if res['open'] else ''
            except:
                public.ExecShell('rm -f /www/adcitra/panel/config/api.json')
                data['api'] = ''
        else:
            data['api'] = ''
        data['total'] = os.path.exists('/www/adcitra/panel/plugin/total') or os.path.exists('/www/adcitra/panel/plugin/monitor')
        data['disk_usage'] = public.get_disk_usage(public.get_panel_path())
        data['uid'] = ''
        if os.path.exists('/www/adcitra/panel/data/userInfo.json'):
            res = public.readFile('/www/adcitra/panel/data/userInfo.json')
            if res:
                try:
                    res = json.loads(res)
                    data['uid'] = res['uid']
        ...
        # 判断是否隐藏广告和企业版标识
        if os.path.exists('/www/adcitra/panel/data/hide_ad.pl'):
            data['hide_ad'] = True
        else:
            data['hide_ad'] = False
        data['o'] = public.get_oem_name()
        ...
        data['wechat_login_status'] = os.path.exists('/www/adcitra/panel/data/wechat_login.pl')
        return data
    ...
    def get_soft_status(self, get):
        ...
        data = {
            ...
            "admin": os.path.exists('/www/adcitra/panel/plugin/' + name),
    ...
    def get_panel_notices(self, get):
        ...
        # 其他公告直接展示
        try:
            notice_risk_ignore = json.loads(public.readFile("/www/adcitra/panel/data/notice_risk_ignore.json"))
        except:
            notice_risk_ignore = {}
