...
class main:
    ...
    def check_plugins(self, get):
        ...
        :param plugin_path: 插件目录的路径，例如 "/www/adcitra/panel/plugin"
    ...
    def backup_download(self, get):
        ...
        if get.storage_type=="webdav" or get.storage_type=="local" or get.storage_type=="ftp":
            if get.storage_type=="webdav":
                import sys
                if '/www/adcitra/panel/plugin/webdav' not in sys.path:
                    sys.path.insert(0, '/www/adcitra/panel/plugin/webdav')
                try:
                ...
                import sys
                if '/www/adcitra/panel/plugin/ftp' not in sys.path:
                    sys.path.insert(0, '/www/adcitra/panel/plugin/ftp')
                try:
    ...
    def backup_php(self, conf):
        try:
            ...
            words_path = "/www/adcitra/panel/config/thesaurus.json"
            wrods_data = None
            words_data_md5 = None
    ...
    def backup_php_data(self, conf):
        try:
            ...
            if not site_info:
                ...
                __redirectfile = "/www/adcitra/panel/data/redirect.conf"
                try:
                ...
                redirect_nginx_data_path = '/www/adcitra/panel/vhost/nginx/redirect/{}'.format(conf["name"])
                redirect_nginx_data = self.copy_file(redirect_nginx_data_path)
                redirect_apache_data_path = '/www/adcitra/panel/vhost/apache/redirect/{}'.format(conf["name"])
                ...
                for i in proxyUrl:
                    if i["sitename"] == conf["name"]:
                        proxy_list.append(i)
                proxy_nginx_data_path = '/www/adcitra/panel/vhost/nginx/proxy/{}'.format(conf["name"])
                proxy_nginx_data = self.copy_file(proxy_nginx_data_path)
                proxy_apache_data_path = '/www/adcitra/panel/vhost/apache/proxy/{}'.format(conf["name"])
                ...
                import config
                # 备份ssl配置
                ssl_path = "/www/adcitra/panel/vhost/cert/{}".format(site_info["name"])
                if os.path.exists(ssl_path):
                    ssl_info = self.copy_file(ssl_path)

                # 备份web配置文件
                nginx_config_path = "/www/adcitra/panel/vhost/nginx/{}.conf".format(site_info["name"])
                apache_config_path = "/www/adcitra/panel/vhost/apache/{}.conf".format(site_info["name"])
                ...
                # 备份目录限制
                dir_auth_json_path = "/www/adcitra/panel/data/site_dir_auth.json"
                if os.path.exists(dir_auth_json_path):
                    dir_auth_json = self.copy_file(dir_auth_json_path)
                dir_auth_file_path = "/www/adcitra/panel/vhost/nginx/dir_auth/{}".format(site_info["name"])
                ...
                # 备份伪静态数据
                pseudo_static_data_path = "/www/adcitra/panel/vhost/rewrite/{}.conf".format(site_info["name"])
    ...
    def backup_proxy_data(self, conf):
        ...
        nginx_config_path = "/www/adcitra/panel/vhost/nginx/{}.conf".format(conf["name"])
        if os.path.exists(nginx_config_path):
            nginx_config = self.copy_file(nginx_config_path)
        apache_config_path = "/www/adcitra/panel/vhost/apache/{}.conf".format(conf["name"])
        if os.path.exists(apache_config_path):
            apache_config = self.copy_file(apache_config_path)
    ...
    def backup_terminal_data(self, conf):
        ...
        self.print_log(conf)
        terminal_data = None
        terminal_data_path = '/www/adcitra/panel/config/ssh_info'
    ...
    def reduction_php(self, conf):
        ...
        if not os.path.exists(php_path):
            ...
            # 安装指定的php版本
            public.ExecShell('cd /www/adcitra/panel/install && /bin/bash install_soft.sh 1 install php {}.{} &>> {}'.format(conf["name"][0], conf["name"][1], self.logs_file))
        ...
        for development in development_list:
            ...
            if development.lower() in now_development_list.keys() and now_development_list[development.lower()]["status"] != True and conf["name"] in now_development_list[development.lower()][
                ...
                public.ExecShell(
                    'wget -O {sh_name}.sh https://download.adcitra.cn/install/1/{sh_name}.sh && sh {sh_name}.sh install {php_version} $>> {log}'.format(sh_name=development.lower(),
        ...
        for development in install_err:
            ...
            self.print_log("开始安装php{}拓展:{}".format(conf["name"], development))
            public.ExecShell(
                'wget -O {sh_name}.sh https://download.adcitra.cn/install/1/{sh_name}.sh && sh {sh_name}.sh install {php_version} $>> {log}'.format(sh_name=development.lower(), php_version=conf["name"],
                                                                                                                                               log=self.logs_file))
            # 检测是否安装成功
            if 'swoole' in development:
                development = 'swoole'
    ...
    def reduction_mysql(self, conf):
        ...
        if not os.path.exists(mysql_path):
            ...
            public.ExecShell("/bin/bash /www/adcitra/panel/install/install_soft.sh 1 install mysql {} &>> {}".format(version, self.logs_file))
        mysql_version = public.readFile('/www/server/mysql/version.pl')
    ...
    def reduction_apache(self, conf):
        ...
        if not os.path.exists(apache_path):
            ...
            if version.count('.') > 1:
                version = version.split('.')[:2]
                version = '.'.join(version)
            public.ExecShell("/bin/bash /www/adcitra/panel/install/install_soft.sh 1 install apache {} &>> {}".format(version, self.logs_file))
    ...
    def reduction_ftp(self, conf):
        ...
        if not os.path.exists(ftp_path):
            ...
            public.ExecShell("/bin/bash /www/adcitra/panel/install/install_soft.sh 1 install pure-ftpd {}&>> {}".format(version, self.logs_file))
        # 恢复配置文件
    ...
    def reduction_php_data(self, conf):
        ...
        nginx_conf_path = '/www/adcitra/panel/vhost/nginx/{}.conf'.format(conf["name"])
        apache_conf_path = '/www/adcitra/panel/vhost/apache/{}.conf'.format(conf["name"])
        ...
        res_nginx = self.uncopy_file(backup_conf_config["redirect_nginx_data"], '/www/adcitra/panel/vhost/nginx/redirect/{}'.format(conf["name"]), isfile=False)
        res_apache = self.uncopy_file(backup_conf_config["redirect_apache_data"], '/www/adcitra/panel/vhost/apache/redirect/{}'.format(conf["name"]), isfile=False)
        ...
        res_n = self.uncopy_file(backup_conf_config["proxy_nginx_data"], '/www/adcitra/panel/vhost/nginx/proxy/{}'.format(conf["name"]), isfile=False)
        res_a = self.uncopy_file(backup_conf_config["proxy_apache_data"], '/www/adcitra/panel/vhost/apache/proxy/{}'.format(conf["name"]), isfile=False)
        ...
        if backup_conf_config["ssl_info"]:
            ssl_path = '/www/adcitra/panel/vhost/cert/{}'.format(conf["name"])
        ...
        if backup_conf_config["dir_auth_json"]:
            ...
            if os.path.exists(dir_auth_path):
                ...
                if res:
                    ...
                    if data:
                        config_path = '/www/adcitra/panel/data/site_dir_auth.json'
                        if os.path.exists(config_path):
        ...
        if backup_conf_config["dir_auth_file"]:
            res = self.uncopy_file(backup_conf_config["dir_auth_file"], '/www/adcitra/panel/vhost/nginx/dir_auth/{}'.format(conf["name"]), isfile=False)
            if not res:
        ...
        if backup_conf_config["pseudo_static_data"]:
            ...
            res = self.uncopy_file(backup_conf_config["pseudo_static_data"], '/www/adcitra/panel/vhost/rewrite/{}.conf'.format(conf["name"]), isfile=True,
    ...
    def reduction_terminal_data(self, conf):
        ...
        terminal = "/www/adcitra/panel/config/ssh_info"
        self.print_log('开始还原terminal数据')
    ...
    def reduction_area_data(self, conf):
        ...
        area_data_path = "/www/adcitra/panel/data/firewall/country.json"
        if not backup_conf:
    ...
    def create_queue(self, get):
        ...
        # 导入任务管理模块
        import panelTask
        print("btpython /www/adcitra/panel/class/panelModel/whole_machine_backupModel.py {} {} &> {}".format(id, type, self.logs_file ))
        # 使用任务管理模块创建任务，执行命令通过 `btpython` 调用对应脚本
        panelTask.bt_task().create_task(
            name,  # 任务名称
            0,     # 任务类型（0 表示普通任务）
            "btpython /www/adcitra/panel/class/panelModel/whole_machine_backupModel.py {} {} &> {}".format(
                id,   # 任务 ID
                type, # 任务类型
