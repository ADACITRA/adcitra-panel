...
class main(safeBase):
    ...
    _trans_status = "/www/adcitra/panel/plugin/firewall/status.json"
    _rule_path = "/www/adcitra/panel/plugin/firewall/"
    old_ips_path = "/www/adcitra/panel/plugin/firewall/ips.txt"
    _ips_path = "/www/adcitra/panel/plugin/firewall/GeoLite2-Country.json"
    _country_path = "/www/adcitra/panel/plugin/firewall/country.txt"
    _white_list_file = "/www/adcitra/panel/plugin/firewall/whitelist.txt"  # 证书验证IP
    ...
    def add_crontab(self):
        ...
        if not public.M('crontab').where('name=?', ('[勿删]系统防火墙域名解析检测任务',)).count():
            cmd = '{} {}'.format(python_path, '/www/adcitra/panel/script/firewall_domain.py')
    ...
    def get_port_info(self, get):
        ...
        _panel_port_file = '/www/adcitra/panel/data/port.pl'
        ...
        result_list.append({"name": "灯塔面板", "status": 0, "port": panel_port})
    ...
    def get_anti_scan_status(self, get):
        ...
        @return:
        """
        plugin_path = "/www/adcitra/panel/plugin/fail2ban"
    ...
    def check_malicious_switch(self):
        ...
        """
        res = public.readFile("/www/adcitra/panel/config/firewalld_malicious_ip.pl")
    ...
    def process_malicious_ip_bantime(self,ban_time=None):
        ...
        :return: 返回封禁时间(天)
        """
        config_file = "/www/adcitra/panel/config/firewall_config.json"
    ...
    def switch_malicious_ip(self, get):
        ...
        if status == 1:
            ...
            public.ExecShell("sh /www/adcitra/panel/script/open_malicious_ip.sh") #打开功能
            public.writeFile("/www/adcitra/panel/config/firewalld_malicious_ip.pl", 'open')
            ...
            get.second = ""
            get.sType = "toShell"
            get.sBody = "/www/adcitra/panel/pyenv/bin/python3 -u /www/adcitra/panel/script/malicious_ip_task.py"
            get.sName = ""
            get.backupTo = ""
            get.save = ""
            get.urladdress = ""
            ...
            public.writeFile("/www/adcitra/panel/config/firewalld_malicious_ip.pl", 'close')
            return public.returnMsg(True, '恶意IP封禁功能已关闭!')
...
class Sqlite():
    ...
    def __init__(self):
        self.db_file = "/www/adcitra/panel/data/db/firewall.db"
        self.create_table()
    ...
    def create_table(self):
        ...
        if public.M('firewall_new').count() < 1:
            ...
            if not public.M('firewall_new').where('ports=?', (panel_port,)).count():
                public.M('firewall_new').add(
                    'ports,brief,addtime,protocol,types',
                    (panel_port, '灯塔面板', '0000-00-00 00:00:00', 'tcp', 'accept')
                )
