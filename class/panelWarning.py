...
class panelWarning:
    __path = '/www/adcitra/panel/data/warning'
    ...
    __report = '/www/adcitra/panel/data/warning_report'
    vul_num = 0
    ...
    def _get_list(self):
        ...
        if not enabled_categories:
            enabled_categories = ['adcitra_basic']
        ...
        # 将结果输出一份到报告目录下
        with open("/www/adcitra/panel/data/warning_report/data.json", "w") as f:
    ...
    def get_list(self, args):
        ...
        command = "btpython /www/adcitra/panel/class/panelWarning.py"
    ...
    def sync_rule(self):
        ...
        '''
        # try:
        #     dep_path = '/www/adcitra/panel/class/safe_warning'
        ...
        if not os.path.exists(self._vuln_result):
            ...
            if os.path.exists("/etc/redhat-release"):
                ...
                if "CentOS Linux release 8" in result:
                    tmp_dict[
                        "msg"] = "当前系统【centos_8】官方已停止维护，为了安全起见，建议升级至centos 8 stream\n详情参考教程：https://www.adcitra.cn/bbs/thread-82931-1-1.html"
            return tmp_dict
    ...
    def compare_md5(self):
        ...
        import requests
        # try:
        #    new_md5 = requests.get("https://www.adcitra.cn/vulscan_d11ad1fe99a5f078548b0ea355db42dc.txt").text
        # except:
        #    return 0
    ...
    def record_times(self):
        ...
        if not os.path.exists("/www/adcitra/panel/data/warning_report/record.json"):
            tmp = {"scan": [], "repair": []}
            ...
            public.WriteFile("/www/adcitra/panel/data/warning_report/record.json", json.dumps(tmp))
        try:
            with open("/www/adcitra/panel/data/warning_report/record.json", "r") as f:
                record = json.load(f)
        ...
        if record["scan"][weekday]["date"] == datetime.datetime.now().strftime("%Y/%m/%d"):
            ...
            record["scan"][weekday]["times"] = 1
        public.WriteFile("/www/adcitra/panel/data/warning_report/record.json", json.dumps(record))
    ...
    def load_category_config(self):
        ...
        @return dict
        """
        config_file = '/www/adcitra/panel/config/safe_categories.json'
        try:
    ...
    def load_modules_from_categories(self, categories):
        ...
        """
        base_dir = '/www/adcitra/panel/class/safe_warning'
    ...
    def set_scan_categories(self, args):
        ...
        @param dict_obj{
            categories<list> 要启用的分类列表，字符串形式传入如"["adcitra_basic","international"]"
        }
        @return dict
        """
        config = self.load_category_config()
        ...
        # 如果全部禁用，默认启用adcitra_basic
        has_enabled = any(v.get('enabled', False) for v in config.values())
        if not has_enabled:
            config['adcitra_basic']['enabled'] = True
    
        # 保存配置
        public.WriteFile('/www/adcitra/panel/config/safe_categories.json', json.dumps(config))
        return public.returnMsg(True, '设置成功')
...
if __name__ == "__main__":
    ...
    panel = panelWarning()
    # 将扫描结果保存到json
    public.WriteFile('/www/adcitra/panel/data/warning/resultresult.json', json.dumps(panel._get_list()))
    # et = time.time()
