...
# | AdCtira灯塔Windows面板
# +-------------------------------------------------------------------
# | Copyright (c) 2015-2020 AdCtira灯塔软件(https://www.adcitra.cn) All rights reserved.
# +-------------------------------------------------------------------
...
class site_push(base_push):
    ...
    def get_version_info(self, get):
        ...
        data['help'] = 'http://www.adcitra.cn/bbs'
        return data
    ...
    def get_panel_update_data(self, data):
        ...
        s_url = s_url.format('https://www.adcitra.cn')
    ...
    def get_panel_safe_push(self, data, result):
        ...
        if len(s_list) > 0:
            ...
            sdata = public.get_push_info('灯塔面板安全告警', s_list)
            for m_module in data['module'].split(','):
                if m_module == 'sms':
                    continue
                elif m_module == 'wx_account':
        ...
        if data['type'] in ['ssl']:
            ...
            if len(web_list) > 0:
                ...
                for m_module in data['module'].split(','):
                    ...
                    sdata = public.get_push_info('灯塔面板网站到期提醒', s_list)
            ...
            if res['expire'] > 0 and res['expire_day'] < data['cycle']:
                ...
                for m_module in data['module'].split(','):
                    ...
                    sdata = public.get_push_info('灯塔面板密码到期提醒', s_list)
            ...
            for m_module in data['module'].split(','):
                ...
                sdata = public.get_push_info('灯塔面板木马检测告警', s_list)
                result[m_module] = sdata
            return result
    ...
    def __get_ssl_result(self, data, clist, push_keys=[]):
        ...
        for m_module in data['module'].split(','):
            ...
            if m_module in ['sms']:
                sdata['sm_type'] = 'ssl_end|灯塔面板SSL到期提醒'
                ...
                sdata = public.get_push_info('灯塔面板SSL到期提醒', s_list)
