...
class panel_push:
    ...
    def get_module_config(self,get):
        ...
        for x in services:
            ...
            item['push'] = channels
            item['helps'] = ['部分服务停止可能会造成业务中断.','短信推送需提前购买，如需购买请联系[<a href="http://q.url.cn/CDfQPS?_type=wpa&amp;qidian=true" target="_blank" class="btlink">AdCtira灯塔客服</a>]']
            data.append(item)
        import json
    ...
    def __get_ltd_result(self,data,pro_data):
        ...
        for m_module in data['module'].split(','):
            ...
            if m_module in ['dingding','weixin',"feishu","mail"]:
                if m_module in ["mail"]:
                    newline="<br/>"
                    result[m_module]["title"] = "灯塔面板业务到期提醒"
                else:
                    newline = "\n\n"
    ...
    def __get_ssl_result(self,data,clist):
        ...
        for m_module in data['module'].split(','):
            ...
            if m_module in ['dingding','weixin',"feishu","mail"]:
                if m_module in ["mail"]:
                    newline="<br/>"
                    result[m_module]["title"] = "灯塔面板SSL到期提醒"
                else:
                    newline = "\n\n"
