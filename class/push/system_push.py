...
class ToWechatAccountMsg:
    ...
    def system_disk(msg_data: str, path: str):
        ...
        msg.thing_type = "AdCtira灯塔首页磁盘告警"
        if len(msg_data) > 20:
            path = path[:17] + "..."
        msg.msg = msg_data
        if len(path) > 14:
    ...
    def system_mem(count: float):
        msg = WxAccountMsg.new_msg()
        msg.thing_type = "AdCtira灯塔首页内存告警"
    ...
    def system_load(count: float):
        msg = WxAccountMsg.new_msg()
        msg.thing_type = "AdCtira灯塔首页负载告警"
    ...
    def system_cpu(count: float):
        msg = WxAccountMsg.new_msg()
        msg.thing_type = "AdCtira灯塔首页cpu告警"
