...
class _SiteDirAuth:
    ...
    def change_dir_auth_file_nginx_phpver(self, site_name, phpv, auth_name):
        ...
        if phpv == 'other':
            php_conf = "include /www/adcitra/panel/vhost/other_php/{}/enable-php-other.conf;".format(site_name)
        else:
            php_conf = 'include enable-php-{}.conf;'.format(phpv)

        rep = r"include\s+(enable-php-\w+|/www/adcitra/panel/vhost/other_php/{}/enable-php-other)\.conf;".format(
            site_name)
    ...
    def set_dir_auth_file(self, site_path, site_name, name, username, site_dir, auth_file):
        ...
        if php_ver:
            if php_ver == 'other':
                php_conf = "include /www/adcitra/panel/vhost/other_php/{}/enable-php-{}.conf;".format(site_name, php_ver)
            else:
