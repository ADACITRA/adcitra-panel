...
try:
    from AdCtiraPanel import cache,session
except:
    pass
...
class panelSSL:
    ...
    __BINDURL = 'https://api.adcitra.cn/Auth/GetAuthToken'
    __CODEURL = 'https://api.adcitra.cn/Auth/GetBindCode'
    __UPATH = 'data/userInfo.json'
    __userInfo = None
    __PDATA = None
    _check_url = None
    __request_url = None
    __WECHAT_AUTH_URL = 'https://www.adcitra.cn/api/v2/wx_auth_panel/auth_url' # 微信授权接口
    __WECHAT_CHECK_USER_URL = 'https://www.adcitra.cn/api/v2/wx_auth_panel/auth_login' # 微信绑定接口
    #构造方法
    def __init__(self):
        pdata = {}
        data = {}
    ...
    def set_cert(self,get):
        ...
        if ("years" in certInfo and certInfo["years"] > 1) and ("cert_ssl_type" in certInfo and certInfo["cert_ssl_type"] == 1):
            get.status = 1
            self.set_auto_renew_cert(get)  # AdCtira灯塔证书且签发时设置自动续签任务
        path = '/www/adcitra/panel/vhost/cert/' + siteName
        if  not os.path.abspath(path).startswith('/www/adcitra/panel/vhost/cert/'):
    ...
    def get_verify_result(self,args):
        ...
        if "cert_ssl_type" not in args or args.cert_ssl_type == '0':
            ...
            if verify_info['data']['application']['status'] == 'ongoing':
                return public.returnMsg(False,'订单出现问题，CA正在人工验证，若24小时内依然出现此提示，请联系AdCtira灯塔')
    ...
    def get_order_details(self,args):
        ...
        try:
            ...
            crontabs = public.M('crontab').where("name like '%自动续签多年AdCtira灯塔证书%'", ()).get()
    ...
    def set_auto_renew_cert(self,args):
        ...
        if str(status) == "1":
            crontab_malicious = public.M('crontab').where("name like '%自动续签多年AdCtira灯塔证书%'", ()).get()
            if len(crontab_malicious) > 0:
                return public.returnMsg(True, '任务已存在!')

            public.WriteLog('自动续签多年AdCtira灯塔证书', '功能开启')
            ...
            args.name = "[勿删]AdCtira灯塔自动续签多年AdCtira灯塔证书任务"
            args.type = "day"
            args.hour = run_hour
            ...
            res = public.M('crontab').where("name like '%自动续签多年AdCtira灯塔证书%'", ()).get()
            if len(res) > 0:
                for crontask in res:
                    args.id = crontask["id"]
    ...
    def GetOrderList(self,get):
        ...
        if hasattr(get,'siteName'):
            ...
            if os.path.exists(path):
                ...
                path = '/www/adcitra/panel/vhost/cert/' + get.siteName + '/partnerOrderId'
                if os.path.exists(path):
                    partnerOrderId = public.readFile(path)
    ...
    def CheckForceHTTPS(self,siteName):
        conf_file = '/www/adcitra/panel/vhost/nginx/{}.conf'.format(siteName)
    ...
    def GetCertList(self,get):
        try:
            vpath = '/www/adcitra/panel/vhost/ssl'
    ...
    def RemoveCert(self,get):
        try:
            vpath = '/www/adcitra/panel/vhost/ssl/' + get.certName.replace("*.",'')
    ...
    def SaveCert(self,get):
        try:
            ...
            vpath = '/www/adcitra/panel/vhost/ssl/' + certInfo['subject']
            vpath=vpath.replace("*.",'')
    ...
    def GetCert(self,get):
        ...
        vpath = os.path.join('/www/adcitra/panel/vhost/ssl' , get.certName.replace("*.",''))
        if not os.path.exists(vpath): return public.returnMsg(False,'证书不存在!')
        data = {}
    ...
    def GetCloudType(self, get):
        ...
        通过文件判断获取云服务商类型
        """
        # client  服务商标识 0：AdCtira灯塔账号，1：腾讯云，2：阿里云
        # login  授权登录方式，0：AdCtira灯塔账号，1：一键登录，2：授权登录
    ...
    def TencentcloudMarketAuth(self, get):
        ...
        try:
            ...
            if error.lower().find('json') >= 0:
                error = '<br>错误：连接AdCtira灯塔官网异常，请按照以下方法排除问题后重试：<br>解决方法：<a target="_blank" class="btlink" href="https://www.adcitra.cn/bbs/thread-87257-1-1.html">https://www.adcitra.cn/bbs/thread-87257-1-1.html</a><br>'
    ...
    def SetWechatBindUser(self, get):
        """
        微信绑定，用于绑定微信账号到AdCtira灯塔账号
        """
        try:
            # 获取参数
            pdata = {}
            pdata['o'] = self.get_suppliers_source() # 供应商来源
            pdata['serverid'] = panelAuth().get_serverid() # AdCtira灯塔ID
    ...
    """
    @name 统一请求接口
    @param url 返回URL不是www.adcitra.cn，修改config/config.json的home字段
    """
