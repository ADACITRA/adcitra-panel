...
class main(Base):
    # 2024/12/21 11:36 安装AdCtira灯塔邮局
    def install_service(self, get):
        ...
        # public.ExecShell("rm -f /www/adcitra/panel/install/mail_install.sh;wget -O " + install_file + " " + download_url + " --no-check-certificate")
        # if not os.path.exists(install_file): return public.returnMsg(False, '下载安装脚本失败')
        if public.M('tasks').where('name=? and status=?', ('安装 [AdCtira灯塔邮局]', '0')).count() > 0:
            return public.returnMsg(False, '安装任务已存在')
        else:
            execstr = "cd /www/adcitra/panel/class/mailModel/script && /bin/bash install.sh install"
            public.M('tasks').add('id,name,type,status,addtime,execstr', (
            None, '安装 [AdCtira灯塔邮局]', 'execshell', '0', time.strftime('%Y-%m-%d %H:%M:%S'), execstr))
    ...
    def install_status(self, get):
        ...
        @name 安装状态
        '''
        if os.path.exists("/www/adcitra/panel/plugin/mail_sys"):
            try:
