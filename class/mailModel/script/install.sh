#!/bin/bash
PATH=/www/adcitra/panel/pyenv/bin:/bin:/sbin:/usr/bin:/usr/sbin:/usr/local/bin:/usr/local/sbin:~/bin
export PATH

pluginPath=/www/adcitra/panel/plugin/mail_sys
pluginStaticPath=/www/adcitra/panel/plugin/mail_sys/static
...
Get_Public() {
    ...
    if [ "${publicFileMd5}" != "${md5check}" ]; then
        wget -O Tpublic.sh https://download.adcitra.cn/install/public.sh -T 20
...
Install() {
    ...
    if [ ! -d "/www/adcitra/panel/AdCtiraPanel/static/ckeditor" ]; then
        unzip $pluginPath/ckeditor.zip -d /www/adcitra/panel/AdCtiraPanel/static
    fi
    ...
    /www/adcitra/panel/pyenv/bin/python3.7 $pluginPath/mail_server_init.py setup_mail_sys
...
if [ "${1}" == 'install' ]; then
    Install
    echo '1' >/www/adcitra/panel/data/reload.pl
elif [ "${1}" == 'update' ]; then
    Update
