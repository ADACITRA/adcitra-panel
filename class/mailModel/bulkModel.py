...
try:
    ...
    if os.path.exists('/www/adcitra/panel/pyenv'):
        public.ExecShell('/www/adcitra/panel/pyenv/bin/pip install dnspython')
    else:
...
class main(Base):
    ...
    plugin_data = f'/www/adcitra/panel/plugin/mail_sys/data'  # 插件数据目录
    blcheck_count = f'/www/adcitra/panel/plugin/mail_sys/data/blcheck.json'  # 统计各个域名黑名单情况
    CONF_DNS_TRIES = 2
    CONF_DNS_DURATION = 6
    CONF_BLACKLISTS = [
            "bl.spamcop.net",
    ...
    def __init__(self):
        super().__init__()
        self.in_bulk_path = '/www/adcitra/panel/data/mail/in_bulk'
        ...
        # 群发任务  收件人发送记录
        self.sent_recipient_path = '/www/adcitra/panel/data/mail/in_bulk/recipient/sent_recipient'
        if not os.path.exists(self.sent_recipient_path):
            os.makedirs(self.sent_recipient_path)
        # 群发 收件人数量记录 取消记录
        # self.recipient_count_path = '/www/adcitra/panel/data/mail/in_bulk/recipient/recipient_count'
        # 数据库文件与名称
        self.db_files = {
    ...
    def restored_domain_send(self):
        restored_domain = '/www/adcitra/panel/plugin/mail_sys/data/restored_domain_send.pl'
    ...
    def check_task_status(self, args):
        ...
        try:
            ...
            start_mark = '/www/adcitra/panel/plugin/mail_sys/start_Task.pl'  # 无ptr记录每日发件数
            SendTaskId = '/www/adcitra/panel/plugin/mail_sys/SendTaskid.pl'
            ...
            if os.path.exists(start_mark):
                ...
                if int(public.readFile(start_mark)) + 86400 < cur_time:
                    ...
                    # 清空统计
                    count_sent = '/www/adcitra/panel/plugin/mail_sys/count_sent_domain.json'
    ...
    def check_task_finish(self, args=None):
        ...
        LastTaskId = '/www/adcitra/panel/plugin/mail_sys/LastTaskid.pl'
        SendTaskId = '/www/adcitra/panel/plugin/mail_sys/SendTaskid.pl'
        ...
        with self.M("email_task") as obj:
            ...
            if not process1_task:  # 没有执行中的  都是未执行或执行完的
                if not obj.where('task_process !=?', (2,)).count():  # 没有待执行或执行中  删掉额度占用
                    path = '/www/adcitra/panel/plugin/mail_sys/data/quota_occupation.json'
        ...
        # task_id = task_info['id']
        # 错误日志路径
        error_log = "/www/adcitra/panel/data/mail/in_bulk/errlog/task_{}.log".format(task_id)
    ...
    def add_task(self, args):
        ...
        # 指定邮件内容上传到 /www/adcitra/panel/data/mail/in_bulk/content/
        # 指定收件人上传到 /www/adcitra/panel/data/mail/in_bulk/recipient/
        # 新增 退订按钮
        # 新增 线程数
    ...
    def send_mail_test(self, args):
        ...
        #         # 测试  通过后删掉
        #         public.writeFile('/www/adcitra/panel/plugin/mail_sys/data/tiaaaa.txt', content)
    ...
    def import_contacts_etypes(self, args):
        ...
        if not os.path.exists("{}/recipient".format(self.in_bulk_path)):
            os.mkdir("{}/recipient".format(self.in_bulk_path))
        file_path = "{}/recipient/{}".format(self.in_bulk_path, file)
        # /www/adcitra/panel/data/mail/in_bulk/recipient/
        if not os.path.exists(file_path):
            return public.returnMsg(False, public.lang('文件不存在'))
    ...
    def get_task_list(self, args):
        ...
        try:
            ...
            for task in task_list:
                ...
                # 记录发送失败日志
                task['error_log'] = "/www/adcitra/panel/data/mail/in_bulk/errlog/task_{}.log".format(task_id)
                if not os.path.exists("/www/adcitra/panel/data/mail/in_bulk/errlog/task_{}.log".format(task_id)):
                    task['error_log'] = "/www/adcitra/panel/data/mail/in_bulk/errlog/{}_{}.log".format(task['task_name'],task_id)
    ...
    def delete_task(self, args):
        ...
        # 删除错误日志
        error_log = "/www/adcitra/panel/data/mail/in_bulk/errlog/task_{}.log".format(task_info['id'])
        if os.path.exists(error_log):
            os.remove(error_log)
        # 兼容旧日志文件
        error_log1 = "/www/adcitra/panel/data/mail/in_bulk/errlog/{}_{}.log".format(task_info['task_name'], task_info['id'])
        ...
        try:
            ...
            public.print_log(public.get_error_info())
        # 删除标记
        start_mark = '/www/adcitra/panel/plugin/mail_sys/start_Task.pl'
        start_send = '/www/adcitra/panel/plugin/mail_sys/start_Send.pl'
        if os.path.exists(start_mark):
            os.remove(start_mark)
    ...
    def _task_mail_send1(self, ):
        ...
        then
            ...
            btpython /www/adcitra/panel/class/mailModel/script/send_bulk_script.py
        fi
        '''
        ...
        try:
            ...
            if not c_id:
                ...
                data['type'] = 'minute-n'
                data['where1'] = '1'
                # data['sBody'] = 'btpython /www/adcitra/panel/plugin/mail_sys/script/send_bulk_script.py'
                data['sBody'] = cmd
                data['backupTo'] = ''
                data['sType'] = 'toShell'
                data['hour'] = ''
    ...
    def _task_mail_send2(self, ):
        ...
        then
            ...
            btpython /www/adcitra/panel/class/mailModel/script/mail_error_logs.py
        fi
        '''
        import crontab
        p = crontab.crontab()
    ...
    def get_unsubscribe_url(self,):
        # 获取退订链接
        path = "/www/adcitra/panel/plugin/mail_sys/setinfo.json"
        url = None
        ...
        # 如果没有设置url，使用默认的url
        if not url:
            ssl_status = public.readFile('/www/adcitra/panel/data/ssl.pl')
            ssl = 'https' if ssl_status else 'http'

            ip = public.readFile("/www/adcitra/panel/data/iplist.txt")
            port = public.readFile('/www/adcitra/panel/data/port.pl')
    ...
    def _mail_error_log_back(self, start, end, error_log, task_id):
        ...
        try:
            ...
            output_file1 = "/www/adcitra/panel/data/mail/in_bulk/errlog"
            # output_file = "/www/adcitra/panel/data/mail/in_bulk/errlog/task_err.log"
    ...
    def check_ptr_domain(self, domain):
        ...
        session = public.readFile('/www/adcitra/panel/plugin/mail_sys/session.json')
        if session:
    ...
    def get_SECRET_KEY(self):
        path = '/www/adcitra/panel/data/mail/jwt-secret.txt'
    ...
    def init_send(self):
        ...
        start_mark = '/www/adcitra/panel/plugin/mail_sys/start_Task.pl'
        start_send = '/www/adcitra/panel/plugin/mail_sys/start_Send.pl'
        # end_mark = '/www/adcitra/panel/plugin/mail_sys/end_Task.pl'
        if os.path.exists(start_mark):
            os.remove(start_mark)
        if os.path.exists(start_send):
            os.remove(start_send)
    ...
    def _check_blacklists(self, ip, domain, dns_servers):
        ...
        domain_check_log = f'/www/adcitra/panel/plugin/mail_sys/data/{domain}_blcheck.txt'
        # 清空历史记录
    ...
    def check_domain_blacklist_corn(self):
        ...
        # 关闭告警 跳过
        blacklist_alarm_switch = '/www/adcitra/panel/plugin/mail_sys/data/blacklist_alarm_switch'
    ...
    def get_alarm_send(self, args):
        ...
        keyword = args.keyword
        task_file_path = '/www/adcitra/panel/data/mod_push_data/task.json'
        sender_file_path = '/www/adcitra/panel/data/mod_push_data/sender.json'
        task_data = {}
    ...
    # 判断是否更新退订方式 是否缺少静态文件 /www/adcitra/panel/AdCtiraPanel/templates/default/unsubscribe.html
    def check_new_unsubscribe(self, ):
        ...
        if not os.path.exists('/www/vmail'):
            return
        # # 判断删掉标记 如果不存在 就删掉就任务
        # path = '/www/adcitra/panel/data/mailsys_check_new_unsubscribe.pl'
        # if os.path.exists(path):
        #     return

        unsubscribe_path = '/www/adcitra/panel/AdCtiraPanel/templates/default/unsubscribe.html'
