...
""".format(tag=re.escape(tag)))
    ...
    def get_net_interface(self, get):
        ...
        if "refresh" not in get or not get.refresh:
            try:
                result = json.loads(public.readFile('/www/adcitra/panel/config/net_interfaces.json'))
        ...
        for interface, addresses in net_interfaces.items():
            ...
            for addr in addresses:
                if addr.family == socket.AF_INET:  # IPv4
                    p_ip = public.ExecShell("curl --interface {} https://www.adcitra.cn/api/getIpAddress --connect-timeout 3 -m 3".format(addr.address))
                    if not p_ip[0]:
                        p_ip[0] = addr.address
                    ...
                    if addr.address.startswith('fe80::'):
                        continue
                    p_ip = public.ExecShell("curl -6 --interface {} https://www.adcitra.cn/api/getIpAddress --connect-timeout 3 -m 3".format(addr.address))
                    if not p_ip[0]:
                        p_ip[0] = addr.address
        ...
        public.writeFile('/www/adcitra/panel/config/net_interfaces.json', json.dumps(result))
    ...
    def get_ip_rotate_conf(self):
        ...
        @return:
        """
        path = "/www/adcitra/panel/config/mail_ip_rotate.json"
        try:
    ...
    def set_ip_rotate_conf(self, bind, tags=None, cycle=None, status=None):
        ...
        public.writeFile("/www/adcitra/panel/config/mail_ip_rotate.json", json.dumps(data))
    ...
    def check_ip_rotate_task_status(self):
        ...
        try:
            ...
            if not c_id:
                ...
                data['type'] = 'minute-n'
                data['where1'] = '1'
                data['sBody'] = 'btpython /www/adcitra/panel/class/mailModel/script/multi_ip_rotate.py'
                data['backupTo'] = ''
                data['sType'] = 'toShell'
                data['hour'] = ''
                data['minute'] = '1'
