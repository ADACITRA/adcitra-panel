...
from AdCtiraPanel import session, cache, json_header
...
class userlogin:
    ...
    def request_post(self, post):
        # 离线版用户登录验证
        if os.path.exists("/www/adcitra/panel/data/local_tip.pl"):
        ...
        try:
            ...
            _key_file = "/www/adcitra/panel/data/two_step_auth.txt"
            ...
            if hasattr(post, 'vcode'):
                ...
                public.writeFile("/www/adcitra/panel/data/dont_vcode_ip.txt", json.dumps({"client_ip": public.GetClientIp(), "add_time": now}))
            ...
            import traceback
            public.writeFile(
                '/www/adcitra/panel/data/login_err.log',
            ...
            return (public.returnJson(
                False, '登录出现异常，请联系AdCtira灯塔官方人员处理，详情：【{}】'.format(stringEx)),
                    json_header)
    ...
    def request_tmp(self, get):
        try:
            ...
            save_path = '/www/adcitra/panel/config/api.json'
    ...
    def check_two_step_auth(self):
        dont_vcode_ip_info = public.readFile("/www/adcitra/panel/data/dont_vcode_ip.txt")
        acc_client_ip = False
    ...
    def clear_session(self):
        try:
            ...
            if s_size > 1024 * 1024 * 10:
                from AdCtiraPanel import sdb
                if os.path.exists(session_file): os.remove(session_file)
                sdb.create_all()
                if not os.path.exists(session_file):
                    public.writeFile('/www/adcitra/panel/data/reload.pl', 'True')
                    return False
            return True
        except:
            return False
    ...
    def get_wechat_qrcode(self):
        ...
        try:
            ...
            if not userinfo:
                return public.returnMsg(False, "请先登录灯塔面板账号")
            params.update(userinfo)
            # 从官网获取二维码和公钥
            result = public.httpPost("https://www.adcitra.cn/api/v2/wx_auth_panel/auth_url",params)
            if not result:
    ...
    def check_wechat_login(self, get):
        ...
        try:
            ...
            # 发送请求检测登录状态
            result = public.httpPost("https://www.adcitra.cn/api/v2/wx_auth_panel/auth_login_bound",params)
            if not result:
                self.__login_error()
