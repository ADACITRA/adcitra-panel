...
# +-------------------------------------------------------------------
# | Author: lkqiang<lkq@adcitra.cn>
# +-------------------------------------------------------------------
# +--------------------------------------------------------------------
# |   AdCtira灯塔webshell 内置扫描
# +--------------------------------------------------------------------
...
class webshell_check:
    __PATH = '/www/adcitra/panel/data/'
    __mail_config = '/www/adcitra/panel/data/stmp_mail.json'
    __mail_list_data = '/www/adcitra/panel/data/mail_list.json'
    __dingding_config = '/www/adcitra/panel/data/dingding.json'
    __mail_list = []
    __weixin_user = []
    __user = {}
    ...
    def __init__(self):
        ...
        if not os.path.exists(self.__mail_list_data):
            ...
            try:
                ...
                self.__user = json.loads(public.ReadFile('/www/adcitra/panel/data/userInfo.json'))
            except:
                ret = []
                public.writeFile(self.__mail_list_data, json.dumps(ret))
    ...
    def send_baota2(self, filename):
        cloudUrl = public.GetConfigValue('home') + '/api/panel/adcitrawaf_submit'
    ...
    def check_webshell(self, filename):
        if not os.path.exists(filename): return False
        cloudUrl = public.GetConfigValue('home') + '/api/panel/adcitrawaf_check_file'
...
if __name__ == "__main__":
    ...
    try:
        public.WriteFile('/www/adcitra/panel/data/webshell_data.json', json.dumps([]))
        type = sys.argv[1]
        path = sys.argv[2]
        send = sys.argv[3]
        ...
        if type == 'dir':
            ...
            public.WriteFile('/www/adcitra/panel/data/webshell_data.json', json.dumps(data))
            localtime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
            if len(data) >= 1:
                public.WriteLog('AdCtira灯塔内置webshell查杀', '查杀%s目录查到如下木马%s' % (path, data))
