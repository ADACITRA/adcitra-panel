...
from AdCtiraPanel import auth, cache
...
class panelDaily:
    ...
    def check_databases(self):
        ...
        import sqlite3
        conn = sqlite3.connect("/www/adcitra/panel/data/system.db")
        cur = conn.cursor()
    ...
    def check_daily_status(self,get=None):
        if os.path.exists('/www/adcitra/panel/data/start_daily.pl'):
            return True
        else:
            return False
    ...
    def store_app_usage(self, time_key=None):
        ...
        plugins_size = public.get_path_size("/www/adcitra/panel/plugin")
        other_plugin_dirs = [
            "/www/server/total",
            "/www/server/adcitrawaf",
            "/www/server/coll",
            "/www/server/nginx",
            "/www/server/apache",
            "/www/server/redis"
    ...
    def get_daily_data(self, get):
        ...
        if not os.path.exists("/www/adcitra/panel/data/start_daily.pl"):
            public.writeFile("/www/adcitra/panel/data/start_daily.pl", get.date)
