...
def is_ipv6() -> bool:
    return os.path.exists('/www/adcitra/panel/data/ipv6.pl')
...
{{
    ...
    location / {{
        ...
        include /www/adcitra/panel/vhost/nginx/webdav/dir_auth/{site_name}.conf;
        # DIRECTORY-AUTH-END
...
class WebDavManager:
    ...
    def check_webdav_conf(cls):
        ...
        if nginx_conf.find("/www/adcitra/panel/vhost/nginx/webdav/*.conf;") == -1:
            rep = re.compile("include +/www/adcitra/panel/vhost/nginx/\*\.conf;\n")
            sun_conf_str = (
                "include /www/adcitra/panel/vhost/nginx/*.conf;\n"
                "include /www/adcitra/panel/vhost/nginx/webdav/*.conf;\n"
            )
