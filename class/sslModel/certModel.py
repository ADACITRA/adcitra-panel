...
class main(sslBase):
    ...
    def get_cert_to_site(self, pure=False):
        ...
        path = "/www/adcitra/panel/vhost/cert/"
    ...
    def remove_cert(self, ssl_id=None, ssl_hash=None, local: bool = False, cloud: bool = False, force=False):
        ...
        if target["cloud_id"] != -1 and cloud:
            ...
            url = "https://www.adcitra.cn/api/Cert_cloud_deploy/del_cert"
            try:
    ...
    def upload_cert(self, ssl_id=None, ssl_hash=None):
        ...
        # 对接云端
        url = "https://www.adcitra.cn/api/Cert_cloud_deploy/cloud_deploy"
    ...
    def _remove_ssl_from_local(self, ssl_hash):
        local_path = '/www/adcitra/panel/vhost/ssl'
    ...
    def save_cert(self, get):
        ...
        path = "/www/adcitra/panel/vhost/ssl_saved/" + hash_data
        csrpath = path + "/fullchain.pem"
        keypath = path + "/privkey.pem"
    ...
    def create_order(self, get):
        try:
            ...
            # 写入config
            config_path = "/www/adcitra/panel/config/letsencrypt_v2.json"
            try:
    ...
    def update_cert_from_cloud(self, get):
        ...
        # 对接云端
        url = "https://www.adcitra.cn/api/Cert_cloud_deploy/get_cert_list"
        try:
            res_text = public.httpPost(url, {
                "uid": user_info["uid"],
                "access_key": user_info["access_key"],
        ...
        for data in res_list:
            try:
                ...
                if hash_data in hash_list:
                    # 删除本地证书
                    public.ExecShell('rm -rf /www/adcitra/panel/vhost/ssl_saved/' + hash_data)
                self.save_cert(get)
    ...
    def add_renewal_task(self, get):
        ...
        get.sBody = "/www/adcitra/panel/pyenv/bin/python3 -u /www/adcitra/panel/class/acme_v2.py --renew=1 --index={} --cycle={}".format(get.index, get.cycle)
        get.type = "day"
        get.week = 1
    ...
    def get_cert_content(self, get):
        ...
        ssl_info["key"] = public.readFile("/www/adcitra/panel/vhost/ssl_saved/{}/privkey.pem".format(ssl_hash))
        ssl_info["cert"] = public.readFile("/www/adcitra/panel/vhost/ssl_saved/{}/fullchain.pem".format(ssl_hash))
    ...
    def new_ssl_proxy_request(self, params=None):
        if params is None:
            params = {}
        request_url = "https://api.adcitra.cn/v2/NewSsl/proxy"
        ...
        if params["url"] in ("/api/v1/ssl/get_order_list", "/api/v1/ssl/get_user_discount_list"):
            try:
                newssl_discount_serverid = json.loads(public.readFile("/www/adcitra/panel/data/newssl_discount.json"))["serverid"]
            except:
    ...
    def get_newssl_discount(self, get=None):
        ...
        if public.get_oem_name() not in ("ssl251104", "ssl251210"):
            return result
        if os.path.exists("/www/adcitra/panel/data/newssl_discount.json"):
            try:
                data = json.loads(public.readFile("/www/adcitra/panel/data/newssl_discount.json"))
    ...
    def _get_newssl_discount(self):
        ...
        request_url = "https://api.adcitra.cn/v2/new_ssl/claim_ssl_package"
        user_info = public.get_user_info()
        ...
        if result["status"]:
            public.writeFile("/www/adcitra/panel/data/newssl_discount.json", json.dumps(result["data"]))
        return result
    ...
    def get_auto_renewal(self, get=None):
        ...
        # 判断标记文件
        status = True
        if os.path.exists("/www/adcitra/panel/data/close_ssl_auto_renewal.pl"):
            status = False
        import acme_v2
        acm_obj = acme_v2.acme_v2()
        acm_obj.set_crond()
    ...
    def set_auto_renewal(self, get=None):
        ...
        """
        if os.path.exists("/www/adcitra/panel/data/close_ssl_auto_renewal.pl"):
            os.remove("/www/adcitra/panel/data/close_ssl_auto_renewal.pl")
        else:
            public.writeFile("/www/adcitra/panel/data/close_ssl_auto_renewal.pl", "1")
        import acme_v2
        acm_obj = acme_v2.acme_v2()
        acm_obj.set_crond()
        return public.returnMsg(True, '设置成功')
