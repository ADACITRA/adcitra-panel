...
try:
    from AdCtiraPanel import session, cache, send_file
    import requests
except:
    pass
...
class panelPlugin:
    ...
    __exists_names = {}
    __plugin_s_list = []
    __panel_path = '/www/adcitra/panel'
    __plugin_info = None
    __plugin_name = None
    ...
    __api_root_url = 'https://api.adcitra.cn'
    ...
    __error_html = '/www/adcitra/panel/AdCtiraPanel/templates/default/block_error.html'
    __sub_rules = []
    __dict__ = None
    __replace_rule = []
    ...
    def __init__(self):
        self.__install_path = '/www/adcitra/panel/plugin'
    ...
    def __get_new_url(self):
        ...
        '''
        self.__api_root_url = public.get_home_node('https://api.adcitra.cn')
    ...
    def __input_plugin(self,
        ...
        if p_info and os.path.exists(plugin_path_panel):
            ...
            for ico_file in ['svg', 'png']:
                ...
                icon_dfile = self.__panel_path + '/AdCtiraPanel/static/img/soft_ico/ico-{}.{}'.format(
    ...
    def __unpackup_plugin(self, tmp_file):
        ...
        try:
            ...
            if not 'author' in plugin_data_info:
                plugin_data_info['author'] = 'AdCtira灯塔'
            if not 'home' in plugin_data_info:
                plugin_data_info['home'] = 'https://www.adcitra.cn'
    ...
    def install_plugin(self, get):
        if not public.check_sys_write():
            return public.returnCode(
                False,
                '<a style="color:red;">错误：检测到系统关键目录不可写!</a><br>1、如果安装了[AdCtira灯塔系统加固]，请先关闭<br>2、如果安装了云锁，请关闭[系统加固]功能<br>3、如果安装了安全狗，请关闭[系统防护]功能<br>4、如果使用了其它安全软件，请先卸载<br>'
                , -1)
        if not 'sName' in get: return public.returnMsg(False, '请指定软件名称!')
        pluginInfo = self.get_soft_find(get.sName)
        p_node = '/www/adcitra/panel/install/public.sh'
    ...
    def install_sync(self, pluginInfo, get):
        if 'download' in pluginInfo['versions'][0]:
            tmp_path = '/www/adcitra/panel/temp'
            ...
            public.downloadFile('https://www.adcitra.cn/api/Pluginother/get_file?fname=' +
    ...
    def set_pyenv(self, filename):
        if not os.path.exists(filename): return False
        env_py = '/www/adcitra/panel/pyenv/bin'
        if not os.path.exists(env_py): return False
        temp_file = public.readFile(filename)
        env_path = [
            'PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/local/bin:/usr/local/sbin:~/bin'
    ...
    def install_async(self, pluginInfo, get):
        ...
        execstr = "cd /www/adcitra/panel/install && /bin/bash install_soft.sh {} {} {} {} {}".format(
    ...
    def uninstall_plugin(self, get):
        ...
        if pluginInfo['type'] != 5:
            ...
            execstr = "cd /www/adcitra/panel/install && /bin/bash install_soft.sh " + get.type + " uninstall " + get.sName.lower(
            ) + " " + get.version.replace('.', '')
            ...
            # 设置PHP环境变量
            if get.sName in ['php']:
                public.ExecShell("btpython /www/adcitra/panel/tools.py phpenv")
            public.start_syssafe()  # 恢复系统加固服务
            return public.returnMsg(True, "PLUGIN_UNINSTALL")
    ...
    def __is_bind_user(self):
        ...
        #     raise public.PanelError('请先绑定AdCtira灯塔帐号!')
        return True
    ...
    def clean_panel_log(self):
        try:
            ...
            # 清理错误日志
            public.clean_max_log('/www/adcitra/panel/logs/error.log', 10, 20)
            public.clean_max_log('/www/adcitra/panel/logs/socks5.log', 10, 20)
            public.clean_max_log('/www/adcitra/panel/logs/oos.log', 10, 20)
            return True
        except:
            return False
        ...
        try:
            ...
            pluginInfo = {
                ...
                "mutex":
                    "",
                "install_checks":
                    "/www/adcitra/panel/plugin/" + info['name'],
                "uninsatll_checks":
                    "/www/adcitra/panel/plugin/" + info['name'],
                "compile_args":
                    0,
        ...
        if 'recommend' in softList:
            for n in range(len(softList['recommend'])):
                ...
                for i in range(len(softList['recommend'][n]['data'])):
                    check_path = '/www/adcitra/panel/plugin/' + softList[
    ...
    def check_status(self, softInfo):
        ...
        if softInfo['setup']:
            ...
            softInfo['versions'] = self.tips_version(softInfo['versions'],
                                                     softInfo['version'])
            softInfo['admin'] = os.path.exists('/www/adcitra/panel/plugin/' +
                                               softInfo['name'])
        ...
        if softInfo['setup']:
            ...
            if softInfo['name'] == 'mysql':
                ...
                softInfo['status'] = False if not os.path.exists("/www/adcitra/panel/plugin/mail_sys/mail_sys_main.py") else True
    ...
    def get_phpmyadmin_stat(self):
        ...
        if webserver == 'nginx':
            ...
            filename = "/www/adcitra/panel/vhost/openlitespeed/detail/phpmyadmin.conf"
        if not os.path.exists(filename): return False
    ...
    def get_soft_find(self, get=None):
        ...
        if hasattr(get, 'sName'):
            uid = session.get('uid')
            if uid != 1 and uid:
                if os.path.exists('/www/adcitra/panel/plugin/users/authority/{}'.format(uid)):
                    user = public.readFile('/www/adcitra/panel/plugin/users/authority/{}'.format(uid))
                    user = json.loads(self._decrypt(user))
                    if user['role'] != 'administrator':
                        if os.path.exists('/www/adcitra/panel/plugin/users/authority/{}.plugin'.format(uid)):
                            data = public.readFile('/www/adcitra/panel/plugin/users/authority/{}.plugin'.format(uid))
        ...
        for softInfo in self.__plugin_s_list:
            ...
            if softInfo['name'] == sName:
                if sName == 'phpmyadmin':
                    ...
                    from AdCtiraPanel import get_phpmyadmin_dir
    ...
    def get_icon(self, name, downFile=None):
        ico_type = 'png'
        '''/static/img/soft_ico/'''
        iconFile = 'AdCtiraPanel/static/img/soft_ico/ico-{}.{}'.format(name, ico_type)
        vue_iconFile = 'AdCtiraPanel/static/vite/images/soft_ico/ico-{}.{}'.format(name, ico_type)
        ...
        if pluginInfo['tip'] == 'lib':
            ...
            if hasattr(get, 'upgrade'):
                if get.upgrade:
                    mtype = 'update'
                    mmsg = 'upgrade'
            execstr = "cd /www/adcitra/panel/install && /bin/bash install_soft.sh " + get.type + " " + mtype + " " + get.name + " " + get.version
            sql = db.Sql()
            if hasattr(get, 'id'):
                id = get.id
            else:
    ...
    def unInstall(self, get):
        ...
        if pluginInfo['tip'] == 'lib':
            ...
            public.writeFile('/var/bt_setupPath.conf',
                             public.GetConfigValue('root_path'))
            execstr = "cd /www/adcitra/panel/install && /bin/bash install_soft.sh " + get.type + " uninstall " + get.name.lower(
            ) + " " + get.version.replace('.', '')
            public.ExecShell(execstr)
            public.WriteLog('TYPE_SETUP', 'PLUGIN_UNINSTALL',
                            (get.name, get.version))
    ...
    def _get_ols_myphpadmin_info(self):
        ...
        filename = '/www/adcitra/panel/vhost/openlitespeed/listen/888.conf'
        conf = public.readFile(filename)
        reg = 'address\s+\*\:(\d+)'
...
`num` INTEGER
    ...
    def getCloudPHPExt(self, get=None):
        ...
        try:
            ...
            # l_file = 'AdCtiraPanel/templates/default/license.html'
    ...
    def update_zip(self, get=None, tmp_file=None, update=False):
        ...
        try:
            ...
            if not 'home' in data:
                data['home'] = 'https://www.adcitra.cn/bbs/forum-40-1.html'

            plugin_path = '/www/adcitra/panel/plugin/' + data[
                'name'] + '/info.json'
            data['old_version'] = '0'
    ...
    def input_zip(self, get):
        ...
        plugin_path = '/www/adcitra/panel/plugin/' + get.plugin_name
        ...
        if p_info:
            ...
            for ico_file in ['svg', 'png']:
                ...
                icon_dfile = '/www/adcitra/panel/AdCtiraPanel/static/img/soft_ico/ico-{}.{}'.format(get.plugin_name, ico_file)
    ...
    def export_zip(self, get):
        ...
        get.dfile = '/www/adcitra/panel/temp/bt_plugin_' + get.plugin_name + '.zip'
        get.type = 'zip'
        import files
        files.files().Zip(get)
    ...
    def set_plugin_ignore(self, get):
        try:
            ...
            if not hasattr(get, 'plugin_name') or not hasattr(get, 'plugin_version'):
                return public.returnMsg(False, "参数错误")
            if os.path.exists('/www/adcitra/panel/data/plugin_ignore.json'):
                plugin_ignore = json.loads(public.readFile('/www/adcitra/panel/data/plugin_ignore.json'))
            else:
                plugin_ignore = []
            ...
            public.writeFile('/www/adcitra/panel/data/plugin_ignore.json', json.dumps(plugin_ignore))
            return public.returnMsg(True, "设置成功")
        except:
...
export PATH
mtype=$1
. /www/adcitra/panel/install/public.sh
...
"""
            public.writeFile("/www/adcitra/panel/install/pre_install_lib.sh", install_lib)
            execstr = "cd /www/adcitra/panel/install && /bin/bash pre_install_lib.sh {}".format(mtype)
            task_id = public.M('tasks').add(
                'id,name,type,status,addtime,execstr',
