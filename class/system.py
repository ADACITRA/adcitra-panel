...
try:
    from AdCtiraPanel import session, cache, request
except:
    pass
...
class system:
    ...
    def GetConcifInfo(self, get=None):
        ...
        if os.path.exists(self.setupPath + '/nginx/version.pl'):
            ...
            try:
                if os.path.exists(configFile):
                    conf = public.readFile('/www/adcitra/panel/vhost/openlitespeed/detail/phpmyadmin.conf')
                    rep = "/usr/local/lsws/lsphp(\d+)/bin/lsphp"
                    rtmp = re.search(rep, conf)
                    if rtmp:
                        phpversion = rtmp.groups()[0]
                    conf = public.readFile('/www/adcitra/panel/vhost/openlitespeed/listen/888.conf')
                    rep = "address\s+\*\:(\d+)"
                    rtmp = re.search(rep, conf)
                    if rtmp:
                        phpport = rtmp.groups()[0]
    ...
    def GetPanelInfo(self, get=None):
        ...
        templates = []
        # for template in os.listdir('AdCtiraPanel/templates/'):
    ...
    def GetDiskInfo(self, get=None):
        ...
        try:
            ...
            for disk in diskIo:
                ...
                tmp['path'] = disk.mountpoint.replace('/usr/local/lighthouse/softwares/adcitra_panel', '/www')
    ...
    def GetDiskInfo2(self, human=True):
        ...
        for tmp in temp1:
            ...
            try:
                ...
                arr['path'] = disk[6].replace('/usr/local/lighthouse/softwares/adcitra_panel', '/www')
    ...
    def set_rname(self, get):
        try:
            config_path = '/www/adcitra/panel/data/set_disk_rname.json'
    ...
    def GetNetWork(self, get=None):
        ...
        if get != False:
            ...
            disk_list = []
            rname_config = {}
            if os.path.exists('/www/adcitra/panel/data/set_disk_rname.json'):
                rname_config = json.loads(public.readFile('/www/adcitra/panel/data/set_disk_rname.json'))
    ...
    def ReWeb(self, get):
        ...
        # public.ExecShell("chmod +x /www/adcitra/panel/script/set_fastest_pip_source.sh ")
        # # 执行脚本并获取输出
        # subprocess.check_output(["/www/adcitra/panel/script/set_fastest_pip_source.sh"], shell=True)
    ...
    def _repair_panel(self,get):
        ...
        if not os.path.exists(nfile):
            os.system('btpython /www/adcitra/panel/script/reload_check.py repair')
    ...
    def RepPanel(self, get):
        public.ExecShell("chmod +x /www/adcitra/panel/script/clean_crontab.sh")
        public.ExecShell("nohup /www/adcitra/panel/script/clean_crontab.sh > /www/clean_crontab.txt 2>&1 &")
    ...
    def upgrade_panel(self, get):
        ...
        if not updateInfo:
            return public.returnMsg(False, '无法连接【AdCtira灯塔官网】，请检查网络原因.')
        ...
        if not 'force' in get:
            ...
            down_url = 'http://download.adcitra.cn/install/update/LinuxPanel-{}.pl'.format(data['local']['version'])
