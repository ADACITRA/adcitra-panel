...
#         if not self.__error_html: self.__error_html = self.panel_path + '/AdCtiraPanel/templates/default/block_error.html'
...
#                 return return_default
#
#             if os.path.exists("/www/adcitra/panel/install/{}_not_support.pl".format(soft_name)):
#                 return (0, '不兼容此系统！请点详情说明！')
#
#             if os.path.exists("/www/adcitra/panel/install/{}_mem_kill.pl".format(soft_name)):
...
#         '''
#         time.sleep(60)
#         # 进程流量监控，如果文件：/www/adcitra/panel/data/is_net_task.pl 或 /www/adcitra/panel/data/control.conf不存在，则不监控进程流量
...
#         '''
#         _ips_path = "/www/adcitra/panel/data/firewall/GeoLite2-Country.json"
#         m_time_file = "/www/adcitra/panel/data/firewall/geoip_mtime.pl"
#
#         if not os.path.exists(_ips_path):
#             os.system("mkdir -p /www/adcitra/panel/data/firewall")
...
#             @return dict{"status":True/False,"msg":"提示信息"}
#         '''
#         public.ExecShell("/www/adcitra/panel/pyenv/bin/python3.7 /www/adcitra/panel/class/adcitraDockerModel/script/syncreposdb.py")
#
...
#             if comm.find('AdCtira-Panel') == -1:
#                 self.service_panel('start')
#                 continue
#
#     def update_panel(self):
#         os.system("curl -k https://download.adcitra.cn/install/update6.sh|bash &")
#
...
#                     n = p.cmdline()[-1]
#                     if n.find('runserver') != -1 or n.find('AdCtira-Panel') != -1:
#                         return pid
#                 except:
#                     pass
#         except:
...
#         return None
#
#     def HttpGet(self, url, timeout=6, UserAgent='AdCtira-Panel'):
#         try:
...
#             return str(ex)
#
#     def HttpPost(self, url, data, timeout=6, UserAgent='AdCtira-Panel'):
#         try:
#             pdata = ""
#             if type(data) == dict:
#                 for key in data:
...
#
#             from adcitraDockerModel import dk_public as dp
...
class _AdCtiraPanelDaemonService:
...
def start_scripts_service():
    ...
    def need_safecloud_task():
        run_tip_file = "/www/adcitra/panel/data/warning_list_runtime.pl"
        last_run_time = 0
    ...
    flush_docker_hub_repos = _Script(
        ...
        cmd='nohup {} {}/class/adcitraDockerModel/script/syncreposdb.py > /dev/null 2>&1 &'.format(
            PY_BIN, PANEL_PATH
        ),
        interval=86400)
    ...
    def check_database_quota_prep():
        run_tip_file = "/www/adcitra/panel/config/quota_list.json"
        try:
            if os.path.exists(run_tip_file):
                with open(run_tip_file, 'r') as fp:
...
def panel_daemon_start():
    _AdCtiraPanelDaemonService().start()
