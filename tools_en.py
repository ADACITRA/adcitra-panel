# coding: utf-8
# +-------------------------------------------------------------------
# | AdCtira-Panel Linux Panel
# +-------------------------------------------------------------------
# | Copyright (c) 2015-2099 AdCtira-Panel(http://adcitra.cn) All rights reserved.
# +-------------------------------------------------------------------
...
def PackagePanel():
    ...
    os.system('rm -f /www/adcitra/panel/data/iplist.txt')
    os.system('rm -f /www/adcitra/panel/data/address.pl')
    os.system('rm -f /www/adcitra/panel/data/*.login')
    os.system('rm -f /www/adcitra/panel/data/domain.conf')
    os.system('rm -f /www/adcitra/panel/data/user*')
    os.system('rm -f /www/adcitra/panel/data/admin_path.pl')
...
'''
    ...
    if not a_input in ['Y', 'y', 'yes', 'YES']:
        public.ExecShell("rm -f /www/adcitra/panel/php_mysql_auto.pl")
    else:
        public.writeFile('/www/adcitra/panel/php_mysql_auto.pl', "True")

    print("|-Please select IDC brand information display settings:")
    print("=" * 50)
    print(" (1) Display default AdCtira-Panel Linux information")
    ...
    if i_input in [2, '2']:
        ...
        pInfo['title'] = u'AdCtira-Panel Linux Panel'
        pInfo['brand'] = u'AdCtira-Panel'
    ...
    print(p_input)
    if p_input in [2, '2']:
        public.writeFile('/www/adcitra/panel/aliyun.pl', "True")
        s_file = '/www/adcitra/panel/install.pl'
        if os.path.exists(s_file): os.remove(s_file)
        public.M('config').where("id=?", ('1',)).setField('status', 1)
    elif p_input in [3, '3']:
        public.writeFile('/www/adcitra/panel/aliyun.pl', "True")
        public.writeFile('/www/adcitra/panel/random_path.pl', "True")
        s_file = '/www/adcitra/panel/install.pl'
        if os.path.exists(s_file): os.remove(s_file)
        public.M('config').where("id=?", ('1',)).setField('status', 1)
    else:
        public.writeFile('/www/adcitra/panel/install.pl', "True")
...
def get_host_all():
    ...
    for ip in bind_ip:
        ...
        ip_list.append(ip)
    net_ip = public.httpGet("https://api.adcitra.cn/api/getipaddress")
...
def ClearOther():
    clearPath = [
        {'path': '/www/adcitra/panel', 'find': 'testDisk_'},
        {'path': '/www/wwwlogs', 'find': 'log'},
        {'path': '/tmp', 'find': 'panelBoot.pl'},
        {'path': '/www/adcitra/panel/install', 'find': '.rpm'},
        {'path': '/www/adcitra/panel/install', 'find': '.zip'},
        {'path': '/www/adcitra/panel/install', 'find': '.gz'}
    ]
...
def set_panel_port(panelport):
    ...
    panelPath = '/www/adcitra/panel/data/o.pl'
...
def set_panel_path(adminpath):
    ...
    msg = ''
    from AdCtiraPanel import admin_path_checks
...
def set_panel_ssl(status):
    ...
    if status == "enable":
        CreateSSL()
    if status == "disable":
        os.system('btpython /www/adcitra/panel/class/config.py SetPanelSSL')
        os.system("/etc/init.d/bt reload")
...
def sync_tencent_ssl(tencent_ssl_path="/root/tencent_ssl"):
    ...
    panel_ssl_path = "/www/adcitra/panel/vhost/ssl"
...
def get_temp_login():
    ...
    if public.M('temp_login').insert(pdata):
        if os.path.exists('/www/adcitra/panel/data/ssl.pl'):
            HTTP_C="https://"
        else:
            HTTP_C="http://"

        IP_ADDRES=public.ExecShell("curl -sS --connect-timeout 10 -m 20 {}".format(public.get_home_node("https://www.adcitra.cn/Api/getIpAddress")))[0]

        PANEL_PORT=public.readFile("/www/adcitra/panel/data/port.pl")
...
def get_temp_login_ipv4():
    ...
    if public.M('temp_login').insert(pdata):
        ...
        IP_ADDRES=public.ExecShell("curl -4 -sS --connect-timeout 10 -m 20 {}".format(public.get_home_node("https://www.adcitra.cn/Api/getIpAddress")))[0]
...
def create_reverse_proxy(get):
    ...
    try:
        close_reverse_proxy()
        __http = 'https' if os.path.exists("/www/adcitra/panel/data/ssl.pl") else 'http'
        if __http != "https":
            CreateSSL()
        __http = "https"
        ...
        args = public.to_dict_obj({
            ...
            "remark": "AdCtira-Panel reverse proxy [Please do not misoperate, modification may cause panel inaccessible]"
        })
        ...
        #     public.ExecShell("rm -rf /www/adcitra/panel/data/http_auth.pwd")
        #
...
def check_disk_space():
    ...
    if disk_info.free < 50 * 1024 * 1024:
        ...
        os.system("echo -e '\\e[33mCommand: btpython /www/adcitra/panel/script/adcitra_cli.py\\e[0m'")
...
def bt_cli(u_input=0):
    ...
    raw_tip1 = "===================================================================================="
    if not u_input:
        print("=================================AdCtira-Panel Command Line==============================")
    ...
    try:
        ...
        if u_input[:6] in ['install', 'update']:
            ...
            print(raw_tip)
            os.system(
                "bash /www/adcitra/panel/install/install_soft.sh {} {} {} {}".format(install_input, install_args[0],
    ...
    if u_input == 32:
        ...
        print(raw_tip)
        print("After setting this function, you can access AdCtira-Panel without port")
        print("For example, set domain as: panel.adcitra.cn")
        print("Then you can access panel via https://panel.adcitra.cn")
        ...
        __http = 'https://' if os.path.exists("/www/adcitra/panel/data/ssl.pl") else 'http://'
        admin_path = public.readFile('/www/adcitra/panel/data/admin_path.pl').strip() if os.path.exists(
            "/www/adcitra/panel/data/admin_path.pl") else "/login"
        ...
        bt_password = public.readFile("/www/adcitra/panel/default.pl")
        ...
        if site_name == "":
            print("Domain or IP cannot be empty, please re-enter, e.g.: 192.168.100.100 or panel.adcitra.cn")
            return
        ...
        if site_name.startswith("0 ") or site_name == "0":
            ...
            if not public.check_ip(site_name) and not public.is_domain(site_name):
                print("Domain or IP format error, please re-enter, e.g.: panel.adcitra.cn")
                return
    ...
    if u_input == 33:
        ...
        print("Panel reverse proxy closed successfully")
    if u_input == 31:
        os.system('tail -50 /www/adcitra/panel/data/login_err.log')
    if u_input == 26:
        os.system('btpython /www/adcitra/panel/class/config.py SetPanelSSL')
        os.system("/etc/init.d/bt reload")
    if u_input == 28:
    ...
    if u_input == 29:
        os.system("rm -rf /www/adcitra/panel/data/ssl_verify_data.pl")
        os.system("/etc/init.d/bt restart")
    ...
    if u_input == 1:
        ...
        sess_file = '/www/adcitra/panel/data/session'
        ...
        try:
            if os.path.exists('/www/adcitra/panel/data/panel_generation.pl'):
                conf = json.loads(public.readFile('/www/adcitra/panel/data/panel_generation.pl'))
                __http = 'https://' if os.path.exists("/www/adcitra/panel/data/ssl.pl") else 'http://'
                admin_path = public.readFile('/www/adcitra/panel/data/admin_path.pl')
                if admin_path:
                ...
                public.writeFile('/www/adcitra/panel/data/panel_site_address.pl', address)
            else:
                public.ExecShell("rm -rf /www/adcitra/panel/data/panel_site_address.pl")
        except:
            pass
        ...
        l_path = '/www/adcitra/panel/data/log_not_gzip.pl'
        ...
        l_path = '/www/adcitra/panel/data/not_auto_backup.pl'
        if os.path.exists(l_path):
        ...
        try:
            limit_area_json = json.loads(public.readFile("/www/adcitra/panel/data/limit_area.json"))
        ...
        print("|-Current panel login restriction rule: [{}] following regions to login AdCtira-Panel!\n".format(rule_type))
        if country:
        ...
        os.system('tail -100 /www/adcitra/panel/logs/error.log')
    elif u_input == 23:
        filename = '/www/adcitra/panel/config/basic_auth.json'
        if os.path.exists(filename): os.remove(filename)
        os.system('bt reload')
        print("|-BasicAuth disabled")
    elif u_input == 24:
        filename = '/www/adcitra/panel/data/two_step_auth.txt'
        if os.path.exists(filename): os.remove(filename)
        print("|-Google Authenticator disabled")
    elif u_input == 25:
        l_path = '/www/adcitra/panel/data/not_file_history.pl'
        if os.path.exists(l_path):
        ...
        public.ExecShell("chmod +x /www/adcitra/panel/script/adcitra_cli.py")
        os.system("/www/adcitra/panel/script/adcitra_cli.py")
    elif u_input == 36:
        public.ExecShell("chmod +x /www/adcitra/panel/script/adcitra_cli.py")
        os.system("/www/adcitra/panel/script/adcitra_cli.py")
    elif u_input == 99:
        os.system("btpython /www/adcitra/panel/tools_en.py cli")
