#!/bin/bash
#===============================================================================
# 灯塔面板Python 3.13环境升级脚本 - 简化版本
# 功能：将灯塔面板Python环境从3.7升级到3.13（仅Python环境升级）
...
GetSysInfo(){
    ...
    echo -e ${SYS_INFO}
    echo -e "请截图以上报错信息发帖至论坛www.adcitra.cn/bbs求助"
}

show_usage() {
    echo "灯塔面板Python 3.13环境升级脚本使用说明："
    echo ""
    echo "用法: $0 [选项]"
    echo ""
    echo "选项："
...
check_bt_panel() {
    if [ ! -d "/www/adcitra/panel" ]; then
        log_error "未检测到灯塔面板，请先安装灯塔面板"
        mkdir -p /www/adcitra/panel
        HAS_OLD_PANEL=false
    fi
    ...
    if [ ! -f "/www/adcitra/panel/init.sh" ] || [ ! -d "/www/adcitra/panel/data" ]; then
        HAS_OLD_PANEL=false
    fi
...
install_python_packages() {
    ...
    if [ -f "/www/adcitra/panel/script/$PIP_REQUIREMENTS_FILE" ]; then
        log_info "使用本地Python包列表: /www/adcitra/panel/script/$PIP_REQUIREMENTS_FILE"
        # 复制本地文件到安装目录
        cp "/www/adcitra/panel/script/$PIP_REQUIREMENTS_FILE" .
    else
...
main() {
    print_separator
    log_info "灯塔面板Python 3.13环境升级脚本开始执行..."
