...
# lets_info = public.readFile("/www/adcitra/panel/ssl/lets.info")
cert_info = public.get_cert_data("/www/adcitra/panel/ssl/certificate.pem")
# if lets_info:
...
#         public.writeFile('/www/adcitra/panel/data/reload.pl',"1")
#         exit(0)
if cert_info and cert_info['issuer'] in ["R10", "Let's Encrypt", "R8", "R11", "R5", "R3"] or cert_info.get('issuer_O', "") == "Let's Encrypt":
    ...
    domain = public.ReadFile('/www/adcitra/panel/data/domain.conf')
    if domain:
        domain = domain.strip()
            ...
            if info['issuer'] in ["R10", "Let's Encrypt", "R8", "R11", "R5", "R3"] or info.get("issuer_O", "") == "Let's Encrypt":
                if ssl['dns'] == [ip] and ssl['not_after'] > today and ssl['not_after'] > cert_info['notAfter']:
                    ...
                    if cert_pem and key_pem:
                        print("替换证书")
                        public.writeFile('/www/adcitra/panel/ssl/certificate.pem', cert_pem)
                        public.writeFile('/www/adcitra/panel/ssl/privateKey.pem', key_pem)
                        public.writeFile('/www/adcitra/panel/data/reload.pl', "1")
                        exit(0)
        print("尝试使用IP申请新的证书")
        import subprocess
        with open("/tmp/auto_apply_ip_ssl.log", "w") as stdout:
            ...
            result = subprocess.run(
                [
                    ...
                    "-u",
                    "/www/adcitra/panel/script/auto_apply_ip_ssl.py",
                    "-ips", ip,
                    "-path", "/www/adcitra/panel/ssl/",
                ],
                stdout=stdout,
                stderr=stdout,
                text=True
        ...
        if result.returncode == 0:
            ...
            def CreateSSL():
                ...
                try:
                    ...
                    if 'status' in result:
                        if result['status']:
                            public.writeFile('/www/adcitra/panel/ssl/certificate.pem', result['cert'])
                            public.writeFile('/www/adcitra/panel/ssl/privateKey.pem', result['key'])
                            public.writeFile('/www/adcitra/panel/ssl/baota_root.pfx', base64.b64decode(result['pfx']), 'wb+')
                            public.writeFile('/www/adcitra/panel/ssl/root_password.pl', result['password'])
                            public.writeFile('/www/adcitra/panel/data/ssl.pl', 'True')
