...
down_list = [
    'download.adcitra.cn',
    'cmcc1-node.adcitra.cn',
    'ctcc1-node.adcitra.cn',
    'hk1-node.adcitra.cn',
    'down-node1.adcitra.cn',
    'down-node2.adcitra.cn'
]
...
def check_node_url():
    ...
    if 'www-node' in data:
        #0.5秒检测一次
        for node in ({"name": "源站","url": "www.adcitra.cn"}, *data['www-node']):
    ...
    if 'api-node' in data:
        #0.5秒检测一次
        for node in ({"name": "源站","url": "api.adcitra.cn"}, *data['api-node']):
