...
# 输出成功信息, 必须输出 "AdCtira-Panel Update Ready" 才证明预处理成功
function success() {
    local message=$1
    if [ -n "$message" ]; then
        echo "$message"
    fi
    echo "AdCtira-Panel Update Ready"
}
...
function prepare_11_5() {
    ...
    local pip_bin="/www/adcitra/panel/pyenv/bin/python3 -m pip"
...
function after_11_5() {
    ...
    chmod +x /www/adcitra/panel/script/adcitra_cli.py
    ln -sf /www/adcitra/panel/script/adcitra_cli.py /usr/bin/btcli
...
function prepare_11_8() {
  ...
  rm -rf /www/adcitra/panel/data/agent/skills/adcitra_panel
  rm -rf /www/adcitra/panel/data/agent/skills/adcitra_panel_files
  rm -rf /www/adcitra/panel/data/agent/skills/adcitra_panel_phpsite
