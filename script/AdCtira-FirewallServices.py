...
def load_iptables():
    ...
    if run_cmd("iptables -C INPUT -j IN_BT"):
        ...
        if run_cmd("iptables-restore --noflush < /www/adcitra/panel/data/iptablesdata"):
            print("iptables 已恢复")
...
def load_ipset():
    ...
    恢复 ipset 规则
    """
    if run_cmd("ipset restore < /www/adcitra/panel/data/ipsetdata"):
        print("ipset 已恢复")
    else:
        print("ipset 已存在")
    ...
    if run_cmd("iptables -C INPUT -j IN_BT"):
        save_cmd = """
            ...
            |awk '
                /^(\*|COMMIT)/ {print; next}
                !seen[$0]++'    \
            > /www/adcitra/panel/data/iptablesdata
        """
        if run_cmd(save_cmd):
            print("iptables 已保存")
...
def save_ipset():
    ...
    保存 ipset 规则
    """
    if run_cmd("ipset save | grep -E '_bt_' > /www/adcitra/panel/data/ipsetdata"):
        print("ipset 已保存")
