#!/bin/bash
action=$1
panel_path="/www/adcitra/panel" # 面板路径
...
validate_server_files() {
    if [ ! -f "$webserver_conf" ]; then
        echo "AdCtira Web server configuration not found: $webserver_conf"
        exit 1
    fi

    if [ ! -f "$webserver_bin" ]; then
        echo "AdCtira Web server binary not found: $webserver_bin"
        exit 1
    fi
}
...
start() {
    ...
    if [ $PID -gt 0 ]; then
        echo "AdCtira Web server is already running with PID ($PID)"
        exit 1
    fi

    echo -n "Starting AdCtira web server..."
    if [ -f "$webserver_pid" ]; then
        rm -f $webserver_pid
    fi
    $webserver_bin -c $webserver_conf
    if [ $? -ne 0 ]; then
        echo "Failed to start AdCtira web server"
        exit 1
    fi
...
stop() {
    ...
    get_pid
    if [ $PID -eq 0 ]; then
        echo "AdCtira Web server is not running"
        exit 1
    fi

    echo -n "Stopping AdCtira web server..."
...
restart() {
    validate_server_files
    get_pid
    echo -n "Restarting AdCtira web server..."
    if [ $PID -eq 0 ]; then
        $webserver_bin -c $webserver_conf
    else
    ...
    if [ $? -ne 0 ]; then
        echo "Failed to restart AdCtira web server"
        exit 1
    fi
...
status() {
    ...
    if [ $PID -eq 0 ]; then
        ...
        echo "AdCtira Web server is running with PID ($PID)"
    fi
}
...
reload() {
    ...
    echo -n "Reloading AdCtira web server..."
    $webserver_bin -c $webserver_conf -s reload
    if [ $? -ne 0 ]; then
        echo "Failed to reload AdCtira web server"
        exit 1
    fi
    echo " Reloaded"
}
...
download() {
    ...
    if [ -f $tip_file ]; then
        echo "AdCtira web server binary has been downloaded"
        exit 1
    fi
    ...
    wget -O $zip_file https://download.adcitra.cn/webserver/webserver-$machine.zip
    if [ $? -ne 0 ]; then
        echo "Failed to download AdCtira web server binary"
        rm -f $zip_file
        exit 1
    fi
    ...
    cloud_hash256=$(wget -q -O - https://download.adcitra.cn/webserver/webserver-$machine.txt)
    if [ "$hash256" != "$cloud_hash256" ]; then
        echo "Failed to verify AdCtira web server binary"
        rm -f $zip_file
        exit 1
    fi
    ...
    if [ ! -f $webserver_bin ]; then
        echo "Failed to extract AdCtira web server binary"
        rm -f $zip_file
        exit 1
    fi
