...
APACHE_CONF_DIRS = [
    "/www/adcitra/panel/vhost/apache"
]
...
class AutoApplyIPSSL:
    ...
    def __init__(self):
        ...
        self._bits = 2048
        self._conf_file_v2 = '/www/adcitra/panel/config/letsencrypt_v2.json'
        self._apis = None
        ...
        for auth_url in order_data['authorizations']:
            ...
            if mode:
                if mode == 'standalone':
                    ...
                    tmp_path = '/www/adcitra/panel/vhost/nginx/tmp_apply_ip_ssl.conf'
                    files = find_nginx_files_by_servername(ips[0])
                    if not files:
                        print("未找到相关Nginx配置文件，尝试创建临时配置文件...")
                        if not os.path.exists('/www/adcitra/panel/vhost/nginx'):
...
""".format(ips[0], challenge['token'], acme_keyauthorization))
                    ...
                    tmp_path = '/www/adcitra/panel/vhost/apache/tmp_apply_ip_ssl.conf'
                    files = find_apache_conf_files(ips[0])
                    if not files:
                        print("未找到相关Apache配置文件，尝试创建临时配置文件...")
                        if not os.path.exists('/www/adcitra/panel/vhost/apache'):
