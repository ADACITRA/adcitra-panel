...
function nothing_do() {
    local version=$1
    # 输出成功信息
    success "已完成[AdCtira-Panel-$version]处理"
}
...
function prepare_main() {
    ...
    11.5.*)
        ...
        success "已完成[AdCtira-Panel-$UPDATE_VERSION]处理"
        ;;
    11.6.*)
        install_package_for_11_5
        success "已完成[AdCtira-Panel-$UPDATE_VERSION]处理"
...
function install_package_for_11_5() {
    pip_bin="/www/adcitra/panel/pyenv/bin/python3 -m pip"
...
function after_main() {
    ...
    11.3.*)
        replace_bt_command
        success "已完成[AdCtira-Panel-$UPDATE_VERSION]启动检查处理"
        ;;
    11.5.*)
        install_after_11_5
        success "已完成[AdCtira-Panel-$UPDATE_VERSION]启动检查处理"
