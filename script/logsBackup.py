...
is_gzip = not os.path.exists('/www/adcitra/panel/data/log_not_gzip.pl')
...
def split_log(siteName, num):
    ...
    if 'nginx' in file_list:
        config_path = '/www/adcitra/panel/vhost/nginx/{}.conf'.format(res + siteName)
        ...
        config_path = '/www/adcitra/panel/vhost/apache/{}.conf'.format(res + siteName)
        config = public.readFile(config_path)
        if not config:
            print('|-正在处理网站:未检测到{}站点的日志'.format(siteName))
            return
