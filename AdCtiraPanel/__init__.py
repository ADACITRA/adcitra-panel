...
def request_check():
    ...
    if request.path.find('/static/') > 0:
        ...
        if request.path.find(_auth_path) == 0:
            static_file = public.get_panel_path() + '/AdCtiraPanel' + request.path.replace(_auth_path, '').replace('//', '/')
            ...
            static_file = public.get_panel_path() + '/AdCtiraPanel' + request.path.replace(new_auth_path, '').replace('//', '/')
            if not os.path.exists(static_file): return abort(404)
            return send_file(static_file, conditional=True, etag=True)
        elif _new_route.startswith(tuple(admin_path_checks)):
            if not session.get('login', False):
                return public.error_not_login()
            static_file = public.get_panel_path() + '/AdCtiraPanel' + request.path[len(_new_route):].replace('//','/')
    ...
    if request.path.find('/static/img/soft_ico/ico') >= 0:
        if not public.path_safe_check(request.path): return abort(404)  # 路径安全检查
        static_file = "{}/AdCtiraPanel/{}".format(panel_path, request.path)
        if not os.path.exists(static_file):
            static_file = "{}/AdCtiraPanel/static/img/soft_ico/icon_plug.svg".format(panel_path)
        return send_file(static_file, conditional=True, etag=True)

    # 处理登录成功状态，更新节点
    if 'login' in session and session['login'] == True:
        if not cache.get('bt_home_node'):
            public.run_thread(public.ExecShell, ('btpython /www/adcitra/panel/script/reload_check.py hour',))
...
During handling of the above exception, another exception occurred:'''
    ...
    if not session.get('login', None):
        ...
        if error_info.find("Traceback (most recent call last):") != -1 and os.path.exists("/www/adcitra/panel/data/debug.pl"):
            is_panel_error = True
        g.auth_error = True
  ...
  USER_AGENT: {user_agent}'''.format(
    ...
    if error_title != "连接云端服务失败!":
        result = public.readFile(
            ...
            '/AdCtiraPanel/templates/default/panel_error.html').format(
...
def adcitrawaf_error():
    ...
    if not os.path.exists(p_path):
        if get.name == 'adcitrawaf' and get.fun == 'index':
...
def send_favicon():
    ...
    s_file = '/www/adcitra/panel/AdCtiraPanel/static/favicon.ico'
...
def file_download_num(filename, num=0):
    if os.path.exists('/www/adcitra/panel/data/download_num.json'):
        try:
            data = json.loads(public.readFile('/www/adcitra/panel/data/download_num.json'))
        except:
            data = {}
    else:
        data = {}
    ...
    if num:
        public.writeFile('/www/adcitra/panel/data/download_num.json', json.dumps(data))
    return download_num
...
def panel_other(name=None, fun=None, stype=None):
    ...
    if name != "mail_sys" or fun != "send_mail_http.json":
        ...
        if not stype:
            ...
            stype = tmp[1]
        if fun:
            if name == 'adcitrawaf' and fun == 'index':
                pass
    ...
    try:
        ...
        try:
            ...
            if name == 'adcitrawaf' and fun == 'index' and str(ex).find(
                    '未购买') != -1:
                return render_template('error3.html', data={})
        ...
        if stype == 'json':  # 响应JSON
            ...
            r_type = type(data)
            if r_type == dict:
                if name == 'adcitrawaf' and 'msg' in data:
...
def sock_shell(ws):
    '''
        ...
        示例：
            ...
            p.send('ping www.adcitra.cn -c 100')
    '''
    comReturn = comm.local()
    if comReturn:
