...
class main:
    ...
    def check_task_status(self, response):
        ...
        if not os.path.exists("/www/adcitra/panel/data/userInfo.json"):
            return
        # 检查是否有邀请码
        if os.path.exists("/www/adcitra/panel/data/activity_inviter_id.pl"):
            inviter_id = public.readFile("/www/adcitra/panel/data/activity_inviter_id.pl")
            if inviter_id:
                from sslModel import certModel
                certModel.main().new_ssl_proxy_request(
                    {"url": "/api/v1/ssl/activity/user_pull_new", "inviter_id": inviter_id})
                # 拉新成功，重命名邀请码文件，避免重复拉新
                os.rename("/www/adcitra/panel/data/activity_inviter_id.pl", "/www/adcitra/panel/data/activity_inviter_id")
