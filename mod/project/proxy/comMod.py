    ...
    def __init__(self):
        ...
        self._init_proxy_conf = {
            ...
            "ssl_info": {
                ...
                "ssl_conf": '#error_page 404/404.html;\n    ssl_certificate    /www/adcitra/panel/vhost/cert/{site_name}/fullchain.pem;\n    ssl_certificate_key    /www/adcitra/panel/vhost/cert/{site_name}/privkey.pem;\n    ssl_protocols TLSv1.1 TLSv1.2 TLSv1.3;\n    ssl_ciphers EECDH+CHACHA20:EECDH+CHACHA20-draft:EECDH+AES128:RSA+AES128:EECDH+AES256:RSA+AES256:EECDH+3DES:RSA+3DES:!MD5;\n    ssl_prefer_server_ciphers on;\n    ssl_session_cache shared:SSL:10m;\n    ssl_session_timeout 10m;\n    add_header Strict-Transport-Security "max-age=31536000";\n    error_page 497  https://$host$request_uri;',
            ...
            "redirect": {
                ...
                "redirect_conf": "    #引用重定向规则，注释后配置的重定向代理将无效\n    include /www/adcitra/panel/vhost/nginx/redirect/{site_name}/*.conf;",
            },
            "proxy_log": {
                "log_type": "default",
                "server_port": "",
...
server {{
    ...
    def write_nginx_conf(self, get):
        ...
        monitor_conf = ""
        if os.path.exists("/www/adcitra/panel/plugin/monitor/monitor_main.py"):
    ...
    #Monitor-Config-End'''.format(pid=get.pid)
        ...
        if not os.path.exists("/www/adcitra/panel/vhost/nginx/extension/{}/".format(get.main_domain)):
            os.makedirs("/www/adcitra/panel/vhost/nginx/extension/{}/".format(get.main_domain), 0o600)
    ...
    def create(self, get):
        ...
        if not os.path.exists(get.site_path + "/proxy_cache_dir"):
            return public.returnResult(status=False,
                                       msg="创建缓存目录失败，请尝试手动创建{}是否成功，如果不成功请联系AdCtira灯塔运维".format(
    ...
    def delete(self, get):
        ...
        if os.path.exists("/www/adcitra/panel/vhost/nginx/extension/{}/".format(get.site_name)):
            import shutil
            shutil.rmtree("/www/adcitra/panel/vhost/nginx/extension/{}/".format(get.site_name))
    ...
    def get_list(self, get):
        ...
        adcitrawaf_path = '/www/server/adcitrawaf'

        try:
            config = json.loads(public.readFile(os.path.join(adcitrawaf_path, 'config.json')))
        except:
            config = {}

        try:
            waf_res = json.loads(public.readFile(os.path.join(adcitrawaf_path, "site.json")))
        except:
            waf_res = {}
...
{server_block}'''.format(
    ...
    def generate_config(self, get):
        ...
        # 2024/6/4 下午4:20 兼容新版监控报表的配置
        monitor_conf = ""
        if (os.path.exists("/www/adcitra/panel/plugin/monitor/monitor_main.py") and
    ...
    #Monitor-Config-End'''.format(sid=id)
                ...
                pass
        if not os.path.exists("/www/adcitra/panel/vhost/nginx/extension/{}/".format(get.site_name)):
            os.makedirs("/www/adcitra/panel/vhost/nginx/extension/{}/".format(get.site_name), 0o600)
