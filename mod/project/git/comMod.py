...
class main():
    ...
    def git_rollback(self, get):
        ...
        # 调用部署脚本
        deploy_script = f"btpython /www/adcitra/panel/mod/project/git/scripts/deploy.py {webhook_name} {branch} {commit}"
    ...
    def _create_webhook(self, site_id,site_name, branch):
        ...
        # 绑定git部署脚本
        ok = webhook_obj.AddHook(public.to_dict_obj({'title':f"{webhook_name}","shell": "btpython /www/adcitra/panel/mod/project/git/scripts/deploy.py $@"}))
        if not ok['status']:
            return "",""
