...
class main(DataManager,BaseUtil,ConfigManager):
    ...
    def add_backup(self,get):
        ...
        if backup_now:
            ...
            exec_command="echo 'nohup btpython /www/adcitra/panel/mod/project/backup_restore/backup_manager.py backup_data {} > /dev/null 2>&1 &'|at -t {}".format(int(get_time),at_time_str)
    ...
    def task_stop(self,get=None):
        ...
        if os.path.exists("/www/adcitra/panel/data/migration.pl"):
            public.ExecShell("rm -f /www/adcitra/panel/data/migration.pl")
    ...
    def exec_backup(self,get=None):
        if not hasattr(get, "timestamp"):
            return self.return_data(False,"参数错误","参数错误")
        timestamp=get.timestamp
        public.ExecShell("nohup btpython /www/adcitra/panel/mod/project/backup_restore/backup_manager.py backup_data {} > /dev/null 2>&1 &".format(int(timestamp)))
    ...
    def add_restore(self,get=None):
        if not hasattr(get, "timestamp"):
            return self.return_data(False,"参数错误","参数错误")
        timestamp=get.timestamp
        public.ExecShell("nohup btpython /www/adcitra/panel/mod/project/backup_restore/restore_manager.py restore_data {} > /dev/null 2>&1 &".format(int(timestamp)))
    ...
    def add_migrate_task(self,get=None):
        ...
        if auth_type == "password":
            ...
            public.ExecShell("nohup btpython /www/adcitra/panel/mod/project/backup_restore/ssh_manager.py --action migrate -H {server_ip} -P {ssh_port} -u {ssh_user} -p {escaped_password} --task-name '我的迁移任务' > /dev/null 2>&1 &".format(server_ip=server_ip,ssh_port=ssh_port,ssh_user=ssh_user,escaped_password=escaped_password))
        elif auth_type == "key":
            public.ExecShell("nohup btpython /www/adcitra/panel/mod/project/backup_restore/ssh_manager.py --action migrate -H {server_ip} -P {ssh_port} -u {ssh_user} --key-file {key_file} --task-name '我的迁移任务' > /dev/null 2>&1 &".format(server_ip=server_ip,ssh_port=ssh_port,ssh_user=ssh_user,key_file=key_file))
        ...
        if os.path.exists(self.migrate_task_json):
            ...
            if migrate_config ['run_status'] == 2:
                if migrate_config['run_type'] == "COMPLETED":
                    ...
                    migrate_config['migrate_msg'] = "灯塔面板安装完成！"
