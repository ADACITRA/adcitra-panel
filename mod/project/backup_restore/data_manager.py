...
class DataManager(SoftModule,BaseUtil):
    ...
    def get_ssh_list(self) -> dict:
        ...
        result = {}
        try:
            ssh_path="/www/adcitra/panel/config/ssh_info"
            ssh_user_command_file="/www/adcitra/panel/config/ssh_info/user_command.json"
    ...
    def get_plugin_list(self) -> list:
        plugin_list=['adcitrawaf','syssafe','monitor','tamper_core']
        result=[]
        for plugin in plugin_list:
            if os.path.exists("/www/adcitra/panel/plugin/{}".format(plugin)):
                if plugin == "adcitrawaf":
                    plugin_name="AdCtira灯塔防火墙"
                elif plugin == "syssafe":
                    plugin_name="系统加固"
                elif plugin == "monitor":
                    plugin_name="网站监控报表"
                ...
                result.append({
                    "name": plugin_name,
                    "size": self.get_file_size("/www/adcitra/panel/plugin/{}".format(plugin))
                })
        return result
