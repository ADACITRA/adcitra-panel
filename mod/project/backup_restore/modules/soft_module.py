...
class SoftModule(BaseUtil,ConfigManager):
    ...
    def get_pyenv_list(self):
        pyenv_path="/www/adcitra/panel/data/project_env.json"
    ...
    def install_web_server(self,timestamp):
        ...
        if web_server['name'] == 'nginx':
            ...
            result=public.ExecShell("cd /www/adcitra/panel/install && wget -O nginx.sh http://download.adcitra.cn/install/{}/nginx.sh && bash nginx.sh install {}".format(install_type,web_server['version']))
            ...
            restore_data['data_list']['soft']['status']['version'] = web_server['version']
            self.update_restore_data_list(timestamp, restore_data)
            result=public.ExecShell("cd /www/adcitra/panel/install && wget -O apache.sh http://download.adcitra.cn/install/{}/apache.sh && bash apache.sh install {}".format(install_type,web_server['version']))
    ...
    def install_php_server(self,timestamp):
        ...
        for php_ver in php_server:
            ...
            # elif isinstance(php_ver, (int, str)) and len(str(php_ver)) == 3:
            #     php_ver = f"{str(php_ver)[0]}.{str(php_ver)[1:]}"
            result=public.ExecShell("cd /www/adcitra/panel/install && wget -O php.sh http://download.adcitra.cn/install/{}/php.sh && bash php.sh install {}".format(install_type,php_ver))
        ...
        for php_server in php_server_data:
            ...
            for ext in php_ext:
                if ext not in filtered_data:
                    ...
                    install_cmd="cd /www/adcitra/panel/install && wget -O {ext}.sh http://download.adcitra.cn/install/1/{ext}.sh && bash {ext}.sh install {php_ver}".format(ext=ext,php_ver=php_ver)
                    if 'OPcache' in ext:
                        install_cmd="cd /www/adcitra/panel/install && wget -O opcache.sh http://download.adcitra.cn/install/1/opcache.sh && bash opcache.sh install {php_ver}".format(php_ver=php_ver)
                    
                    if 'SourceGuardian' in ext:
                        install_cmd="cd /www/adcitra/panel/install && wget -O sg16.sh http://download.adcitra.cn/install/1/sg16.sh && bash sg16.sh install {php_ver}".format(php_ver=php_ver)

                    if 'swoole' in ext:
                        if int(php_ver) >= 81:
                            install_cmd="cd /www/adcitra/panel/install && wget -O swoole6.sh http://download.adcitra.cn/install/1/swoole6.sh && bash swoole6.sh install {php_ver}".format(php_ver=php_ver)
                        elif int(php_ver) >= 80:
                            install_cmd="cd /www/adcitra/panel/install && wget -O swoole5.sh http://download.adcitra.cn/install/1/swoole5.sh && bash swoole5.sh install {php_ver}".format(php_ver=php_ver)
                        else:
                            install_cmd="cd /www/adcitra/panel/install && wget -O swoole4.sh http://download.adcitra.cn/install/1/swoole4.sh && bash swoole4.sh install {php_ver}".format(php_ver=php_ver)
    ...
    def install_node(self,timestamp):
        ...
        for node_data in node_list:
            ...
            if os.path.exists("/www/server/nodejs/{}".format(node_ver)):
                ...
                self.replace_log(log_str,new_log_str,"restore")
                continue
            result=public.ExecShell("cd /www/adcitra/panel/install && wget -O node_plugin_install.sh http://download.adcitra.cn/install/0/node_plugin_install.sh && bash node_plugin_install.sh {}".format(node_ver))
...
/www/server/nodejs/{node_ver}/bin/npm install {mod_name} -g &> /www/adcitra/panel/plugin/nodejs/exec.log           
    ...
    def install_mysql_server(self,timestamp):
        ...
        if mysql_server['type'] == 'mariadb':
            ...
            if os.path.exists("/www/server/mysql/bin/mysql"):
                ...
                self.replace_log(log_str,new_log_str,"restore")
                return
            result=public.ExecShell("cd /www/adcitra/panel/install && wget -O mysql.sh http://download.adcitra.cn/install/{}/mysql.sh && bash mysql.sh install {}".format(install_type,mysql_server['version']))
            if os.path.exists("/www/server/mysql/bin/mysql"):
                new_log_str="mysql-{} ✓".format(mysql_server['version'])
                self.replace_log(log_str,new_log_str,"restore")
            else:
    ...
    def install_jdk(self,timestamp):
        ...
        for jdk in jdk_list:
            ...
            self.update_restore_data_list(timestamp, restore_data)
            if "jdk" in jdk_ver:
                result=public.ExecShell("cd /www/adcitra/panel/install && wget -O jdk.sh http://download.adcitra.cn/install/0/jdk.sh && bash jdk.sh install {}".format(jdk_ver))
            else:
                result=public.ExecShell("cd /www/adcitra/panel/install && wget -O jdk.sh http://download.adcitra.cn/install/0/jdk.sh && bash jdk.sh install jdk-{}".format(jdk_ver))
    ...
    def install_mongodb_server(self,timestamp):
        ...
        self.update_restore_data_list(timestamp, restore_data)
        result=public.ExecShell("cd /www/adcitra/panel/install && wget -O mongodb.sh http://download.adcitra.cn/install/0/mongodb.sh && bash mongodb.sh install {}".format(mongodb_server['version']))
    ...
    def install_memcached_server(self,timestamp):
        ...
        self.update_restore_data_list(timestamp, restore_data)
        result=public.ExecShell("cd /www/adcitra/panel/install && wget -O memcached.sh http://download.adcitra.cn/install/0/memcached.sh && bash memcached.sh install")
        if os.path.exists("/usr/local/memcached/bin/memcached"):
            new_log_str="memcached-{} ✓".format(memcached_server['version'])
            self.replace_log(log_str,new_log_str,"restore")
        else:
    ...
    def install_redis_server(self,timestamp):
        ...
        self.update_restore_data_list(timestamp, restore_data)
        result=public.ExecShell("cd /www/adcitra/panel/install && wget -O redis.sh http://download.adcitra.cn/install/0/redis.sh && bash redis.sh install {}".format(redis_server['version']))
    ...
    def install_pgsql_server(self,timestamp):
        ...
        down_url="http://download.adcitra.cn/src/postgresql-{pgsql_version}.tar.gz".format(pgsql_version=pgsql_server['version'])

        result=public.ExecShell("cd /www/adcitra/panel/install && wget -O pgsql_install.sh http://download.adcitra.cn/install/0/pgsql_install.sh && bash pgsql_install.sh {} {}".format(down_file,down_url))
    ...
    def install_phpmyadmin(self,timestamp):
        ...
        self.update_restore_data_list(timestamp, restore_data)
        result=public.ExecShell("cd /www/adcitra/panel/install && wget -O phpmyadmin.sh http://download.adcitra.cn/install/0/phpmyadmin.sh && bash phpmyadmin.sh install {}".format(phpmyadmin_server['version']))
    ...
    def install_dotnet(self,timestamp):
        try:
            ...
            if not os.path.exists("/www/adcitra/panel/plugin/dotnet"):
                public.ExecShell(" cd /www/adcitra/panel/plugin;wget -O dotnet_T8Y5P.tar.gz http://download.adcitra.cn/src/dotnet_T8Y5P.tar.gz;tar -xvf dotnet_T8Y5P.tar.gz;rm -f dotnet_T8Y5P.tar.gz")
                if not os.path.exists("/www/adcitra/panel/plugin/dotnet"):
            ...
            for dotnet in dotnet_list:
                ...
                try:
                    ...
                    public.ExecShell('wget -O /www/adcitra/panel/plugin/dotnet/{} https://download.adcitra.cn/src/dotnet/{}'.format(dotnet_tar_file, dotnet_tar_file))
                    public.ExecShell('mkdir -p /www/server/dotnet/{}'.format(version))
                    public.ExecShell('tar -zxvf /www/adcitra/panel/plugin/dotnet/{} -C /www/server/dotnet/{}'.format(dotnet_tar_file, version))
                    public.ExecShell('rm -rf /www/adcitra/panel/plugin/dotnet/{}'.format(dotnet_tar_file))
    ...
    def install_python(self,timestamp):
        ...
        for python in python_list:
            ...
            if os.path.exists("/www/server/pyporject_evn/versions/{}/bin/python3".format(python_ver)) or os.path.exists("/www/server/pyporject_evn/versions/{}/bin/python2".format(python_ver)):
                ...
                self.replace_log(log_str,new_log_str,"restore")
                continue
            install_shell="/www/adcitra/panel/pyenv/bin/python3 /www/adcitra/panel/class/projectModel/adcitra_pyvm.py install {} --extend=''".format(python_ver)
    ...
    def install_ftp_server(self,timestamp):
        ...
        if os.path.exists("/www/server/pure-ftpd/bin/pure-pw"):
            ...
            return
        self.update_restore_data_list(timestamp, restore_data)
        result=public.ExecShell("cd /www/adcitra/panel/install && wget -O pureftpd.sh http://download.adcitra.cn/install/0/pureftpd.sh && bash pureftpd.sh install")
