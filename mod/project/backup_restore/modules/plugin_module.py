...
class PluginModule(BaseUtil,ConfigManager):
    def __init__(self):
        ...
        self.plugin_path = '/www/adcitra/panel/plugin'
    ...
    def backup_plugin_data(self,timestamp):
        ...
        plugin_info = {}
        adcitrawaf_path=os.path.join(self.plugin_path,"adcitrawaf")
        ...
        if os.path.exists(adcitrawaf_path):
            result=self.backup_adcitrawaf_data(timestamp)
            plugin_info['adcitrawaf']=result
    ...
    def backup_adcitrawaf_data(self,timestamp):
        backup_path=self.base_path + "/{timestamp}_backup/plugin/adcitrawaf".format(timestamp=timestamp)
        if not os.path.exists(backup_path):
            public.ExecShell('mkdir -p {}'.format(backup_path))

        try:
            adcitrawaf_info_json=json.loads(public.ReadFile(os.path.join(self.plugin_path,"adcitrawaf","info.json")))
            result={
                "status": 2,
                "err_msg": None,
                "version": adcitrawaf_info_json['versions'],
                "size": self.get_file_size(os.path.join(self.plugin_path,"adcitrawaf"))
            }
        
            public.ExecShell("\cp -rpa /www/server/adcitrawaf/config.json {backup_path}/config.json".format(backup_path=backup_path))
            public.ExecShell("\cp -rpa /www/server/adcitrawaf/site.json {backup_path}/site.json".format(backup_path=backup_path))
            public.ExecShell("\cp -rpa /www/server/adcitrawaf/rule {backup_path}/rule".format(backup_path=backup_path))
    ...
    def backup_monitor_data(self,timestamp):
        ...
        try:
            ...
            if os.path.exists("/www/adcitra/panel/plugin/monitor/site_robots.json"):
                public.ExecShell("\cp -rpa /www/adcitra/panel/plugin/monitor/site_robots.json {backup_path}/site_robots.json".format(backup_path=backup_path))
            if os.path.exists("/www/adcitra/panel/plugin/monitor/site_sitemap_info.json"):
                public.ExecShell("\cp -rpa /www/adcitra/panel/plugin/monitor/site_sitemap_info.json {backup_path}/site_sitemap_info.json".format(backup_path=backup_path))
            if os.path.exists("/www/adcitra/panel/plugin/monitor/spider_api.config"):
                public.ExecShell("\cp -rpa /www/adcitra/panel/plugin/monitor/spider_api.config {backup_path}/spider_api.config".format(backup_path=backup_path))
            if os.path.exists("/www/adcitra/panel/plugin/monitor/baidu_user.config"):
                public.ExecShell("\cp -rpa /www/adcitra/panel/plugin/monitor/baidu_user.config {backup_path}/baidu_user.config".format(backup_path=backup_path))
            if os.path.exists("/www/adcitra/panel/plugin/monitor/360_user.config"):
                public.ExecShell("\cp -rpa /www/adcitra/panel/plugin/monitor/360_user.config {backup_path}/360_user.config".format(backup_path=backup_path))
    ...
    def backup_tamper_core_data(self,timestamp):
        ...
        try:
            ...
            public.ExecShell("\cp -rpa /www/adcitra/panel/plugin/tamper_core/tamper_push_template.json {backup_path}/tamper_push_template.json".format(backup_path=backup_path))
            public.ExecShell("\cp -rpa /www/adcitra/panel/plugin/tamper_core/rule.json {backup_path}/rule.json".format(backup_path=backup_path))
    ...
    def backup_syssafe_data(self,timestamp):
        ...
        try:
            ...
            public.ExecShell("\cp -rpa /www/adcitra/panel/plugin/syssafe/config.json {backup_path}/config.json".format(backup_path=backup_path))
            public.ExecShell("\cp -rpa /www/adcitra/panel/plugin/syssafe/sys_process.json {backup_path}/sys_process.json".format(backup_path=backup_path))
            public.ExecShell("\cp -rpa /www/adcitra/panel/plugin/syssafe/config {backup_path}/".format(backup_path=backup_path))
            backup_size=self.format_size(self.get_file_size(backup_path))
    ...
    def restore_plugin_data(self,timestamp):
        ...
        self.print_log("如迁移机器未绑定AdCtira灯塔用户并升级为专业版或企业版，将会出现插件还原失败等情况", "restore")
        ...
        if 'adcitrawaf' in plugin_info and plugin_info['adcitrawaf']:
            log_str="开始安装nginx防火墙"
            self.print_log(log_str,"restore")
            restore_data['data_list']['plugin']['adcitrawaf']['restore_status']=1
            self.update_restore_data_list(timestamp,restore_data)
            plugin_version=plugin_info['adcitrawaf']['version']
            install_result=self.install_plugin('adcitrawaf',plugin_version)
            if install_result['status'] == True:
                new_log_str="nginx防火墙 ✓"
                self.replace_log(log_str,new_log_str,"restore")
                log_str="开始还原nginx防火墙数据"
                self.print_log(log_str,"restore")
                self.restore_adcitrawaf_data(timestamp)
                restore_data['data_list']['plugin']['adcitrawaf']['restore_status']=2
                self.update_restore_data_list(timestamp,restore_data)
                new_log_str="nginx防火墙数据 ✓"
                self.replace_log(log_str,new_log_str,"restore")
            else:
                restore_data['data_list']['plugin']['adcitrawaf']['restore_status']=3
                restore_data['data_list']['plugin']['adcitrawaf']['err_msg']=install_result['msg']
                self.update_restore_data_list(timestamp,restore_data)
                new_log_str="nginx防火墙 ✗"
                self.replace_log(log_str,new_log_str,"restore")
    ...
    def restore_adcitrawaf_data(self,timestamp):
        restore_path=self.base_path + "/{timestamp}_backup/plugin/adcitrawaf".format(timestamp=timestamp)
        plugin_path="/www/server/adcitrawaf"
        if  os.path.exists(restore_path) and os.path.exists(plugin_path):
            public.ExecShell("\cp -rpa {restore_path}/config.json /www/server/adcitrawaf/config.json".format(restore_path=restore_path))
            public.ExecShell("\cp -rpa {restore_path}/site.json /www/server/adcitrawaf/site.json".format(restore_path=restore_path))
            public.ExecShell("\cp -rpa {restore_path}/rule/* /www/server/adcitrawaf/rule".format(restore_path=restore_path))
    ...
    def restore_monitor_data(self,timestamp):
        ...
        plugin_path="/www/adcitra/panel/plugin/monitor/"
        if  os.path.exists(restore_path) and os.path.exists(plugin_path):
            public.ExecShell("\cp -rpa {restore_path}/site_robots.json /www/adcitra/panel/plugin/monitor/site_robots.json".format(restore_path=restore_path))
            public.ExecShell("\cp -rpa {restore_path}/site_sitemap_info.json /www/adcitra/panel/plugin/monitor/site_sitemap_info.json".format(restore_path=restore_path))
            public.ExecShell("\cp -rpa {restore_path}/spider_api.config /www/adcitra/panel/plugin/monitor/spider_api.config".format(restore_path=restore_path))
            public.ExecShell("\cp -rpa {restore_path}/baidu_user.config /www/adcitra/panel/plugin/monitor/baidu_user.config".format(restore_path=restore_path))
            public.ExecShell("\cp -rpa {restore_path}/360_user.config /www/adcitra/panel/plugin/monitor/360_user.config".format(restore_path=restore_path))
    ...
    def restore_tamper_core_data(self,timestamp):
        ...
        plugin_path="/www/adcitra/panel/plugin/tamper_core/"
        if  os.path.exists(restore_path) and os.path.exists(plugin_path):
            public.ExecShell("\cp -rpa {restore_path}/tamper_push_template.json /www/adcitra/panel/plugin/tamper_core/tamper_push_template.json".format(restore_path=restore_path))
            public.ExecShell("\cp -rpa {restore_path}/rule.json /www/adcitra/panel/plugin/tamper_core/rule.json".format(restore_path=restore_path))
    ...
    def restore_syssafe_data(self,timestamp):
        ...
        plugin_path="/www/adcitra/panel/plugin/syssafe/"
        if  os.path.exists(restore_path) and os.path.exists(plugin_path):
            public.ExecShell("\cp -rpa {restore_path}/config.json /www/adcitra/panel/plugin/syssafe/config.json".format(restore_path=restore_path))
            public.ExecShell("\cp -rpa {restore_path}/sys_process.json /www/adcitra/panel/plugin/syssafe/sys_process.json".format(restore_path=restore_path))
            public.ExecShell("\cp -rpa {restore_path}/config/* /www/adcitra/panel/plugin/syssafe/config".format(restore_path=restore_path))
            public.ExecShell("/etc/init.d/bt_syssafe stop")
