...
class SiteModule(DataManager,BaseUtil,ConfigManager):
    def __init__(self):
        ...
        self.site_dir_auth_path="/www/adcitra/panel/data/site_dir_auth.json"
        self.redirect_conf_path="/www/adcitra/panel/data/redirect.conf"
        self.proxy_conf_path="/www/adcitra/panel/data/proxyfile.json"
    ...
    def backup_site_config(self,site_backup_path,site_id):
        #直接备份站点db文件
        public.ExecShell("\cp -rpa /www/adcitra/panel/data/db/site.db {site_backup_path}site.db".format(site_backup_path=site_backup_path))
        ...
        public.ExecShell("\cp -rpa /www/adcitra/panel/data/db/default.db {site_backup_path}default.db".format(site_backup_path=site_backup_path))
        #备份加密访问配置文件
        if os.path.exists("/www/adcitra/panel/data/site_dir_auth.json"):
            public.ExecShell("\cp -rpa /www/adcitra/panel/data/site_dir_auth.json {site_backup_path}site_dir_auth.json".format(site_backup_path=site_backup_path))
        # 备份加密密码
        ...
        #备份重定向配置
        if os.path.exists("/www/adcitra/panel/data/redirect.conf"):
            public.ExecShell("\cp -rpa /www/adcitra/panel/data/redirect.conf {site_backup_path}redirect.conf".format(site_backup_path=site_backup_path))

        if os.path.exists("/www/adcitra/panel/data/proxyfile.json"):
            public.ExecShell("\cp -rpa /www/adcitra/panel/data/proxyfile.json {site_backup_path}proxyfile.json".format(site_backup_path=site_backup_path))
        ...
        #备份well-known文件
        if os.path.exists("/www/adcitra/panel/vhost/nginx/well-known"):
            public.ExecShell("\cp -rpa /www/adcitra/panel/vhost/nginx/well-known {site_backup_path}/well-known".format(site_backup_path=site_backup_path))

        public.ExecShell("mkdir -p {site_backup_path}/monitor_conf/".format(site_backup_path=site_backup_path))
        public.ExecShell("\cp -rpa /www/adcitra/panel/vhost/nginx/0.monitor*.conf {site_backup_path}/monitor_conf/".format(site_backup_path=site_backup_path))
    ...
    def restore_site_config(self,site_backup_path):
        ...
        if os.path.exists(site_db_file):
            public.ExecShell("\cp  -rpa {site_db_file} /www/adcitra/panel/data/db".format(site_db_file=site_db_file))
        if os.path.exists(default_db_file):
            public.ExecShell("\cp  -rpa {default_db_file} /www/adcitra/panel/data/default.db".format(default_db_file=default_db_file))
        if os.path.exists(dir_auth_file):
            public.ExecShell("mkdir -p /www/server/pass")
            public.ExecShell("\cp  -rpa {dir_auth_file} /www/adcitra/panel/data/site_dir_auth.json".format(dir_auth_file=dir_auth_file))
        ...
        if os.path.exists(redirect_file):
            public.ExecShell("\cp  -rpa {redirect_file} /www/adcitra/panel/data/redirect.conf".format(redirect_file=redirect_file))
        if os.path.exists(proxyfile_file):
            public.ExecShell("\cp  -rpa {proxyfile_file} /www/adcitra/panel/data/proxyfile.json".format(proxyfile_file=proxyfile_file))
        ...
        if os.path.exists(site_backup_path + "well-known"):
            if not os.path.exists("/www/adcitra/panel/vhost/nginx/well-known"):
                public.ExecShell("mkdir -p /www/adcitra/panel/vhost/nginx/well-known")   
            public.ExecShell("\cp -rpa {site_backup_path}well-known/* /www/adcitra/panel/vhost/nginx/well-known/".format(site_backup_path=site_backup_path))

        public.ExecShell("\cp -rpa {site_backup_path}monitor_conf/* /www/adcitra/panel/vhost/nginx/".format(site_backup_path=site_backup_path))
    ...
    def test_get_web_conf(self,site_name:str):
        conf_list=os.listdir("/www/adcitra/panel/vhost/nginx/")
    ...
    def backup_web_conf(self, site_name: str, conf_backup_path: str) -> None:
        ...
        # 定义需要备份的配置文件和路径映射
        conf_paths = {
            'cert': "/www/adcitra/panel/vhost/cert/{site_name}".format(site_name=site_name),
            'rewrite': "/www/adcitra/panel/vhost/rewrite/{site_name}.conf".format(site_name=site_name),
            'nginx': {
                'main': "/www/adcitra/panel/vhost/nginx/{site_name}.conf".format(site_name=site_name),
                'redirect': "/www/adcitra/panel/vhost/nginx/redirect/{site_name}".format(site_name=site_name),
                'proxy': "/www/adcitra/panel/vhost/nginx/proxy/{site_name}".format(site_name=site_name),
                'dir_auth': "/www/adcitra/panel/vhost/nginx/dir_auth/{site_name}".format(site_name=site_name)
            },
            'apache': {
                'main': "/www/adcitra/panel/vhost/apache/{site_name}.conf".format(site_name=site_name),
                'redirect': "/www/adcitra/panel/vhost/apache/redirect/{site_name}".format(site_name=site_name),
                'proxy': "/www/adcitra/panel/vhost/apache/proxy/{site_name}".format(site_name=site_name),
                'dir_auth': "/www/adcitra/panel/vhost/apache/dir_auth/{site_name}".format(site_name=site_name)
            }
        }
        ...
        rewrite_file_list = os.listdir("/www/adcitra/panel/vhost/rewrite/")
        for rewrite_file in rewrite_file_list:
            if rewrite_file.endswith(".conf"):  
                if site_name in rewrite_file:
                    public.ExecShell(f"\cp -rpa /www/adcitra/panel/vhost/rewrite/{rewrite_file} {conf_backup_path}rewrite")
        ...
        if not os.path.exists(nginx_paths['main']):
            print(site_name)
            web_conf_list=os.listdir("/www/adcitra/panel/vhost/nginx/")
            for web_conf_name in web_conf_list:
                if web_conf_name.endswith(".conf"):
                    if site_name in web_conf_name:
                        public.ExecShell(f"\cp -rpa /www/adcitra/panel/vhost/nginx/{web_conf_name} {conf_backup_path}nginx/")
        ...
        if not os.path.exists(apache_paths['main']):
            web_conf_list=os.listdir("/www/adcitra/panel/vhost/apache/")
            for web_conf_name in web_conf_list:
                if web_conf_name.endswith(".conf"):
                    if site_name in web_conf_name:
                        public.ExecShell(f"\cp -rpa /www/adcitra/panel/vhost/apache/{web_conf_name} {conf_backup_path}apache/")
        ...
        for site in restore_data['data_list']['site']:
            try:   
                ...
                if os.path.exists(site_conf_zip):
                    ...
                    public.ExecShell("cd {site_backup_path} && \cp -rpa {site_name}_conf/*  /www/adcitra/panel/vhost".format(site_backup_path=site_backup_path,site_name=site_name))
