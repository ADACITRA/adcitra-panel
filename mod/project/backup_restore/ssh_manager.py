...
class BtInstallManager:
    ...
    def install_bt_panel(self):
        """安装灯塔面板"""
        if self.task_id:
            update_migration_status(self.task_id, 'PANEL_INSTALL', message="开始安装灯塔面板")
            
        print("[*] 开始安装灯塔面板...")
        
        # 检查是否已安装灯塔面板
        # stdout, _ = self.exec_command("test -d /www/adcitra/panel && echo 'exists'")
        # if stdout is None:
        #     error_msg = "执行命令失败，无法检查灯塔面板是否已安装"
        #     if self.task_id:
        #         update_migration_status(self.task_id, 'PANEL_INSTALL', MIGRATION_STATUS['FAILED'], message=error_msg)
        #     return {"status": False, "msg": error_msg}
            
        # if 'exists' in stdout:
        #     message = "灯塔面板已安装，跳过安装步骤"
        #     print(f"[+] {message}")
        #     if self.task_id:
        ...
        # install_cmd = ""
        # if os_type == "centos":
        #     install_cmd = "yum install -y wget && wget -O install.sh https://download.adcitra.cn/install/install_6.0.sh && bash install.sh -y -P 8888"
        # elif os_type in ["ubuntu", "debian"]:
        #     install_cmd = "apt-get update && apt-get install -y wget && wget -O install.sh https://download.adcitra.cn/install/install-ubuntu_6.0.sh && echo 'y' | bash install.sh"
        #     install_cmd = "apt-get update && apt-get install -y wget && wget -O install.sh https://download.adcitra.cn/install/install-ubuntu_6.0.sh &&  bash install.sh -y -P 8888"
        # else:
        ...
        install_cmd = "if [ -f /usr/bin/curl ];then curl -sSO http://download.adcitra.cn/install/install_panel_backup_last.sh;else wget -O install_panel_backup_last.sh http://download.adcitra.cn/install/install_panel_backup_last.sh;fi;nohup bash install_panel_backup_last.sh -y -P 8888 > /root/bt_install.log 2>&1 &"
        
        if self.task_id:
            update_migration_status(self.task_id, 'PANEL_INSTALL', message=f"使用命令安装灯塔面板,请稍等...")
            
        print(f"[*] 使用命令安装灯塔面板: {install_cmd}")
        ...
        while time.time() - start_time < timeout:
            ...
            if stdout is None or stdout.strip() == "":
                ...
                if "安装完成" in stdout or "Installed successfully" in stdout:
                    ...
                    admin_path, stderr = self.exec_command(" cat /www/adcitra/panel/data/admin_path.pl")
                    ...
                    #update_panel_cmd = "wget -O git_ol.sh http://downooad-test.adcitra.cn/git_ol.sh;bash git_ol.sh LinuxPanel-9.6.0-9.6.0.zip"
                    #stdoutd, stderrd = self.exec_command(update_panel_cmd)
                    return {"status": True, "msg": message, "data": panel_info}
                else:
                    write_migration_log(f"灯塔面板安装失败 {self.host}:{self.port} 错误信息: {stdout}", self.task_id)
                    error_msg = "灯塔面板安装失败"
                    print(f"[!] {error_msg}")
                    if self.task_id:
                        update_migration_status(
    ...
    def restore_backup(self, timestamp):
        ...
        # 等待灯塔面板服务启动
        print("[*] 等待灯塔面板服务启动...")
        #self.exec_command("systemctl restart adcitra_panel")
        #time.sleep(5)
        
        # 检查还原模块是否存在
        restore_script = "/www/adcitra/panel/mod/project/backup_restore/restore_manager.py"
        ...
        touch_pl_cmd = "echo 'True'  > /www/adcitra/panel/data/migration.pl"
    ...
    def migrate(self, task_id):
        ...
        try:
            ...
            # 安装灯塔面板
            update_migration_status(task_id, 'PANEL_INSTALL', message="准备安装灯塔面板")
            write_migration_log("正在安装灯塔面板...预估5分钟....")
            install_result = self.install_bt_panel()
            if not install_result.get("status", False):
                return install_result
            write_migration_log("灯塔面板安装完成，正在启动备份任务...")
...
class BackupRestoreManager:
    ...
    def add_backup_task(self):
        ...
        if backup_now:
            print("[*] 执行备份命令...")
            public.ExecShell("nohup btpython /www/adcitra/panel/mod/project/backup_restore/backup_manager.py backup_data {} > /dev/null 2>&1 &".format(int(get_time)))
...
def parse_arguments():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='通过SSH连接新服务器，安装灯塔面板并进行备份还原')
    ...
    parser.add_argument('--panel-port', type=int, default=8888, help='灯塔面板端口，默认8888')
    ...
    parser.add_argument('--action', choices=['verify', 'install', 'upload', 'restore', 'migrate', 'all', 'status'], default='all', 
                       help='要执行的操作: verify=验证SSH连接, install=安装灯塔面板, upload=上传备份文件, restore=还原备份, migrate=执行完整迁移, status=获取任务状态, all=全部执行')
