...
class main:
    ...
    def slave_list(self, get=None):
        ...
        # 检查旧版本兼容性
        try:
            old_config_json = "/www/adcitra/panel/plugin/mysql_replicate/config.json.0"
            if os.path.exists(old_config_json):
                ...
                if slave_list and len(slave_list) > 0 and master_list and len(master_list) > 0:
                    return public.returnMsg(False, "当前主从版本不支持旧版主从同步，如需还原旧版本请在终端执行以下命令还原(不影响当前主从配置) 命令：wget -O remysql_slave.sh https://download.adcitra.cn/tools/remysql_slave.sh && bash remysql_slave.sh")
        except:
            pass
    ...
    def add_slave_server(self, get):
        ...
        if not os.path.exists("/www/adcitra/panel/config/mysql_slave_info"):
            public.ExecShell("mkdir -p /www/adcitra/panel/config/mysql_slave_info")
        ...
        if os.path.exists("/www/adcitra/panel/config/mysql_slave_info/log/{}.log".format(get.master_ip)):
            public.ExecShell("rm -rf /www/adcitra/panel/config/mysql_slave_info/log/{}.log".format(get.master_ip))
