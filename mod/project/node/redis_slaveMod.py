...
class main:
    ...
    def create_replication_group(self, get):
        ...
        # 创建配置目录
        if not os.path.exists("/www/adcitra/panel/config/redis_slave_info"):
            public.ExecShell("mkdir -p /www/adcitra/panel/config/redis_slave_info")
        # 启动创建工作流
    ...
    def get_available_nodes(self, get=None):
        """获取可用的Redis节点列表"""
        #_REPO_DIR="/www/adcitra/panel/data/mod_node_status_cache/"
        try:
