...
class SyncService:
    ...
    def add_slave_server(self, panel_addr: str, panel_key: str, master_ip: str, 
        ...
        if parsed_url.scheme not in ('http', 'https') or not parsed_url.netloc :
            return public.returnMsg(False, "请输入正确的面板地址！ 如：http://1.1.1.1:8888/ http://www.adcitra.cn:8888/")
        ...
        # 启动同步进程
        if sync_type == "auto":
            public.ExecShell("nohup btpython /www/adcitra/panel/mod/project/node/mysql_slave_util/start_sync.py auto_sync_data {} > /dev/null 2>&1 &".format(slave_ip))
        elif sync_type == "manual":
            public.ExecShell("nohup btpython /www/adcitra/panel/mod/project/node/mysql_slave_util/start_sync.py manual_sync_data {} > /dev/null 2>&1 &".format(slave_ip))
