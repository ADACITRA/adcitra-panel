...
class Acme_V2(Sites):
    ...
    _request_type = 'curl'
    _user_agent = "AdCtira/1.0 (+https://www.adcitra.cn)"
    _replay_nonce = None
    _acme_timeout = 30
    _verify = False
    ...
    def can_use_lua_for_site(self, site_name: str):
        ...
        rep_well_known = re.compile(
            r"(#.*\n)?\s*include\s+/www/adcitra/panel/vhost/nginx/well-known/.*\.conf;.*\n(#.*\n)?"
    ...
    def register(self, existing=False):
        if not 'email' in self._config:
            self._config['email'] = 'demo@adcitra.cn'
        if existing:
    ...
    def cloud_check_domain(self,domain):
        try:
            result = requests.post(public.get_home_node('https://www.adcitra.cn/api/panel/check_domain'),{"domain":domain,"ssl":1},s_type=self._request_type).json()
...
"""
    ...
    def write_ngin_authx_file(self, auth_to, token, acme_keyauthorization, index):
        ...
        file_check_config_path = "/www/adcitra/panel/vhost/nginx/well-known/{}.conf".format(site_name)
        if not os.path.exists("/www/adcitra/panel/vhost/nginx/well-known"):
            os.makedirs("/www/adcitra/panel/vhost/nginx/well-known", 0o755)

        # 如果主配置中，没有引用则尝试添加，添加失败就跳出
        if not re.search(r"\s*include\s+/www/adcitra/panel/vhost/nginx/well-known/.*\.conf;", nginx_conf, re.M):
            ...
            default_cert_apply_check = (
                ...
                "    include /www/adcitra/panel/vhost/nginx/well-known/{}.conf;\n"
    ...
    def sub_all_cert(self, key_file, pem_file):
        ...
        paths = ['/www/adcitra/panel/vhost/cert', '/www/adcitra/panel/vhost/ssl','/www/adcitra/panel']
        is_panel = False
        for path in paths:
            if not os.path.exists(path):
                continue
