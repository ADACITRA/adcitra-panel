...
class main():
    ...
    data = {}
    firewall_exec = "btpython /www/adcitra/panel/script/vhost_virtual.py"
    ...
    def create_account(self, args):
        ...
        @创建用户
        """
        public.set_module_logs('AdCtira灯塔多用户虚拟主机面板', 'create_account', 1)
    ...
    def get_service_info(self, args):
        ...
        if os.path.exists('{}/vhost_virtual'.format(self.setup_path)) and os.path.exists('{}/v-apache/bin/v-httpd'.format(self.setup_path)):
            ...
            if self.get_service_status_achieve():
                run_status = 1
        elif public.M('tasks').where('name=? and status !=?', ('安装 [AdCtira灯塔多用户虚拟主机面板]', 1)).count() > 0:
            install_status = 1
    ...
    def install_service(self, args):
        ...
        public.ExecShell("rm -f /www/adcitra/panel/install/vhost_virtual.sh;wget -O " + install_file + " " + download_url + " --no-check-certificate")
        if not os.path.exists(install_file):
            return public.return_message(-1, 0, public.lang('下载安装脚本失败'))
        if public.M('tasks').where('name=? and status=?', ('安装 [AdCtira灯塔多用户虚拟主机面板]', '0')).count() > 0:
            return public.return_message(-1, 0, public.lang('安装任务已存在'))
        else:
            execstr = "cd /www/adcitra/panel/install && /bin/bash vhost_virtual.sh install"
            public.M('tasks').add('id,name,type,status,addtime,execstr', (
            None, '安装 [AdCtira灯塔多用户虚拟主机面板]', 'execshell', '0', time.strftime('%Y-%m-%d %H:%M:%S'), execstr))
    ...
    def update_service(self, args):
        ...
        # 执行更新脚本
        cmd = 'cd /www/adcitra/panel/install && /bin/bash vhost_virtual.sh install'
        public.ExecShell(cmd)
