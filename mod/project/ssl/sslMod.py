...
class main:
    ...
    def get_dns_api_and_domains(self, get=None):
        ...
        # AdCtira灯塔域名
        dns_api_data.update({"bt": {"dns_name": "AdCtira灯塔DNS", "id": "bt", "dns_type": "AdCtira灯塔DNS", "domains": []}})
    ...
    def set_dns_record(self, domain_name, dns_value, dns_id=None, record_type="TXT",sub_domain=""):
        ...
        if dns_id == "bt":
            ...
            if not domain_data:
                return public.returnMsg(False, "域名【{}】非AdCtira灯塔域名，请确定域名是在AdCtira灯塔域名注册或已转入AdCtira灯塔域名后，在域名管理中刷新本地缓存后重试！".format(root))
