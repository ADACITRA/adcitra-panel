...
def syssafe_admin(func):
    def wrapper(*args, **kwargs):
        syssafe_flag = 0
        # 检查系统加固并且关闭
        if os.path.exists('/www/adcitra/panel/plugin/syssafe/init.sh'):
            res = public.ExecShell('bash /www/adcitra/panel/plugin/syssafe/init.sh status')
            if 'already running' in res[0]:
                try:
                    syssafe_flag = 1
                    public.ExecShell('bash /www/adcitra/panel/plugin/syssafe/init.sh stop')
                    res = public.ExecShell('bash /www/adcitra/panel/plugin/syssafe/init.sh status')
                    if 'already running' in res[0]:
                        import PluginLoader
        ...
        try:
            if syssafe_flag:
                public.ExecShell('bash /www/adcitra/panel/plugin/syssafe/init.sh start')
                res = public.ExecShell('bash /www/adcitra/panel/plugin/syssafe/init.sh status')
    ...
    def __is_installation(self, name: str) -> bool:
        ...
        if name == "postgres":
            ...
            if 'PostgreSQL' in res:
                pgsql = True
            Manager = False
            if os.path.exists('/www/adcitra/panel/plugin/pgsql_manager'):
                Manager = True
        ...
        if name == "tamper_proof":
            return os.path.exists('/www/adcitra/panel/plugin/tamper_proof')

        if name == "bt_security":
            return os.path.exists('/www/adcitra/panel/plugin/bt_security')

        if name == "syssafe":
            return os.path.exists('/www/adcitra/panel/plugin/syssafe')
