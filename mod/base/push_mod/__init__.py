...
def update_mod_push_system():
    ...
    try:
        push_data = json.loads(read_file("/www/adcitra/panel/class/push/push.json"))
    except:
        return
...
def _update_site_push(old_data: Dict[str, Dict[str, Union[str, int, float, list]]],
    ...
    if os.path.exists(login_send_type_conf):
        ...
        if os.path.exists("/www/adcitra/panel/data/login_send_type.pl"):
            send_type = read_file("/www/adcitra/panel/data/login_send_type.pl")
        else:
...
def _update_ftp_log_push(pmgr: PushManager, df_mdl: Dict[str, str]):
    old_file = "/www/adcitra/panel/data/analysis_config.json"
    try:
...
def _update_ftp_user_push(pmgr: PushManager, df_mdl: Dict[str, str]):
    old_file = "/www/adcitra/panel/data/ftp_push_config.json"
    try:
        data = json.loads(read_file(old_file))
    except:
        return
...
def _update_web_log_push(pmgr: PushManager, df_mdl: Dict[str, str]):
    old_file = "/www/adcitra/panel/data/cron_task_analysis.json"
    try:
...
def _update_site_content_push(pmgr: PushManager, df_mdl: Dict[str, str]):
    db_file = "/www/adcitra/panel/class/projectModel/content/content.db"
