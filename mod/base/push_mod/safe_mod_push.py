    ...
    def add_crontab(self, day, channel):
        ...
        if not DB('crontab').where('name=?',(cron_name,)).count():
            args = {
                ...
                "save_local": '1',
                "backupTo": '',
                "sBody": 'btpython /www/adcitra/panel/script/cron_scaning.py {}'.format(channel),
                "urladdress": ''
            }
            import crontab
    ...
    def add_crontab(self, hour, minute, channel):
        ...
        if not DB('crontab').where('name=?', (cron_name,)).count():
            args = {
                ...
                "save_local": '1',
                "backupTo": '',
                "sBody": 'btpython /www/adcitra/panel/script/cron_file.py {}'.format(channel),
                "urladdress": ''
            }
            import crontab
            res = crontab.crontab().AddCrontab(args)
...
class SafeCloudTask(BaseTask):
    _config_file = "/www/adcitra/panel/data/safeCloud/config.json"
