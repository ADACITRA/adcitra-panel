...
class _NgLog(_BaseLogFormat):
    ...
    def _set_log_format_include(self) -> Optional[str]:
        ...
        rep_include = re.compile(r"include\s+/www/adcitra/panel/vhost/nginx/log_format/\*\.conf\s*;")
    ...
    def set_site_log_path(self, site_name, site_log_path, conf_prefix, mutil=False) -> Optional[str]:
        ...
        # nginx
        nginx_config_path = '/www/adcitra/panel/vhost/nginx/{}{}.conf'.format(conf_prefix, site_name)
    ...
    def get_site_log_path(self, site_name, conf_prefix) -> Union[str, dict]:
        config_path = '/www/adcitra/panel/vhost/nginx/{}{}.conf'.format(conf_prefix, site_name)
...
class _ApLog(_BaseLogFormat):
    ...
    def set_site_log_path(self, site_name, site_log_path, conf_prefix, mutil=False) -> Optional[str]:
        ...
        # apache
        apache_config_path = '/www/adcitra/panel/vhost/apache/{}{}.conf'.format(conf_prefix, site_name)
    ...
    def get_site_log_path(self, site_name, conf_prefix) -> Union[str, dict]:
        config_path = '/www/adcitra/panel/vhost/apache/{}{}.conf'.format(conf_prefix, site_name)
        config = read_file(config_path)
        if not config:
            return "站点配置文件丢失"
    ...
    def _set_log_format_include(self) -> Optional[str]:
        ...
        if not os.path.isdir(self._log_format_dir):
            os.makedirs(self._log_format_dir)
        rep_include = re.compile(r"IncludeOptional\s+/www/adcitra/panel/vhost/apache/log_format/\*\.conf")
        if rep_include.search(config_data):
            return
        new_conf = config_data + """
<IfModule log_config_module>
    IncludeOptional /www/adcitra/panel/vhost/apache/log_format/*.conf
</IfModule>
"""
        write_file(config_file, new_conf)
