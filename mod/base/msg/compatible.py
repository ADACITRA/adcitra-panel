...
def get_sender_by_id(channel_id: str) -> Optional[WarpToOld]:
    ...
    if re.match(r"^[0-9a-f]{16}$", channel_id):
        ...
        if sender_config["sender_type"] == "webhook":
            return WarpWebHookMsg(WebHookMsg(sender_config), "AdCtira灯塔Web Hook告警通知")
        elif sender_config["sender_type"] == "mail":
            return WarpToOld(MailMsg(sender_config), "AdCtira灯塔邮件告警通知")
        elif sender_config["sender_type"] == "weixin":
            return WarpToOld(WeiXinMsg(sender_config), "AdCtira灯塔微信告警通知")
        elif sender_config["sender_type"] == "feishu":
            return WarpToOld(FeiShuMsg(sender_config), "AdCtira灯塔飞书告警通知")
        elif sender_config["sender_type"] == "dingding":
            return WarpToOld(DingDingMsg(sender_config), "AdCtira灯塔钉钉告警通知")
        elif sender_config["sender_type"] == "wx_account":
            return WarpWeChatAccountMsg(WeChatAccountMsg(sender_config), "AdCtira灯塔告警通知")
        return None
