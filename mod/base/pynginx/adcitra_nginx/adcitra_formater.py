...
class BtNginxConf:
    ...
    def test_nginx(self, nginx_bin_path: str = None ):
        ...
        vhost_path = "/www/adcitra/panel/vhost"
...
class _Formatter:
    ...
    def to_do_warning(self) -> List[str]:
        ...
        def _check_lua_package_path(directive: Directive):
            if directive.get_name() in ("lua_package_path", "lua_package_cpath"):
                is_bt_waf = any([("adcitrawaf" in i or "/www/server/nginx" in i) for i in directive.get_parameters()])
                if not is_bt_waf:
    ...
    def _format_sites(self):
        ...
        for site in sites:
            if site.server_names == ['phpmyadmin']:  # phpmyadmin 跳过 会在主配置文件中处理
                self._format_php_site(site)  # 解析但不保留，每次重新生成符合AdCtira灯塔的phpmyadmin配置
                continue
    ...
    def set_ssl_and_ext_and_rewrite(self, site: Union[_PHPSite, _ProxySite, _StaticSite]):
        ...
        # extension
        extension_conf_path = "/www/adcitra/panel/vhost/nginx/extension/{}/*.conf".format(site.name)
        ...
        # CERT-APPLY-CHECK
        apply_conf_path = "/www/adcitra/panel/vhost/nginx/well-known/{}.conf".format(site.name)
        ...
        if conf_pre in ('php_', "proxy_"):
            conf_pre = ""
        rewrite_conf_path = "/www/adcitra/panel/vhost/rewrite/{}{}.conf".format(conf_pre, site.name)
        ...
        def _prep_block(
            ...
            for d in b.directives:
                if d.__class__ is Include:
                    ...
                    # 引入的是AdCtira灯塔的配置文件
                    if d.include_path.startswith("/www/adcitra/panel/vhost/"):
                        for c in d.configs:
                            c.file_path = os.path.join(
                                self._tmp_path, "vhost",
                                c.file_path[len("/www/adcitra/panel/vhost/"):]
                            )
    ...
    def set_php_info(self, site: _PHPSite):
        ...
        args_path = "/www/adcitra/panel/vhost/other_php/{}/enable-php-other.conf".format(site.name)
        server.block.directives[idx] = Include(
            name="include",
    ...
    def _set_main_bt_conf(self):
        ...
        http.directives.append(
            Include(
                name="include",
                include_path="/www/adcitra/panel/vhost/nginx/*.conf",
                parameters=["/www/adcitra/panel/vhost/nginx/*.conf"],
                configs=[]
            )  # 不实际引入
        )
        return
